...
type: docs
weight: 60
url: /pythonnet/working-solution-for-chart-resizing-in-pptx/
---
...
{{% /alert %}} 
## **Background**
In [previous article](/slides/pythonnet/creating-excel-chart-and-embedding-it-in-presentation-as-ole-object/) , we have explained how to create an Excel Chart using Aspose.Cells for .NET and further embed this chart in a PowerPoint Presentation using Aspose.Slides for Python via .NET. In order to accommodate the [object changed issue](/slides/pythonnet/updating-ole-objects-automatically-using-ms-powerpoint-add-in/) , we assigned the chart image to the Chart OLE Object Frame. In the output presentation, when we double click the OLE Object Frame showing the Chart Image, the Excel Chart is activated. The end users can make any desired changes in the actual Excel Workbook and then return to the concerned Slide by clicking outside the activated Excel Workbook. The size of the OLE Object Frame will change when the user gets back to the slide. The resizing factor will be different for different sizes of OLE Object Frame and embedded Excel Workbook. 
## **Cause of Resizing**
...
## **Working Solution**
There are two possible scenarios for creation of the PowerPoint Presentations using Aspose.Slides for Python via .NET. 
