...
- Python
- Aspose.Slides
description: "Aspose.Slides for Python via .NET 提供完整的功能集，包括管理文字、圖形、表格和動畫、為投影片加入音訊與影片、投影片預覽，以及匯出為 SVG、PDF 等格式。"
---
{{% alert color="primary" %}}
...
Aspose.Slides for Python via .NET 是一個功能強大的類別庫，使您的應用程式能在不需要 Microsoft PowerPoint® 的情況下讀取和寫入 PowerPoint® 簡報。

它是首個也是唯一提供完整 PowerPoint® 文件管理功能的元件，專為 Python 開發人員設計。

Aspose.Slides for Python via .NET 包含廣泛的功能，例如處理文字、圖形、表格和動畫；加入音訊與影片；預覽投影片；以及將投影片匯出為 SVG、PDF 等格式。
...
Aspose.Slides for Python via .NET 是一個功能強大的 Python 程式庫，讓您能在未安裝 Microsoft PowerPoint 的情況下，以程式方式建立、編輯與轉換 PowerPoint 簡報 (PPT、PPTX、ODP)。

### Aspose.Slides 支援哪些簡報功能？

此程式庫支援管理文字、圖形、表格、圖表、動畫、母片投影片、音訊、影片等。它亦支援投影片預覽、渲染、列印，並可匯出為 PDF、SVG、HTML 與影像等格式。

### 我可以使用 Aspose.Slides 將簡報轉換成其他格式嗎？

可以。Aspose.Slides 能將 PowerPoint 檔案高保真且高效能地轉換為 PDF、SVG、HTML、JPG、PNG、TIFF 以及其他格式。

### 使用 Aspose.Slides 必須安裝 Microsoft PowerPoint 嗎？
...
### 我要如何開始使用 Aspose.Slides for Python？

您可以透過 PyPi 安裝，並參考 [開發者指南](/slides/zh-hant/python-net/developer-guide/) 取得範例、API 參考與教學，快速上手。