...
TIFF（**Tagged Image File Format**）是一種廣泛使用的無損光柵圖像格式，以其卓越的品質和對圖形的細緻保留而聞名。設計師、攝影師以及桌面出版人員常選擇 TIFF 以在圖像中保留圖層、顏色精確度和原始設定。

使用 Aspose.Slides，您可以輕鬆地將 PowerPoint 投影片 (PPT、PPTX) 與 OpenDocument 投影片 (ODP) 直接轉換為高品質的 TIFF 圖像，確保您的簡報保留最高的視覺忠實度。

## **將簡報轉換為 TIFF**

使用由 [Presentation](https://reference.aspose.com/slides/zh-hant/php-java/aspose.slides/presentation/) 類別提供的 [save](https://reference.aspose.com/slides/zh-hant/php-java/aspose.slides/presentation/#save) 方法，您可以快速將整個 PowerPoint 簡報轉換為 TIFF。產生的 TIFF 圖像對應預設投影片大小。
...
在 [TiffOptions](https://reference.aspose.com/slides/zh-hant/php-java/aspose.slides/tiffoptions/) 類別中的方法 [setBwConversionMode](https://reference.aspose.com/slides/zh-hant/php-java/aspose.slides/tiffoptions/#setBwConversionMode) 允許您指定在將彩色投影片或影像轉換為黑白 TIFF 時所使用的演算法。請注意，此設定僅在 [setCompressionType](https://reference.aspose.com/slides/zh-hant/php-java/aspose.slides/tiffoptions/#getCompressionType) 方法設定為 `CCITT4` 或 `CCITT3` 時套用。

{{% alert color="info" title="注意" %}}
[TiffOptions::setBwConversionMode](https://reference.aspose.com/slides/zh-hant/php-java/aspose.slides/tiffoptions/#setBwConversionMode) 是一個匯出層級的設定，用於為整個 TIFF 圖像選擇像素轉換演算法。若要在啟用黑白顯示模式時定義單一圖形的顯示方式，請使用 [Shape::setBlackWhiteMode](https://reference.aspose.com/slides/zh-hant/php-java/aspose.slides/shape/#setBlackWhiteMode)。有關範例，請參閱 [Control Black-and-White Rendering for Shapes](/php-java/shape-formatting/#control-black-and-white-rendering-for-shapes)。
{{% /alert %}}
...
如果您需要具備特定尺寸的 TIFF 圖像，您可以使用 [TiffOptions](https://reference.aspose.com/slides/zh-hant/php-java/aspose.slides/tiffoptions/) 中提供的方法設定所需的值。例如，[setImageSize](https://reference.aspose.com/slides/zh-hant/php-java/aspose.slides/tiffoptions/#getImageSize) 方法允許您定義產生圖像的大小。
...
以下程式碼示範如何將 PowerPoint 簡報轉換為具備自訂像素格式的 TIFF 圖像：
...
沒有，Aspose.Slides 對投影片數量不設任何限制。您可以將任何規模的簡報轉換為 TIFF 格式。

**將投影片轉換為 TIFF 時，PowerPoint 的動畫和過場效果會被保留嗎？**

不會，TIFF 為靜態圖像格式。因此，動畫與過場效果不會被保留；僅會匯出投影片的靜態快照。