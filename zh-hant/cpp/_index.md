---
title: Aspose.Slides for C++
second_title: Aspose.Slides for C++
type: docs
weight: 30
url: /zh-hant/cpp/
keywords:
- 文件說明
- 簡報處理
- 簡報轉換
- PowerPoint
- OpenDocument
- C++
- Aspose.Slides
description: Aspose.Slides for C++ 是一個 Microsoft PowerPoint® 管理 API，允許 C++ 應用程式在不使用 Microsoft PowerPoint® 的情況下讀寫 PowerPoint® 文件。
is_root: true
---
{{% alert color="primary" %}}

![Aspose.Slides for C++ Product Logo](home_1.png)

**歡迎使用 Aspose.Slides for C++**

Aspose.Slides for C++ 是一個 Microsoft PowerPoint® 管理 API，允許 C++ 應用程式在不使用 Microsoft PowerPoint® 的情況下讀寫 PowerPoint® 文件。Aspose.Slides for C++ 是首個也是唯一提供管理 PowerPoint® 文件功能的元件。Aspose.Slides for C++ 提供許多關鍵功能，例如管理文字、圖形、表格與動畫、將音訊與影片加入投影片、預覽投影片、將投影片匯出為 SVG、PDF 格式等。

{{% /alert %}}

## **Aspose.Slides for C++ 資源**

以下是實用資源的連結：

- [Aspose.Slides for C++ 在線說明文件](/slides/zh-hant/cpp/)
- [Aspose.Slides for C++ 功能](/slides/zh-hant/cpp/features-overview/)
- [Aspose.Slides for C++ 版本說明](https://releases.aspose.com/slides/zh-hant/cpp/release-notes/)
- [Aspose.Slides for C++ 產品頁面](https://products.aspose.com/slides/zh-hant/cpp/)
- [下載 Aspose.Slides for C++](https://releases.aspose.com/slides/zh-hant/cpp/)
- [安裝 Aspose.Slides for C++ NuGet 套件](https://www.nuget.org/packages/Aspose.Slides.CPP/)
- [Aspose.Slides for C++ API 參考指南](https://reference.aspose.com/slides/zh-hant/cpp)
- [從 GitHub 庫下載範例](https://github.com/aspose-slides/Aspose.Slides-for-C)
- [Aspose.Slides for C++ 免費支援論壇](https://forum.aspose.com/c/slides/zh-hant/11)
- [Aspose.Slides for C++ 付費支援服務台](https://helpdesk.aspose.com/)