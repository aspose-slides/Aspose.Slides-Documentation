---
title: Aspose.Slides for Java 15.10.0 的公共 API 及不向後相容的變更
...
此頁面列出所有[已新增](/slides/zh-hant/java/public-api-and-backwards-incompatible-changes-in-aspose-slides-for-java-15-10-0/)或[已移除](/slides/zh-hant/java/public-api-and-backwards-incompatible-changes-in-aspose-slides-for-java-15-10-0/)的類別、方法、屬性等，並列出隨 Aspose.Slides for Java 15.10.0 API 引入的其他變更。

{{% /alert %}} 
## **公共 API 變更**
#### **已在 ISequence 加入圖表序列動畫 API**
...
新增了兩個與圖表元素動畫相關的列舉型別 EffectChartMajorGroupingType 與 EffectChartMinorGroupingType。

若要為圖表新增序列動畫，可使用以下程式碼。範例檔案中的圖表有三個序列，因此對每個索引 0 到 2 各加入一個效果：
...
```
#### **新增 com.aspose.slides.VideoPlayerHtmlController 以支援將多媒體檔案匯出為 HTML**
已新增公開類別 com.aspose.slides.VideoPlayerHtmlController。使用此類別的實例即可將影片與音訊檔案匯出為 HTML。

VideoPlayerHtmlController 建構函式接受以下參數：

- path：產生影片與音訊檔案的路徑（資料夾必須已存在）
- fileName：HTML 檔案的名稱
