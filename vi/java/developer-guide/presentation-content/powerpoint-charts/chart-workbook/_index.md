...
Nó cũng đề cập tới việc làm việc với sổ làm việc bên ngoài như nguồn dữ liệu cho biểu đồ. Các ví dụ minh họa cách tạo và gán một sổ làm việc bên ngoài, lấy đường dẫn của sổ làm việc bên ngoài được liên kết với biểu đồ, và chỉnh sửa dữ liệu biểu đồ khi sổ làm việc khả dụng.
...
Đoạn mã Java dưới đây minh họa một thao tác mẫu:
...
Đoạn mã Java dưới đây minh họa một thao tác trong đó phương thức [IChartDataWorkbook.Worksheets](https://reference.aspose.com/slides/vi/java/com.aspose.slides/IChartDataWorkbook#getWorksheets--) được sử dụng để truy cập bộ sưu tập worksheet:
...
Đoạn mã Java dưới đây cho bạn cách chỉ định một kiểu cho nguồn dữ liệu:
...
try {
    ...
    for (IShape shape : slide.getShapes()) {
        ...
        if (chartData.getDataSourceType() == ChartDataSourceType.InternalWorkbook &&
            ...
            // Workbook được nhúng có định dạng .xlsb, không được hỗ trợ.
            continue;
        }
...
Đoạn mã Java dưới đây minh họa quá trình tạo workbook bên ngoài:
...
Mặc dù bạn không thể chỉnh sửa dữ liệu trong các workbook được lưu trữ ở vị trí từ xa hoặc tài nguyên, bạn vẫn có thể dùng các workbook đó làm nguồn dữ liệu bên ngoài. Nếu đường dẫn tương đối cho một workbook bên ngoài được cung cấp, nó sẽ tự động được chuyển sang đường dẫn đầy đủ.

Đoạn mã Java dưới đây cho bạn cách gán một workbook bên ngoài:
...
Tham số thứ hai (`boolean`) của phương thức `setExternalWorkbook` được dùng để chỉ định xem workbook Excel có được tải hay không. 

* Khi giá trị được đặt thành `false`, chỉ đường dẫn workbook được cập nhật — dữ liệu biểu đồ sẽ không được tải hoặc cập nhật từ workbook đích. Bạn có thể muốn sử dụng cài đặt này khi workbook đích không tồn tại hoặc không khả dụng. 
* Khi giá trị được đặt thành `true`, dữ liệu biểu đồ sẽ được cập nhật từ workbook đích.
...
Bạn có thể chỉnh sửa dữ liệu trong workbook bên ngoài theo cùng cách như chỉnh sửa nội dung của workbook nội bộ. Khi một workbook bên ngoài không thể tải, một ngoại lệ sẽ được ném ra.

Đoạn mã Java dưới đây là một triển khai của quy trình đã mô tả:
...
**Tôi có thể xác định được một biểu đồ cụ thể có liên kết tới workbook bên ngoài hay là workbook nhúng không?**

Có. Một biểu đồ có [kiểu nguồn dữ liệu](https://reference.aspose.com/slides/vi/java/com.aspose.slides/chartdata/#getDataSourceType--) và một [đường dẫn tới workbook bên ngoài](https://reference.aspose.com/slides/vi/java/com.aspose.slides/chartdata/#getExternalWorkbookPath--); nếu nguồn là một workbook bên ngoài, bạn có thể đọc đường dẫn đầy đủ để chắc chắn rằng đang sử dụng một tệp bên ngoài.

**Các đường dẫn tương đối tới workbook bên ngoài có được hỗ trợ không, và chúng được lưu như thế nào?**

Có. Nếu bạn chỉ định một đường dẫn tương đối, nó sẽ tự động được chuyển sang đường dẫn tuyệt đối. Điều này thuận tiện cho việc di động dự án; tuy nhiên, hãy nhớ rằng bản trình bày sẽ lưu đường dẫn tuyệt đối trong tệp PPTX.

**Tôi có thể sử dụng các workbook nằm trên tài nguyên/mạng chia sẻ không?**

Có, các workbook như vậy có thể được dùng làm nguồn dữ liệu bên ngoài. Tuy nhiên, việc chỉnh sửa trực tiếp các workbook từ xa thông qua Aspose.Slides không được hỗ trợ — chúng chỉ có thể được dùng làm nguồn.

**Aspose.Slides có ghi đè lên file XLSX bên ngoài khi lưu bản trình bày không?**

Không. Bản trình bày lưu một [liên kết tới file bên ngoài](https://reference.aspose.com/slides/vi/java/com.aspose.slides/chartdata/#getExternalWorkbookPath--) và sử dụng nó để đọc dữ liệu. File bên ngoài không bị thay đổi khi bản trình bày được lưu.

**Nếu file bên ngoài được bảo vệ bằng mật khẩu, tôi nên làm gì?**

Aspose.Slides không chấp nhận mật khẩu khi liên kết. Một cách thường gặp là loại bỏ bảo vệ trước hoặc chuẩn bị một bản sao đã giải mã (ví dụ, bằng cách sử dụng [Aspose.Cells](/cells/java/)) và liên kết tới bản sao đó.

**Nhiều biểu đồ có thể tham chiếu cùng một workbook bên ngoài không?**

Có. Mỗi biểu đồ lưu liên kết riêng của mình. Nếu chúng đều trỏ tới cùng một file, việc cập nhật file đó sẽ được phản ánh trong mỗi biểu đồ vào lần tải dữ liệu tiếp theo.