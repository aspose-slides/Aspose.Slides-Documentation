---
title: شکل گروهی
type: docs
weight: 170
url: /fa/java/examples/elements/group-shape/
keywords:
- مثال کد
- شکل گروهی
- PowerPoint
- OpenDocument
- ارائه
- Java
- Aspose.Slides
description: "مدیریت اشکال گروه‌بندی شده در Aspose.Slides برای Java: ایجاد، تو در تو کردن، تراز کردن، ترتیب‌گذاری مجدد و استایل‌دهی به اشکال گروهی با مثال‌های Java در ارائه‌های PPT، PPTX و ODP."
---
مثال‌هایی برای ایجاد گروه‌هایی از اشکال، دسترسی به آن‌ها، جداسازی و حذف با استفاده از **Aspose.Slides for Java**.

## **اضافه کردن شکل گروهی**

یک گروه شامل دو شکل پایه ایجاد کنید.

```java
static void addGroupShape() {
    Presentation presentation = new Presentation();
    try {
        ISlide slide = presentation.getSlides().get_Item(0);

        IGroupShape group = slide.getShapes().addGroupShape();
        group.getShapes().addAutoShape(ShapeType.Rectangle, 0, 0, 50, 50);
        group.getShapes().addAutoShape(ShapeType.Ellipse, 60, 0, 50, 50);
    } finally {
        presentation.dispose();
    }
}
```

## **دسترسی به شکل گروهی**

اولین شکل گروهی را از یک اسلاید بازیابی کنید.

```java
static void accessGroupShape() {
    Presentation presentation = new Presentation();
    try {
        ISlide slide = presentation.getSlides().get_Item(0);

        IGroupShape group = slide.getShapes().addGroupShape();
        group.getShapes().addAutoShape(ShapeType.Rectangle, 0, 0, 50, 50);

        IGroupShape firstGroup = null;
        for (IShape shape : slide.getShapes()) {
            if (shape instanceof IGroupShape) {
                firstGroup = (IGroupShape) shape;
                break;
            }
        }
    } finally {
        presentation.dispose();
    }
}
```

## **حذف شکل گروهی**

یک شکل گروهی را از اسلاید حذف کنید.

```java
static void removeGroupShape() {
    Presentation presentation = new Presentation();
    try {
        ISlide slide = presentation.getSlides().get_Item(0);

        IGroupShape group = slide.getShapes().addGroupShape();

        slide.getShapes().remove(group);
    } finally {
        presentation.dispose();
    }
}
```

## **جداسازی اشکال**

اشکال را از داخل یک مخزن گروه خارج کنید.

```java
static void ungroupShapes() {
    Presentation presentation = new Presentation();
    try {
        ISlide slide = presentation.getSlides().get_Item(0);

        IGroupShape group = slide.getShapes().addGroupShape();
        IAutoShape rect = group.getShapes().addAutoShape(ShapeType.Rectangle, 0, 0, 50, 50);

        // انتقال شکل خارج از گروه.
        slide.getShapes().addClone(rect);
        group.getShapes().remove(rect);
    } finally {
        presentation.dispose();
    }
}
```