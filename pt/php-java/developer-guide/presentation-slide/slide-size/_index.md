...
Depois de alterar o tamanho do slide de uma apresentação, o conteúdo dos slides (imagens ou objetos, por exemplo) pode ficar distorcido. Por padrão, os objetos são redimensionados automaticamente para se ajustarem ao novo tamanho do slide. Entretanto, ao mudar o tamanho do slide de uma apresentação, você pode especificar uma configuração que determina como o Aspose.Slides lida com o conteúdo nos slides.
...
- `EnsureFit`
  ...
  Se você deseja dimensionar para um tamanho de slide menor e precisa que o Aspose.Slides reduza os objetos dos slides para garantir que todos caibam nos slides (assim, você evita perder conteúdo), use esta configuração.
...
Sim. Dimensões maiores de slide (em pontos) combinadas com uma escala de renderização mais alta levam a um aumento no consumo de memória e a tempos de processamento mais longos. Procure um tamanho de slide prático e ajuste a escala de renderização somente quando necessário para alcançar a qualidade de saída desejada.

**Posso definir um tamanho de slide não padrão e, em seguida, mesclar slides de apresentações que têm tamanhos diferentes?**

Você não pode [mesclar apresentações](/slides/pt/php-java/merge-presentation/) enquanto eles têm tamanhos de slide diferentes — primeiro, redimensione uma apresentação para corresponder à outra. Ao mudar o tamanho do slide, você pode escolher como o conteúdo existente será tratado via a opção [SlideSizeScaleType](https://reference.aspose.com/slides/pt/php-java/aspose.slides/slidesizescaletype/). Depois de alinhar os tamanhos, você pode mesclar os slides preservando a formatação.
