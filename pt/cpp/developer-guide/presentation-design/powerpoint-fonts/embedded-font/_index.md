...
**Fontes incorporadas no PowerPoint** ajudam a garantir que sua apresentação mantenha a aparência pretendida ao ser aberta em qualquer sistema ou dispositivo. Isso é especialmente importante ao usar fontes personalizadas, de terceiros ou não‑padrão para branding ou propósitos criativos. Sem fontes incorporadas, o texto pode ser substituído, layouts podem quebrar e caracteres podem aparecer como símbolos ilegíveis ou retângulos, comprometendo o design geral.

Aspose.Slides for C++ fornece um conjunto de APIs poderosas para gerenciar fontes incorporadas programaticamente. Você pode usar a classe [FontsManager](https://reference.aspose.com/slides/pt/cpp/aspose.slides/fontsmanager/) e a classe [FontData](https://reference.aspose.com/slides/pt/cpp/aspose.slides/fontdata/) para inspecionar, adicionar ou remover fontes incorporadas em seus arquivos de apresentação. Além disso, a classe [Compress](https://reference.aspose.com/slides/pt/cpp/aspose.slides.lowcode/compress/) permite otimizar o tamanho do arquivo comprimindo os dados de fonte sem afetar a qualidade ou a aparência.

Essas ferramentas dão controle total sobre a incorporação de fontes, ajudando a manter tipografia consistente em diferentes plataformas enquanto reduz o tamanho do arquivo quando necessário.

## **Obter fontes incorporadas de uma apresentação**

Aspose.Slides for C++ fornece o método `GetEmbeddedFonts` através da classe [FontsManager](https://reference.aspose.com/slides/pt/cpp/aspose.slides/fontsmanager/), que permite recuperar uma lista de fontes incorporadas em uma apresentação PowerPoint. Isso pode ser útil para auditoria de uso de fontes, garantir conformidade com diretrizes de marca ou verificar se todas as fontes necessárias estão incluídas corretamente antes de compartilhar o arquivo.
...
Normalmente não — elas quase sempre estão disponíveis. Mas para plena portabilidade em ambientes “light” (Docker, um servidor Linux sem fontes pré‑instaladas), incorporar fontes do sistema pode eliminar o risco de substituições inesperadas.