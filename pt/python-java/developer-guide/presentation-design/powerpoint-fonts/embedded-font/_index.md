---
title: Incorporar fontes em apresentações em Python via Java
linktitle: Fontes incorporadas
type: docs
weight: 40
url: /pt/python-java/embedded-font/
keywords:
- adicionar fonte
- incorporar fonte
...
- Java
- Aspose.Slides
description: "Gerencie fontes incorporadas no PowerPoint com Aspose.Slides para Python via Java. Adicione, recupere, remova e compacte fontes para preservar a aparência do texto e reduzir o tamanho do arquivo."
---
## **Introdução**
...
Aspose.Slides for Python via Java lets you retrieve, add, and remove embedded fonts through the [FontsManager](https://reference.aspose.com/slides/pt/python-java/aspose.slides/fontsmanager/) class returned by [Presentation.getFontsManager](https://reference.aspose.com/slides/pt/python-java/aspose.slides/presentation/#getFontsManager). You can also reduce the size of embedded font data by removing characters that the presentation does not use.
...
Use [getEmbeddedFonts](https://reference.aspose.com/slides/pt/python-java/aspose.slides/fontsmanager/#getEmbeddedFonts) to list the fonts stored in a presentation. To remove one, pass a font from that list to [removeEmbeddedFont](https://reference.aspose.com/slides/pt/python-java/aspose.slides/fontsmanager/#removeEmbeddedFont), then save the presentation.
...
Use the [FontsManager](https://reference.aspose.com/slides/pt/python-java/aspose.slides/fontsmanager/) class to inspect fonts before embedding them. Call [FontsManager.getFonts](https://reference.aspose.com/slides/pt/python-java/aspose.slides/fontsmanager/#getFonts) to retrieve the fonts used in the presentation. For each font, pass a [FontData](https://reference.aspose.com/slides/pt/python-java/aspose.slides/fontdata/) object and the required [FontStyleType](https://reference.aspose.com/slides/pt/python-java/aspose.slides/fontstyletype/) value to [FontsManager.getFontBytes](https://reference.aspose.com/slides/pt/python-java/aspose.slides/fontsmanager/#getFontBytes). The method returns the binary data for that font style, or `None` when the requested font or style is unavailable. Do not pass a `None` result to [FontsManager.getFontEmbeddingLevel](https://reference.aspose.com/slides/pt/python-java/aspose.slides/fontsmanager/#getFontEmbeddingLevel), because that method requires a byte array.

[EmbeddingLevel](https://reference.aspose.com/slides/pt/python-java/aspose.slides/embeddinglevel/) is a flags enumeration that reports the embedding restrictions stored in the font:
...
Use [addEmbeddedFont](https://reference.aspose.com/slides/pt/python-java/aspose.slides/fontsmanager/#addEmbeddedFont) to embed a font. Its overloads accept either a [FontData](https://reference.aspose.com/slides/pt/python-java/aspose.slides/fontdata/) object or a byte array containing the font data. The [EmbedFontCharacters](https://reference.aspose.com/slides/pt/python-java/aspose.slides/embedfontcharacters/) enumeration controls which characters are included:

- [All](https://reference.aspose.com/slides/pt/python-java/aspose.slides/embedfontcharacters/) embeds all characters in the font. Use this option when recipients need to edit the presentation and enter new text.
- [OnlyUsed](https://reference.aspose.com/slides/pt/python-java/aspose.slides/embedfontcharacters/) embeds only the characters used in the presentation to reduce file size. Choose this option for a finished presentation that is primarily intended for viewing.

The following example uses [getFonts](https://reference.aspose.com/slides/pt/python-java/aspose.slides/fontsmanager/#getFonts) to retrieve the fonts used in `Fonts.pptx` and embeds those that are not already embedded. The fonts to add must be available on the machine running the code. Existing embedded fonts retain their current character sets.
...
[Compress.compressEmbeddedFonts](https://reference.aspose.com/slides/pt/python-java/aspose.slides/compress/#compressEmbeddedFonts) reduces embedded font data by removing unused characters. It operates on fonts that are already embedded, so the size reduction depends on how much unused font data the presentation contains.
...
Call [getSubstitutions](https://reference.aspose.com/slides/pt/python-java/aspose.slides/fontsmanager/#getSubstitutions) in the environment where you render the presentation to see which fonts Aspose.Slides will replace. Also check font substitution settings and font fallback rules. Fallback handles missing characters, so embedding a font does not resolve characters that the font itself does not contain.
