---
title: Guia do desenvolvedor
type: docs
weight: 20
url: /pt/python-net/developer-guide/
keywords:
- guia do desenvolvedor
- gerenciar apresentação
- design de apresentação
- apresentação profissional
- criar apresentação
- visualizador de apresentação
- segurança da apresentação
- propriedades da apresentação
- visualizar propriedades
- comentários da apresentação
- notas da apresentação
- animação
- apresentação 3D
- projeto VBA
- processamento de imagem
- PowerPoint
- OpenDocument
- apresentação
- Python
- Aspose.Slides
description: "Descubra o Aspose.Slides para Python via .NET Guia do Desenvolvedor para APIs passo a passo que permitem criar, editar e converter apresentações PPT, PPTX e ODP com facilidade."
---