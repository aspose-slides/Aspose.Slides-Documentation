...
Registrar fontes personalizadas para renderização é independente de incorporar fontes em um arquivo PPTX. Se uma fonte precisar ser armazenada dentro da própria apresentação, use os recursos de incorporação de fontes explicitamente.
...
Aspose.Slides fornece o método [GetFontFolders](https://reference.aspose.com/slides/pt/net/aspose.slides/fontsloader/getfontfolders/) para que você encontre pastas de fontes. Esse método retorna as pastas adicionadas através do método `LoadExternalFonts` e as pastas de fontes do sistema.
...
Aspose.Slides fornece a propriedade [DocumentLevelFontSources](https://reference.aspose.com/slides/pt/net/aspose.slides/loadoptions/documentlevelfontsources/) para que você indique fontes externas que serão usadas com a apresentação.
...
Aspose.Slides fornece o método [LoadExternalFont](https://reference.aspose.com/slides/pt/net/aspose.slides/fontsloader/loadexternalfont/)(byte[] data) para que você carregue fontes externas a partir de dados binários.
...
Não. Registrar uma fonte para renderização não é o mesmo que incorporá‑la em um PPTX. Se precisar que a fonte seja transportada dentro do arquivo da apresentação, use os recursos explícitos de **incorporação de fontes**.

**Posso controlar o comportamento de fallback quando uma fonte personalizada não contém determinados glifos?**
...
> **Observação para Linux/Docker**: Ao chamar `FontsLoader.LoadExternalFonts`, verifique se cada entrada no array `directories` contém um caminho não vazio para um diretório existente. Se uma variável de ambiente usada para construir o caminho da fonte estiver indefinida ou vazia, o Aspose.Slides pode tentar resolver o valor vazio como um caminho completo, resultando em `System.ArgumentException`.

**E quanto à licença — posso incorporar qualquer fonte personalizada sem restrições?**

Você é responsável por garantir a conformidade com a licença da fonte. Os termos variam; algumas licenças proíbem a incorporação ou uso comercial. Sempre revise a EULA da fonte antes de distribuir os resultados.