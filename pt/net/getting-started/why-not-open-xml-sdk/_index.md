---
title: Por que não usar Open XML SDK
type: docs
weight: 50
...
- Conversões de alta qualidade envolvendo todos os formatos populares de apresentações PowerPoint suportados, incluindo conversão para PDF, XPS, TIFF e impressão.  
...
- Controle (extenso) e gerenciamento da formatação de texto em níveis de TextFrames, Paragraphs e Portions.  
...
## **Compare Open XML SDK com Aspose.Slides**
...
|<p>Renderização e impressão:</p><p>- Renderizar apresentações para PDF, PDF Notes, XPS, imagens TIFF.</p><p>- Renderizar miniaturas de slides para PNG, JPEG, BMP, SVG e TIFF.</p><p>- Especificar resolução, qualidade, compressão e outras opções de imagem.</p><p>- Imprimir apresentações usando a infraestrutura de impressão do .NET. O componente possui método de impressão integrado para imprimir as apresentações como mostrado na visualização de impressão do MS PowerPoint.</p>|Não|Sim|
|Plataformas suportadas|Windows, .NET|Windows, Linux, Java, .NET, Mono|

## **Conclusão**
Open XML SDK e Aspose.Slides não competem diretamente porque atendem a necessidades consideravelmente diferentes e visam públicos distintos. 

{{% alert color="info" %}} 

Open XML SDK é uma biblioteca de classes que fornece uma forma tipada para trabalhar com documentos OOXML, enquanto Aspose.Slides é uma biblioteca de processamento de apresentações incrivelmente útil que oferece grande suporte para quase todos os formatos de arquivos do Microsoft PowerPoint. 
...
Se precisar executar tarefas complexas em documentos—como as tarefas da lista abaixo—então o Aspose.Slides é a melhor opção. 
