...
Aspose.Slides fornece ferramentas abrangentes para ajustar o tamanho do slide e a proporção em apresentações do PowerPoint, fundamentais tanto para impressão quanto para exibição em tela.

Tamanhos e proporções de slides populares:
...
As páginas de notas e de folhetos têm dimensões separadas das slides regulares. Veja [Tamanho da página de notas](/slides/pt/java/notes-size/) para alterar seu tamanho e orientação.
...
Se os tamanhos de slide comuns (4:3 e 16:9) não forem adequados ao seu trabalho, você pode optar por usar um tamanho de slide específico ou exclusivo. Por exemplo, se pretende imprimir slides em tamanho real a partir da sua apresentação em um layout de página personalizado ou exibir sua apresentação em determinados tipos de tela, é provável que se beneficie ao usar uma configuração de tamanho personalizado para sua apresentação.

Este código de exemplo mostra como usar Aspose.Slides for Java para especificar um tamanho de slide personalizado para uma apresentação em Java:
...
- `EnsureFit`
  ...
  Se você deseja dimensionar para um slide menor e precisa que o Aspose.Slides reduza os objetos dos slides para garantir que todos caibam nos slides (assim, você evita perda de conteúdo), use esta configuração.

- `Maximize`

  Se você deseja dimensionar para um slide maior e precisa que o Aspose.Slides aumente os objetos dos slides para que sejam proporcionais ao novo tamanho do slide, use esta configuração.
...
Sim. Dimensões de slide maiores (em pontos) combinadas com uma escala de renderização mais alta levam a maior consumo de memória e tempos de processamento mais longos. Procure um tamanho de slide prático e ajuste a escala de renderização apenas quando necessário para alcançar a qualidade de saída desejada.

**Posso definir um tamanho de slide não padrão e depois mesclar slides de apresentações que têm tamanhos diferentes?**

Você não pode [mesclar apresentações](/slides/pt/java/merge-presentation/) enquanto elas possuem tamanhos de slide diferentes — primeiro, redimensione uma apresentação para coincidir com a outra. Ao mudar o tamanho do slide, você pode escolher como o conteúdo existente será tratado via a opção [SlideSizeScaleType](https://reference.aspose.com/slides/pt/java/com.aspose.slides/slidesizescaletype/). Após alinhar os tamanhos, você pode mesclar os slides mantendo a formatação.
