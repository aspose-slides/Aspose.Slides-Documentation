...
Efekty WordArt pozwalają dodać atrakcyjny, stylizowany tekst do prezentacji PowerPoint. Dzięki Aspose.Slides programiści mogą programowo tworzyć, dostosowywać i zarządzać WordArt tak, jak w Microsoft PowerPoint — bez konieczności instalowania Office. Ten artykuł przedstawia przegląd pracy z WordArt, w tym jak stosować transformacje tekstu, style wypełnień, obrysy, cienie i inne opcje formatowania, aby treść prezentacji była bardziej wyrazista i angażująca. WordArt umożliwia traktowanie tekstu jako obiektu graficznego. Składa się z efektów lub specjalnych modyfikacji stosowanych do tekstu, aby uczynić go atrakcyjniejszym lub bardziej widocznym.
...
```
Teraz ustawiamy wysokość czcionki tekstu na większą wartość, aby efekt był bardziej zauważalny, za pomocą tego kodu:
...
Z menu po prawej stronie możesz wybrać predefiniowany efekt WordArt. Z menu po lewej możesz określić ustawienia nowego WordArt. 
...
Tutaj stosujemy wzorzec koloru [SmallGrid](https://reference.aspose.com/slides/pl/java/com.aspose.slides/PatternStyle#SmallGrid) do tekstu i dodajemy czarną obwódkę o szerokości 1, korzystając z tego kodu:
...
Z interfejsu programu możesz zastosować te efekty do tekstu, bloku tekstu, kształtu lub podobnego elementu:

![todo:image_alt_text](image-20200930114129-5.png)

Na przykład efekty Cień, Odbicie i Poświata mogą być zastosowane do tekstu; efekty Format 3D i Obrót 3D mogą być zastosowane do bloku tekstu; właściwość Miękkie krawędzie może być zastosowana do obiektu Shape (działa również wtedy, gdy nie jest ustawiona właściwość Format 3D). 

### **Stosowanie efektów cienia**

Tutaj zamierzamy ustawić właściwości dotyczące wyłącznie tekstu. Stosujemy efekt cienia do tekstu przy użyciu tego kodu w języku Java:
...
Przy użyciu PresetShadow możesz zastosować cień do tekstu (korzystając z predefiniowanych wartości). 
...
Aspose.Slides rzeczywiście pozwala zastosować jednocześnie dwa typy cieni: InnerShadow i PresetShadow.

**Uwaga:**

- Gdy OuterShadow i PresetShadow są używane razem, stosowany jest tylko efekt OuterShadow. 
- Jeśli OuterShadow i InnerShadow są używane jednocześnie, wynikowy efekt zależy od wersji PowerPoint. Na przykład w PowerPoint 2013 efekt zostaje podwojony. Natomiast w PowerPoint 2007 stosowany jest efekt OuterShadow. 

### **Stosowanie wyświetlania do tekstów**

Dodajemy wyświetlanie do tekstu przy użyciu tego przykładu kodu w języku Java:
...
Możesz zmienić parametry cienia, wyświetlania i poświaty. Właściwości efektów są ustawiane oddzielnie dla każdej części tekstu. 
...
Zarówno Microsoft PowerPoint, jak i Aspose.Slides dla Javy udostępniają określoną liczbę predefiniowanych typów transformacji. 
...
Rezultujący tekst i jego kształt:
...
Rozważmy scenę dla tekstu i kształtu zawierającego ten tekst. Efekt 3D obejmuje reprezentację obiektu 3D oraz scenę, na której obiekt został umieszczony. 

- Gdy scena jest ustawiona zarówno dla figury, jak i tekstu, scenie figury przyznawany jest wyższy priorytet — scena tekstu jest ignorowana. 
...
## **Zastosowanie efektu zewnętrznego cienia do tekstów**
Aspose.Slides dla Javy udostępnia klasy [**IOuterShadow**](https://reference.aspose.com/slides/pl/java/com.aspose.slides/ioutershadow/) i [**IInnerShadow**](https://reference.aspose.com/slides/pl/java/com.aspose.slides/iinnershadow/), które umożliwiają zastosowanie efektów cienia do tekstu zawartego w [TextFrame](https://reference.aspose.com/slides/pl/java/com.aspose.slides/textframe/). Wykonaj poniższe kroki:
...
Poniższy kod w języku Java — implementacja powyższych kroków — pokazuje, jak zastosować efekt zewnętrznego cienia do tekstu:
...
Poniższy kod (oparty na powyższych krokach) pokazuje, jak zastosować efekt wewnętrznego cienia do tekstu w kształcie w języku Java:
...
### Czy mogę stosować efekty WordArt do elementów mastera slajdu?

Tak, możesz stosować efekty WordArt do kształtów na slajdach głównych, w tym do miejsc wypełniania tytułu, stopek lub tekstu tła. Zmiany w układzie mastera będą odzwierciedlane na wszystkich powiązanych slajdach.
