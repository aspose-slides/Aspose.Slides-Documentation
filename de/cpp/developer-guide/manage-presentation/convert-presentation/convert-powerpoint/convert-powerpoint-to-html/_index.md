...
- C++
- Aspose.Slides
description: "PowerPoint-Präsentationen in responsives HTML in C++ konvertieren. Layout, Links und Bilder mit der Aspose.Slides-Konvertierungsanleitung bewahren für schnelle, fehlerfreie Ergebnisse."
---

## **Übersicht**

Dieser Artikel erklärt, wie Sie PowerPoint‑Präsentationen mithilfe von C++ in das HTML‑Format konvertieren. Er behandelt die folgenden Themen.
...
Für C++‑Beispielcode zur Konvertierung von PowerPoint nach HTML siehe bitte den untenstehenden Abschnitt, d.h.[PowerPoint in HTML konvertieren](#convert-powerpoint-to-html). Der Code kann mehrere Formate wie PPT, PPTX und ODP im Presentation‑Objekt laden und in das HTML‑Format speichern.
...
**Aspose.Slides** stellt viele Optionen (größtenteils aus der [**HtmlOptions**](https://reference.aspose.com/slides/cpp/class/aspose.slides.export.html_options)-Klasse) bereit, die den PowerPoint‑nach‑HTML‑Konvertierungsprozess definieren:
...
* Eine PowerPoint‑Präsentation nach HTML konvertieren, während der neue CSS‑Stil verwendet wird.  
...
Neben den hier beschriebenen Konvertierungsprozessen unterstützt Aspose.Slides außerdem diese Konvertierungsoperationen, die das HTML‑Format betreffen: 
...
## **PowerPoint in HTML mit Originalschriften konvertieren**

Aspose.Slides stellt die Klasse [EmbedAllFontsHtmlController](https://reference.aspose.com/slides/cpp/class/aspose.slides.export.embed_all_fonts_html_controller) bereit, die es ermöglicht, beim Konvertieren einer Präsentation nach HTML alle Schriftarten einzubetten.

Um zu verhindern, dass bestimmte Schriftarten eingebettet werden, können Sie dem parametrisierten Konstruktor der Klasse [EmbedAllFontsHtmlController](https://reference.aspose.com/slides/cpp/class/aspose.slides.export.embed_all_fonts_html_controller) ein Array von Schriftartnamen übergeben. Beliebte Schriftarten wie Calibri oder Arial müssen, wenn sie in einer Präsentation verwendet werden, nicht eingebettet werden, da die meisten Systeme diese bereits enthalten. Werden diese Schriftarten eingebettet, wird das resultierende HTML‑Dokument unnötig groß.
...
Standardmäßig gibt Aspose.Slides beim Konvertieren von PowerPoint nach HTML ein kleines HTML mit Bildern bei 72 DPI und entfernten zugeschnittenen Bereichen aus. Um HTML‑Dateien mit höherwertigen Bildern zu erhalten, müssen Sie die Eigenschaft `PicturesCompression` (aus der Klasse `HtmlOptions`) auf 96 (d. h. `PicturesCompression::Dpi96`) oder höhere [Werte](https://reference.aspose.com/slides/cpp/namespace/aspose.slides.export#adc51ca67b7e5c99f6fad75b02ebfd6d8) setzen.

Dieser C++‑Code zeigt, wie Sie eine PowerPoint‑Präsentation nach HTML konvertieren und dabei hochwertige Bilder mit 150 DPI erhalten (d. h. `PicturesCompression::Dpi150`):
```cpp
...
Dieser Code in C++ zeigt, wie Sie HTML mit Vollqualitäts‑Bildern ausgeben:
```cpp
auto pres = System::MakeObject<Presentation>(u"InputDoc.pptx");
...
Mit neuen CSS‑Stildateien können Sie das Aussehen der aus dem PowerPoint‑nach‑HTML‑Konvertierungsprozess erzeugten HTML‑Datei leicht ändern. 

Der C++‑Code in diesem Beispiel zeigt, wie Sie überschreibbare Methoden nutzen, um ein benutzerdefiniertes HTML‑Dokument mit einem Link zu einer CSS‑Datei zu erstellen:
``` cpp
...
Dieser C++‑Code zeigt, wie Sie ein PowerPoint nach HTML konvertieren, dabei alle Schriftarten verlinken und „Calibri“ sowie „Arial“ ausschließen (da diese bereits im System vorhanden sind):
```cpp
...
public:
        ...
        String fontStyle, String fontWeight, ArrayPtr<uint8_t> fontData)
    {
        ...
        String path = String::Format(u"{0}.woff", fontName); // ggf. muss der Pfad bereinigt werden
...
Die Leistung hängt von Größe und Komplexität der Präsentationen ab. Aspose.Slides ist für Batch‑Operationen äußerst effizient und skalierbar. Um bei der Konvertierung vieler Präsentationen optimale Leistung zu erzielen, wird empfohlen, nach Möglichkeit Multithreading oder Parallelverarbeitung zu nutzen.
...
Bei der Verwendung von Aspose.Slides gibt es keine Begrenzung der Folienzahl. Sie können Präsentationen jeder Größe konvertieren. Bei Präsentationen mit einer sehr hohen Folienzahl kann die Leistung jedoch von den verfügbaren Ressourcen Ihres Servers oder Systems abhängen.