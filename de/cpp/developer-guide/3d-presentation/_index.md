...
- C++
- Aspose.Slides
description: "Erstellen Sie mühelos interaktive 3D‑Präsentationen in C++ mit Aspose.Slides. Schnell in PowerPoint‑ und OpenDocument‑Formate exportieren für vielseitige Verwendung."
---

## **Übersicht**
Seit Aspose.Slides 20.9 ist es möglich, PowerPoint‑3D‑Modelle zu erstellen und zu bearbeiten. Dies kann erreicht werden, indem 2D‑Formen eine Reihe von 3D‑Effekten verliehen werden. Durch das Erstellen einer Kameraperspektive auf die Form können Sie sie um die Achse drehen. Durch das Erzeugen einer Extrusion oder Tiefe der Form wird die Form von einer 2D‑Form in ein 3D‑Modell umgewandelt. Das Einstellen des Lichteffekts auf die 3D‑Form oder das Ändern der Materialien kann ihr ein lebendigeres Aussehen verleihen. Das Ändern der Farben von 3D‑Modellen zu einem 3D‑Verlauf, das Anpassen der Kontur von Formen, das Hinzufügen eines Bevels macht das 3D‑Modell voluminöser. Alle 3D‑Effekte können sowohl auf PowerPoint‑3D‑Modelle als auch auf Texte angewendet werden.
...
## **3D‑Drehung**
In PowerPoint ist die Formdrehung verfügbar über:

![todo:image_alt_text](img_02_01.png)

Um PowerPoint‑3D‑Modelle zu drehen, ist es erforderlich, eine Kameraperspektive auf die Form zu erstellen. Dies wird mit der Methode [IThreeDFormat.get_Camera()](https://reference.aspose.com/slides/cpp/class/aspose.slides.three_d_format#ad2f989bd1fd64fd4136e1f17660035d4) durchgeführt. Die Drehmethode wird aus der Kameraklasse aufgerufen, als würden Sie die Kamera drehen. Tatsächlich drehen Sie, wenn Sie die Kamera relativ zur Form drehen, die Form in der 3D‑Ebene.
``` cpp
auto shape = slide->get_Shapes()->AddAutoShape(ShapeType::Rectangle, 200.0f, 150.0f, 200.0f, 200.0f);
shape->get_ThreeDFormat()->get_Camera()->SetRotation(20.0f, 30.0f, 40.0f);
...
Um einem PowerPoint‑3D‑Modell Tiefe und Extrusion hinzuzufügen, verwenden Sie die Methode [IThreeDFormat.set_ExtrusionHeight()](https://reference.aspose.com/slides/cpp/class/aspose.slides.three_d_format#adf0bad4894b1c36d9e4b044ef4978295). Um die Extrusionsfarbe zu ändern, verwenden Sie die Methode [IThreeDFormat.get_ExtrusionColor()](https://reference.aspose.com/slides/cpp/class/aspose.slides.three_d_format#aa7db8859d23a9b4eb2f35f3a42025e9e):
``` cpp
auto shape = slide->get_Shapes()->AddAutoShape(ShapeType::Rectangle, 200.0f, 150.0f, 200.0f, 200.0f);
shape->get_ThreeDFormat()->get_Camera()->SetRotation(20.0f, 30.0f, 40.0f);
...
// .. Einrichtung 3D: Camera, LightRig, Extrusion
...
## **3D‑Text (WordArt)**
Um Drehung, Extrusion, Licht und Verlauf auf Text anzuwenden und daraus einen 3D‑Text (WordArt) zu machen, müssen Sie die Methode [IAutoShape.get_TextFrame().get_TextFrameFormat().get_ThreeDFormat()](https://reference.aspose.com/slides/cpp/class/aspose.slides.i_text_frame_format#a5e681109403c2e57aa76a500fe508b30) verwenden:
``` cpp
using namespace Aspose::Slides;
using namespace Aspose::Slides::Export;
...
// Einrichtung "Arch Up" WordArt Transformations-Effekt
...
Ja. Slides stellt APIs bereit, um [effektive Werte](/slides/de/cpp/shape-effective-properties/) zu lesen (einschließlich für 3D — Beleuchtung, Bevels usw.), sodass Sie die endgültig angewendeten Einstellungen sehen können.
