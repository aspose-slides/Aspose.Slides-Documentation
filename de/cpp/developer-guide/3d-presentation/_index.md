...
- C++
- Aspose.Slides
description: "Erstellen Sie mühelos interaktive 3D-Präsentationen in C++ mit Aspose.Slides. Exportieren Sie schnell in PowerPoint- und OpenDocument-Formate für vielseitige Verwendung."
---

## **Überblick**
Seit Aspose.Slides 20.9 ist es möglich, PowerPoint‑3D‑Modelle zu erstellen und zu ändern. Dies kann erreicht werden, indem 2D‑Formen eine Reihe von 3D‑Effekten hinzugefügt werden. Durch Erstellen einer Kameraperspektive auf der Form können Sie sie um die Achse drehen. Durch Erzeugen einer Extrusion oder Tiefe auf der Form wird die 2D‑Form in ein 3D‑Modell verwandelt. Das Einstellen des Lichteffekts auf der 3D‑Form oder das Ändern der Materialien lässt sie lebendiger erscheinen. Das Ändern der Farben von 3D‑Modellen zu einem 3D‑Verlauf, das Modifizieren der Kontur von Formen, das Hinzufügen einer Abschrägung geben dem 3D‑Modell mehr Volumen. Alle 3D‑Effekte können sowohl auf PowerPoint‑3D‑Modelle als auch auf Texte angewendet werden.

Betrachten wir das erste Beispiel für das Erstellen von 3D‑Modellen, das alle oben genannten Funktionen enthält:
``` cpp
...
## **3D‑Drehung**
In PowerPoint steht die Formdrehung zur Verfügung über:

![todo:image_alt_text](img_02_01.png)

Um PowerPoint‑3D‑Modelle zu drehen, muss eine Kameraperspektive auf der Form erstellt werden. Dies erfolgt mit der Methode [IThreeDFormat.get_Camera()](https://reference.aspose.com/slides/cpp/class/aspose.slides.three_d_format#ad2f989bd1fd64fd4136e1f17660035d4). Die Rotationsmethode wird von der Kameraklasse aufgerufen, als ob Sie die Kamera drehen würden. Tatsächlich drehen Sie die Form auf der 3D‑Ebene, wenn Sie die Kamera relativ zur Form drehen.
``` cpp
auto shape = slide->get_Shapes()->AddAutoShape(ShapeType::Rectangle, 200.0f, 150.0f, 200.0f, 200.0f);
shape->get_ThreeDFormat()->get_Camera()->SetRotation(20.0f, 30.0f, 40.0f);
...
Um die Farbe der Extrusion zu ändern, verwenden Sie die Methode [IThreeDFormat.get_ExtrusionColor()](https://reference.aspose.com/slides/cpp/class/aspose.slides.three_d_format#aa7db8859d23a9b4eb2f35f3a42025e9e):
``` cpp
auto shape = slide->get_Shapes()->AddAutoShape(ShapeType::Rectangle, 200.0f, 150.0f, 200.0f, 200.0f);
shape->get_ThreeDFormat()->get_Camera()->SetRotation(20.0f, 30.0f, 40.0f);
...
## **3D‑Verlauf**
Das Zeichnen eines 3D‑Verlaufs auf einem PowerPoint‑3D‑Modell kann über die Methode [Shape.get_FillFormat().get_GradientFormat()](https://reference.aspose.com/slides/cpp/class/aspose.slides.fill_format#a1f075336cb7a0e05cd5d7a706b6f4f58) durchgeführt werden:
``` cpp
using namespace Aspose::Slides;
...
Um einen Bild‑Verlauf zu erstellen, verwenden Sie die Methode [Shape.get_FillFormat().get_PictureFillFormat()](https://reference.aspose.com/slides/cpp/class/aspose.slides.fill_format#ac01c9a38197ddcd80c180aceeaf155cb):
``` cpp
...
## **3D‑Text (WordArt)**
Um Drehung, Extrusion, Licht und Verlauf auf Text anzuwenden und ihn zu einem 3D‑Text (WordArt) zu machen, müssen Sie die Methode [IAutoShape.get_TextFrame().get_TextFrameFormat().get_ThreeDFormat()](https://reference.aspose.com/slides/cpp/class/aspose.slides.i_text_frame_format#a5e681109403c2e57aa76a500fe508b30) aufrufen:
``` cpp
using namespace Aspose::Slides;
using namespace Aspose::Slides::Export;
...
Ja. Slides stellt APIs zum [Lesen effektiver Werte](/slides/de/cpp/shape-effective-properties/) bereit (einschließlich für 3D‑Beleuchtung, Abschrägungen usw.), sodass Sie die letztendlich angewendeten Einstellungen sehen können.
