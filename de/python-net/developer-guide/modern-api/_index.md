...
type: docs
weight: 280
url: /de/python-net/developer-guide/modern-api/
keywords:
- moderne API
- Zeichnen
- Folien‑Miniaturbild
...
- Python
- Aspose.Slides
description: "Modernisieren Sie die Folien‑Bildverarbeitung, indem Sie veraltete Imaging‑APIs durch die Python Moderne API für nahtlose PowerPoint‑ und OpenDocument‑Automatisierung ersetzen."
---
...
Um `aspose.pydrawing` aus der öffentlichen API zu entfernen, haben wir die **Moderne API** eingeführt. Methoden, die `aspose.pydrawing.Image` und `aspose.pydrawing.Bitmap` verwenden, sind veraltet und werden durch ihre Gegenstücke der Modernen API ersetzt. Methoden, die `aspose.pydrawing.Graphics` nutzen, sind veraltet und die Unterstützung dafür wird aus der öffentlichen API entfernt.

Die Entfernung der veralteten API, die von `aspose.pydrawing` abhängt, ist für die Veröffentlichung **24.8** vorgesehen.
...
Ein typisches Nutzungsszenario für die neue API sieht folgendermaßen aus:
...
Für einen einfacheren Übergang spiegelt das neue `IImage`‑Interface die separaten APIs der Klassen `Image` und `Bitmap` wider. In den meisten Fällen müssen Sie lediglich Aufrufe von Methoden, die `aspose.pydrawing` verwenden, durch ihre Gegenstücke der Modernen API ersetzen.
...
**Warum wurde `aspose.pydrawing.Graphics` eingestellt?**

Die Unterstützung für Graphics wird aus der öffentlichen API entfernt, um die Arbeit mit Rendering und Bildern zu vereinheitlichen, Abhängigkeiten von plattformspezifischen Bibliotheken zu beseitigen und auf einen plattformübergreifenden Ansatz mit [IImage](https://reference.aspose.com/slides/python-net/aspose.slides/iimage/) zu setzen. Alle Rendering‑Methoden zu Graphics werden entfernt.
...
**Beeinflusst die Moderne API die Leistung bei der Erstellung von Miniaturbildern?**

Der Wechsel von `get_thumbnail` zu `get_image` verschlechtert die Szenarien nicht: Die neuen Methoden bieten dieselben Möglichkeiten zur Bildgenerierung mit Optionen und Größen, während sie weiterhin Rendering‑Optionen unterstützen. Der konkrete Gewinn oder Verlust hängt vom jeweiligen Szenario ab, funktional sind die Ersatzmethoden jedoch äquivalent.