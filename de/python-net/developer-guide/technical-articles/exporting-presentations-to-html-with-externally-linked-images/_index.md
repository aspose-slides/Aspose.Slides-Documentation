---
title: Export Präsentationen nach HTML mit extern verknüpften Bildern in Python
linktitle: Export Präsentationen nach HTML mit extern verknüpften Bildern
type: docs
weight: 100
...
- Python
- Aspose.Slides
description: "Exportieren von PowerPoint- und OpenDocument-Präsentationen nach HTML in Python mit Aspose.Slides, wobei Bilder als extern verknüpfte Dateien gespeichert werden."
---
## **Übersicht**
...
- generierte Bilder nach dem Export inspizieren, ersetzen, komprimieren oder nachbearbeiten;
- die Ausgabestruktur näher an das bringen, was eine Webanwendung erwartet.

Für den allgemeinen HTML-Konvertierungs‑Workflow siehe [PowerPoint-Präsentationen in HTML konvertieren](/slides/de/python-net/convert-powerpoint-to-html/). Dieser Artikel konzentriert sich auf den Bildverlinkungs‑Teil des Exports.

## **Wie der Export verknüpfter Bilder funktioniert**

Unter .NET und Java stellt [ILinkEmbedController](https://reference.aspose.com/slides/de/python-net/aspose.slides.export/ilinkembedcontroller/) die Callback‑Schnittstelle dar, die vom Exporter verwendet wird, um zu entscheiden, ob eine Ressource eingebettet oder verlinkt werden soll. In Python über .NET können Python‑Klassen diese .NET‑Callback‑Schnittstelle derzeit nicht direkt implementieren, daher lautet der praktische Ablauf:
1. Exportieren Sie die Präsentation nach HTML mit [HtmlOptions](https://reference.aspose.com/slides/de/python-net/aspose.slides.export/htmloptions/).
2. Verwenden Sie [SlideImageFormat](https://reference.aspose.com/slides/de/python-net/aspose.slides.export/slideimageformat/) zusammen mit [SVGOptions](https://reference.aspose.com/slides/de/python-net/aspose.slides.export/svgoptions/), sodass die Folien im HTML als SVG dargestellt werden.
3. Verschieben Sie die Base64‑Bilddaten aus den HTML‑`data:`‑URLs in separate Dateien.
4. Ersetzen Sie die ursprünglichen `data:`‑URLs durch relative Links wie `assets/resource-1.jpg`.

Der Dateisystempfad und die Browser‑URL sind getrennte Aspekte. Zum Beispiel schreibt das untenstehende Beispiel Bilddateien nach `html-output/assets` auf die Festplatte, während das HTML relative URLs wie `assets/resource-1.jpg` enthält. Ein Browser löst diese URLs relativ zu der HTML‑Datei auf, die den Link enthält.

## **HTML mit verknüpften Bildern exportieren**

Das nachfolgende Python‑Beispiel erstellt ein Ausgabeverzeichnis, speichert die HTML‑Datei dort, legt extrahierte Bilder in einem Unterverzeichnis `assets` ab und ersetzt Base64‑Bild‑URLs durch relative Links. Das Beispiel extrahiert gängige Base64‑Bildformate, wenn Aspose.Slides eine sichere Dateierweiterung bereitstellt. Nicht erkannte Data‑URLs bleiben eingebettet.
...
Nach dem Export kann der Ausgabordner diese Struktur haben:
...
Die genauen Dateien hängen vom Inhalt der Präsentation und den Exportoptionen ab. Rasterbilder werden beispielsweise häufig als JPEG oder PNG exportiert. Aspose.Slides kann einen anderen Bild‑Codec wählen als im Ausgangsdokument verwendet, wenn dadurch eine kleinere oder passendere Datei entsteht. Bilder mit Transparenz werden als PNG exportiert.
...
In Server‑Anwendungen sollten Sie für jeden Konvertierungsauftrag ein eindeutiges Ausgabeverzeichnis oder ein Objekt‑Speicher‑Präfix verwenden, um das Überschreiben von Dateien aus einem anderen Export zu vermeiden.

## **Wann stattdessen einbetten**

Eingebettetes Base64‑HTML ist nach wie vor nützlich, wenn die Ausgabe eine einzige Datei sein muss, etwa als E‑Mail‑Anhang, Offline‑Vorschau oder Dokument, das ohne unterstützendes Asset‑Verzeichnis verschoben wird. Verknüpfte Bilder eignen sich besser, wenn das HTML von einer Web‑Anwendung bereitgestellt, in einem CMS gespeichert, durch eine Build‑Pipeline optimiert oder von Browsern unabhängig vom HTML gecached wird.
...
**Warum unterscheidet sich die exportierte Bild‑Erweiterung von der Ausgangspräsentation?**
...
Nein. Verwenden Sie für jeden Konvertierungsauftrag ein eindeutiges Ausgabeverzeichnis oder Speicher‑Präfix. So werden Dateinamen‑Kollisionen vermieden und ein Export überschreibt nicht die Ressourcen eines anderen Exports.