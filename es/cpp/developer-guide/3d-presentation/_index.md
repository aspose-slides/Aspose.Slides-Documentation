...
## **Visión general**
Desde Aspose.Slides 20.9 es posible crear y modificar modelos 3D de PowerPoint. Esto se puede lograr aplicando a las formas 2D un conjunto de efectos 3D. Al crear una vista de cámara en la forma, puede rotarla alrededor del eje. Crear una extrusión o profundidad en la forma, lo que transformará la forma de 2D a un modelo 3D. Establecer el efecto de luz en la forma 3D o cambiar los materiales puede hacer que se vea más viva. Cambiar los colores de los modelos 3D a un degradado 3D, modificar el contorno de las formas, añadir un bisel hacen que el modelo 3D tenga más volumen. Todos los efectos 3D pueden aplicarse tanto a los modelos 3D de PowerPoint como al texto.
...
Para rotar los modelos 3D de PowerPoint, es necesario crear una vista de cámara en la forma. Esto se hace con el método [IThreeDFormat.get_Camera()](https://reference.aspose.com/slides/cpp/class/aspose.slides.three_d_format#ad2f989bd1fd64fd4136e1f17660035d4). El método de rotación se llama desde la clase de cámara como si estuviera rotando la cámara. De hecho, cuando rota la cámara respecto a la forma, rota la forma en el plano 3D.
``` cpp
auto shape = slide->get_Shapes()->AddAutoShape(ShapeType::Rectangle, 200.0f, 150.0f, 200.0f, 200.0f);
shape->get_ThreeDFormat()->get_Camera()->SetRotation(20.0f, 30.0f, 40.0f);
...
## **Texto 3D (WordArt)**
Para aplicar rotación, extrusión, luz o degradado al texto y convertirlo en un texto 3D (WordArt), debe acceder al método [IAutoShape.get_TextFrame().get_TextFrameFormat().get_ThreeDFormat()](https://reference.aspose.com/slides/cpp/class/aspose.slides.i_text_frame_format#a5e681109403c2e57aa76a500fe508b30):
``` cpp
using namespace Aspose::Slides;
using namespace Aspose::Slides::Export;
...
Sí. Cuando se [generan los fotogramas para el video](/slides/es/cpp/convert-powerpoint-to-video/), los efectos 3D se renderizan de la misma forma que para las [imágenes exportadas](/slides/es/cpp/convert-powerpoint-to-png/).