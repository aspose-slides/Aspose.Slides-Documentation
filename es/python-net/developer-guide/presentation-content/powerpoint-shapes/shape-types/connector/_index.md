...
- Python
- Aspose.Slides
description: "Empodere aplicaciones Python para dibujar, conectar y enrutar automáticamente líneas en diapositivas PowerPoint y OpenDocument—obtenga control total sobre conectores rectos, en codo y curvos."
---

## **Introducción**

Un conector de PowerPoint es una línea especializada que enlaza dos formas y permanece adherida cuando las formas se mueven o se reposicionan en una diapositiva. Los conectores se adhieren a **puntos de conexión** (puntos verdes) en las formas. Los puntos de conexión aparecen cuando el puntero se acerca a ellos. Los **mangos de ajuste** (puntos amarillos), disponibles en ciertos conectores, le permiten modificar la posición y forma del conector.

## **Tipos de conectores**

En PowerPoint, puede usar tres tipos de conectores: recto, en codo (angular) y curvo.
...
Esta sección muestra cómo enlazar formas con conectores en Aspose.Slides. Añadirá un conector a una diapositiva y conectará su inicio y fin a formas objetivo. Utilizar sitios de conexión garantiza que el conector permanezca “pegado” a las formas incluso cuando se muevan o redimensionen.
...
1. Añadir dos objetos [AutoShape](https://reference.aspose.com/slides/python-net/aspose.slides/autoshape/) a la diapositiva mediante el método `add_auto_shape` expuesto por el objeto [ShapeCollection](https://reference.aspose.com/slides/python-net/aspose.slides/shapecollection/).
1. Añadir un conector usando el método `add_connector` expuesto por el objeto [ShapeCollection](https://reference.aspose.com/slides/python-net/aspose.slides/shapecollection/) y especificar el tipo de conector.
...
El método `connector.reroute` vuelve a enrutar un conector, forzándolo a tomar la ruta más corta posible entre las formas. Para ello, el método puede cambiar los valores de `start_shape_connection_site_index` y `end_shape_connection_site_index`.

{{% /alert %}}

## **Especificar puntos de conexión**

Esta sección explica cómo adherir un conector a un punto de conexión específico en una forma dentro de Aspose.Slides. Al dirigirse a sitios de conexión precisos, puede controlar el enrutamiento y la disposición del conector, produciendo diagramas limpios y predecibles en sus presentaciones.

1. Crear una instancia de la clase [Presentation](https://reference.aspose.com/slides/python-net/aspose.slides/presentation/).
1. Obtener una referencia a la diapositiva por su índice.
1. Añadir dos objetos [AutoShape](https://reference.aspose.com/slides/python-net/aspose.slides/autoshape/) a la diapositiva mediante el método `add_auto_shape` expuesto por el objeto [ShapeCollection](https://reference.aspose.com/slides/python-net/aspose.slides/shapecollection/).
1. Añadir un conector usando el método `add_connector` en el objeto [ShapeCollection](https://reference.aspose.com/slides/python-net/aspose.slides/shapecollection/) y especificar el tipo de conector.
1. Conectar las formas con el conector.
1. Establecer sus puntos de conexión preferidos en las formas.
1. Guardar la presentación.
...
Puede modificar los conectores mediante sus puntos de ajuste. Solo los conectores que exponen puntos de ajuste pueden editarse de esta forma. Para detalles sobre qué conectores admiten ajustes, consulte la tabla bajo [Tipos de conectores](/slides/es/python-net/connector/#connector-types).
...
Para evitar la tercera forma, ajuste el conector moviendo su segmento vertical a la izquierda:
...
- El punto ajustable de un conector está gobernado por una fórmula que determina su posición. Cambiar este punto puede modificar la forma general del conector.
- Los puntos de ajuste se almacenan en un arreglo estrictamente ordenado, numerado desde el inicio al fin del conector.
- Los valores de los puntos de ajuste representan porcentajes del ancho/alto de la forma del conector.
  - La forma está limitada por los puntos de inicio y fin del conector y se escala por 1000.
...
Considere un caso donde dos objetos de marco de texto están enlazados con un conector:
...
Cambie los valores de los puntos de ajuste del conector incrementando el porcentaje de ancho en un 20 % y el de alto en un 200 %:
...
En **Caso 1**, demostramos un ajuste simple del conector usando principios básicos. En escenarios típicos, debe considerar la rotación del conector y sus ajustes de visualización (controlados por `connector.rotation`, `connector.frame.flip_h` y `connector.frame.flip_v`). A continuación, el proceso.

Primero, añada un nuevo objeto de marco de texto (**To 1**) a la diapositiva (para conexión) y cree un nuevo conector verde que lo enlace a los objetos existentes.
...
## **Encontrar ángulos de líneas de conectores**

Utilice el ejemplo a continuación para determinar el ángulo de las líneas de conectores en una diapositiva con Aspose.Slides. Aprenderá a leer los extremos de un conector y a calcular su orientación para alinear con precisión flechas, etiquetas y otras formas.

1. Crear una instancia de la clase [Presentation](https://reference.aspose.com/slides/python-net/aspose.slides/presentation/).
1. Obtener una referencia a la diapositiva por índice.
1. Acceder a la forma de línea de conector.
...
Verifique que la forma exponga [sitios de conexión](https://reference.aspose.com/slides/python-net/aspose.slides/shape/connection_site_count/). Si no hay ninguno o el recuento es cero, la fijación no está disponible; en ese caso, use extremos libres y colóquelos manualmente. Es aconsejable comprobar el recuento antes de adjuntar.

**¿Qué ocurre con un conector si elimino una de las formas conectadas?**

Sus extremos se despegarán; el conector permanece en la diapositiva como una línea ordinaria con inicio/final libres. Puede eliminarlo o reasignar las conexiones y, si es necesario, [volver a enrutar](https://reference.aspose.com/slides/python-net/aspose.slides/connector/reroute/).
