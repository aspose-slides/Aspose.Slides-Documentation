...
- Python
- Aspose.Slides
description: "Empaquete Aspose.Slides for Python via .NET con PyInstaller. Siga esta guía para crear, configurar y solucionar problemas de su aplicación en un ejecutable independiente."
---

## **Compatibilidad con PyInstaller y cx_Freeze**

Las extensiones de Aspose.Slides for Python via .NET son extensiones estándar de C para Python, por lo que pueden congelarse como dependencias del programa con herramientas como PyInstaller y cx_Freeze (o similares). Esto le permite crear archivos ejecutables a partir de sus scripts de Python. Estas herramientas se denominan “congeladores” porque agrupan su código y sus dependencias en un único archivo distribuible que se ejecuta en otras máquinas sin requerir una instalación de Python ni bibliotecas adicionales. Este enfoque simplifica la distribución de sus aplicaciones Python.

Congelar una extensión de Aspose.Slides for Python via .NET como dependencia se ilustra a continuación con un programa sencillo que usa Aspose.Slides.

### **PyInstaller**

En general, no se requiere nada especial al empaquetar un programa que depende de una extensión de Aspose.Slides for Python via .NET. Cuando un programa importa la extensión de una manera visible para PyInstaller, la extensión se incluirá en el programa. Dado que Aspose.Slides for Python via .NET incluye hooks para PyInstaller, sus dependencias se detectan y copian automáticamente en el paquete.
...
Para congelar un programa con cx_Freeze, configúrelo para que incluya el paquete raíz de la extensión de Aspose.Slides for Python via .NET que está utilizando. Esto garantiza que la extensión y todos los módulos dependientes se copien en la compilación junto con su aplicación.
...
**¿Cómo debería adjuntar correctamente la licencia a una aplicación congelada?**
...
**¿Qué debo hacer si las fuentes se renderizan de forma diferente después de la compilación comparado con el desarrollo?**
