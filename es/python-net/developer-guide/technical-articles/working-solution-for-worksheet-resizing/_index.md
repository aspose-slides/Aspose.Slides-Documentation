...
- Python
- Aspose.Slides
description: "Soluciona el redimensionamiento OLE de la hoja de cálculo de Excel en presentaciones: dos formas de mantener los marcos de objetos consistentes—escalar el marco o la hoja—en los formatos PPT y PPTX."
---

{{% alert color="primary" %}} 
Se ha observado que las hojas de cálculo de Excel incrustadas como objetos OLE en una presentación de PowerPoint mediante componentes Aspose se redimensionan a una escala no identificada después de la primera activación. Este comportamiento crea una diferencia visual notable en la presentación entre los estados antes y después de la activación del objeto OLE. Hemos investigado este problema en detalle y hemos proporcionado una solución, que se describe en este artículo.
{{% /alert %}} 

## **Antecedentes**

En el artículo [Administrar OLE](/slides/es/python-net/manage-ole/), explicamos cómo añadir un marco OLE a una presentación de PowerPoint usando Aspose.Slides for Python via .NET. Para abordar el [problema de vista previa del objeto](/slides/es/python-net/object-preview-issue-when-adding-oleobjectframe/), asignamos una imagen del área de la hoja de cálculo seleccionada al marco del objeto OLE. En la presentación resultante, al hacer doble clic en el marco OLE que muestra la imagen de la hoja, se activa el libro de Excel. Los usuarios pueden realizar los cambios deseados en el libro real y luego volver a la diapositiva haciendo clic fuera del libro de Excel activado. El tamaño del marco OLE cambiará cuando el usuario regrese a la diapositiva. El factor de redimensionamiento variará según el tamaño del marco OLE y del libro de Excel incrustado. 
...
with cells.Workbook("sample.xlsx") as workbook:
    ...
    # Establecer el tamaño visible cuando el archivo de libro de trabajo se usa como objeto OLE en PowerPoint.
...
En este enfoque, aprenderemos a escalar la altura de las filas participantes y el ancho de las columnas participantes para que coincidan con un tamaño de marco OLE personalizado.

Supongamos que tenemos una hoja de Excel plantilla y queremos añadirla a una presentación como un marco OLE. En este escenario, estableceremos el tamaño del marco OLE y escalaremos el tamaño de las filas y columnas que participan en el área del marco OLE. Luego, guardaremos el libro en un flujo para aplicar los cambios y lo convertiremos a una matriz de bytes para añadirlo al marco OLE. Para evitar el mensaje rojo “EMBEDDED OLE OBJECT” del marco OLE en PowerPoint, también capturaremos una imagen de las porciones deseadas de las filas y columnas del libro y la estableceremos como la imagen del marco OLE.
```py
