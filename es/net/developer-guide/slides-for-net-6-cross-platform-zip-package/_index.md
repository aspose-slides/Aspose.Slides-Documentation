...
Este artículo explica cómo usar Aspose.Slides para .NET 6 Cross-Platform a partir de un paquete ZIP. Describe cómo descargar el paquete, descomprimir los archivos de la carpeta `net6.0/crossplatform`, agregar una referencia a `Aspose.Slides.dll` y configurar el archivo del proyecto para que las bibliotecas dependientes necesarias se copien al directorio de salida de la aplicación.

El artículo también describe el contenido del paquete multiplataforma, incluyendo el ensamblado principal de Aspose.Slides .NET y las bibliotecas del subsistema gráfico específicas de cada plataforma para Windows, Linux y macOS.
...
1. Descargue el paquete ZIP de la última versión de Aspose.Slides desde la [Página de lanzamiento](https://releases.aspose.com/slides/es/net/).
...
4. Coloque los archivos restantes (de los que depende Aspose.Slides) en el directorio de salida añadiendo instrucciones al archivo de proyecto csproj de la siguiente manera:
...
5. Preste atención a `TargetPath`.
   ...
   Por defecto, `<CopyToOutputDirectory>` copia los archivos preservando su ruta relativa, pero necesitamos que las bibliotecas dependientes se copien en la misma carpeta donde se genera la salida (ubicación de Aspose.Slides.dll).

## **Notas**

### **Subsistema gráfico propietario**

Aspose.Slides cross-platform es una colección de bibliotecas:
...
Aspose.Slides.dll utiliza la biblioteca que requiera el sistema en el que se está ejecutando. Normalmente, las bibliotecas se encuentran en la misma ubicación que Aspose.Slides.dll en cualquier sistema de archivos.
...
* Cada carpeta contiene ensamblados para la versión .NET correspondiente. Hay dos versiones para net6.0: default y crossplatform. Esta última contiene el Aspose.Slides.dll multiplataforma y todas sus dependencias. El contenido descomprimido de esta carpeta puede usarse como una adición de dependencia en un proyecto para desarrollo multiplataforma y otras instancias de uso de Aspose.Slides.
