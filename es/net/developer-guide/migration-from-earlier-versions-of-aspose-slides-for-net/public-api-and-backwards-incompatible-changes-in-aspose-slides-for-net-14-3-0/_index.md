---
title: API pública y cambios incompatibles retroactivos en Aspose.Slides for .NET 14.3.0
...
Los métodos GetThumbnail() y GetThumbnail(ShapeThumbnailBounds bounds, float scaleX, float scaleY) se usan para crear una miniatura de forma independiente. La enumeración ShapeThumbnailBounds define los posibles tipos de límites de la miniatura de forma.
### **Propiedad UniqueId añadida a Aspose.Slides.IShape**
La propiedad Aspose.Slides.IShape.UniqueId obtiene un identificador de forma único en el alcance de la presentación. Estos identificadores únicos se almacenan en etiquetas personalizadas de la forma.
### **Firma del método SetGroupingItem modificada en IChartCategoryLevelsManager**
Firma del método IChartCategoryLevelsManager

```csharp
 void SetGroupingItem(int level, IChartDataCell value);
``` 

ahora está obsoleta y se reemplaza por la firma
...
Pase un valor como "Group 1" a SetGroupingItem, pero no un valor del tipo IChartDataCell. Construir un IChartDataCell con una hoja de cálculo, fila y columna definidas para niveles de categoría debe cumplir ciertos requisitos y se ha encapsulado en el método SetGroupingItem(int, object).
...
### **Soporte añadido para generar miniaturas de forma individuales**
