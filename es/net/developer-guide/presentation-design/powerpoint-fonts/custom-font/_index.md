...
- C#
- Aspose.Slides
description: "Personaliza fuentes en diapositivas de PowerPoint con Aspose.Slides para .NET y mantén tus presentaciones nítidas y consistentes en cualquier dispositivo."
---
## **Descripción general**
...
Aspose Slides permite cargar estas fuentes mediante el método [FontsLoader.LoadExternalFonts](https://reference.aspose.com/slides/es/net/aspose.slides/fontsloader/loadexternalfonts/):
...
Aspose.Slides permite cargar fuentes utilizadas en una presentación sin instalarlas en el sistema. Esto afecta la salida de exportación—como PDF, imágenes y otros formatos compatibles—para que los documentos resultantes se vean coherentes en distintos entornos. Las fuentes se cargan desde directorios personalizados.
...
FontsLoader.LoadExternalFonts añade carpetas adicionales a las rutas de búsqueda de fuentes, pero no modifica el orden de inicialización de fuentes.  
...
## **Especificar fuentes personalizadas utilizadas en una presentación**
...
No. Registrar una fuente para la renderización no es lo mismo que incrustarla en un PPTX. Si necesita que la fuente esté dentro del archivo de presentación, debe utilizar las [funciones de incrustación](/slides/es/net/embedded-font/) de forma explícita.

**¿Puedo controlar el comportamiento de reserva cuando una fuente personalizada carece de ciertos glifos?**

Sí. Configure [font substitution](/slides/es/net/font-substitution/), [replacement rules](/slides/es/net/font-replacement/) y [fallback sets](/slides/es/net/fallback-font/) para definir exactamente qué fuente se utiliza cuando falta el glifo solicitado.
...
> **Nota para Linux/Docker**: Al llamar a `FontsLoader.LoadExternalFonts`, asegúrese de que cada entrada en el array `directories` contenga una ruta no vacía a un directorio existente. Si una variable de entorno utilizada para construir la ruta de la fuente está indefinida o vacía, Aspose.Slides podría intentar resolver el valor vacío como una ruta completa, lo que provocaría `System.ArgumentException`.

**¿Qué pasa con la licencia—puedo incrustar cualquier fuente personalizada sin restricciones?**

Usted es responsable de cumplir con la licencia de la fuente. Los términos varían; algunas licencias prohíben la incrustación o el uso comercial. Siempre revise el EULA de la fuente antes de distribuir los resultados.