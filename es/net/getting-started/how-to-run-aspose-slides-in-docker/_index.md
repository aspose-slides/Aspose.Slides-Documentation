...
Para usar Docker, primero debe instalarlo en su sistema. Para aprender cómo instalar Docker en Windows o Mac, utilice estos enlaces:
...
La instalación y configuración de Docker en Windows Server Nano no es compatible. Desafortunadamente, Windows Server Nano no contiene el subsistema gráfico integrado. No incluye gdiplus.dll, que requiere la biblioteca System.Drawing.Common, y no puede usarse con la biblioteca Aspose.Slides.

Aunque es posible ejecutar contenedores Linux en Windows, recomendamos ejecutarlos nativamente en Linux (incluso en un Linux instalado manualmente en una VM usando VirtualBox).

## **Instalación y configuración de Docker en Linux (apt-get libgdiplus)**
- SO: Ubuntu 18.04.
- Dockerfile: Dockerfile-Ubuntu18_04_apt_get_libgdiplus

Este archivo Docker contiene instrucciones para crear una imagen de contenedor con el paquete libgdiplus instalado desde los repositorios oficiales de Ubuntu.

Este es el contenido del archivo Docker:
``` csharp
...
4. Declara la carpeta /slides-src como punto de montaje que utilizaremos para proporcionar acceso a la carpeta de fuentes slide‑net en la máquina host.
``` csharp
...
5. Establece slides‑src como directorio de trabajo dentro del contenedor.
``` csharp
...
6. Declara un comando por defecto que se ejecutará al iniciar el contenedor en caso de que no se haya especificado un comando explícito.
``` csharp
...
Según las instrucciones del archivo Docker, la imagen resultante del contenedor tendrá Ubuntu 18.04, dotnet‑sdk, libgdiplus y los paquetes libc6‑dev ya instalados. Además, esta imagen tendrá un punto de montaje predefinido y un comando predefinido al ejecutarse.
...
*-f Dockerfile-Ubuntu18_04_apt_get_libgdiplus* -- la opción indica qué archivo Docker usar.

*-t ubuntu18_04_apt_get_libgdiplus* -- especifica la etiqueta (nombre) para la imagen resultante.

*'.'* -- especifica el contexto para Docker. En nuestro caso, el contexto es la carpeta actual y está vacío, ya que elegimos proporcionar las fuentes de slides‑net como punto de montaje (esto nos permite no reconstruir la imagen Docker en cada cambio de las fuentes).
...
*-it* -- indica que el comando debe ejecutarse de forma interactiva, permitiendo ver la salida y capturar la entrada.

*-v `pwd`/../../:/slides-src* -- especifica la carpeta para el punto de montaje predefinido; dado que el directorio de trabajo actual es slides‑netuildocker, la carpeta slides‑src dentro del contenedor apuntará a la carpeta slides‑net en el host. `pwd` se usa para indicar la ruta relativa.
...
*ubuntu1804aptgetlibgdiplus:latest* -- especifica la imagen a ejecutar.

El resultado del comando anterior será la salida de netcore.linux.tests.sh (ya que se definió como comando por defecto para el contenedor):
``` csharp
...
Del resultado se observa que los archivos de registro de las pruebas Func y Regr se colocaron en el directorio **/build-out/netstandard20/test-results/main/**. Aproximadamente 200 pruebas fallaron en total, todas relacionadas con problemas de renderizado por ausencia de fuentes requeridas en el contenedor.

Para sobrescribir el comando por defecto del contenedor al ejecutarlo, podríamos usar este comando:
``` csharp
...
Así, en lugar de netcore.linux.tests.sh, se ejecutará **/bin/bash**, proporcionando una sesión de terminal activa del contenedor desde la cual se podrá ejecutar (**./build/netcore.linux.tests.sh**). Este enfoque puede ser útil en escenarios de depuración.

## **Instalación y configuración de Docker en Linux (make install libgdiplus)**
- SO: Ubuntu 18.04.
- Dockerfile: Dockerfile-Ubuntu18_04_make_libgdiplus

En la actualidad, Ubuntu solo contiene la versión 4.2 de libgdiplus, mientras que la versión 5.6 ya está disponible en el [sitio oficial](https://github.com/mono/libgdiplus/releases) del producto. Para probar la última versión de libgdiplus, necesitamos preparar una imagen con libgdiplus compilado a partir del código fuente.
...
#set powershell ejecutor predeterminado
...
#return cmd como ejecutor predeterminado

SHELL ["cmd", "/S", "/C"]
\# Para establecer la variable de entorno PATH del sistema, se debe usar ContainerAdministrator
...
La imagen resultante se construirá sobre la imagen microsoft/windowsservercore:1803 provista por Microsoft en el [docker hub](https://hub.docker.com/r/microsoft/windowsservercore/). El SDK dotnet de la versión especificada se descargará y descomprimirá; la variable de entorno PATH del sistema se actualizará para incluir la ruta al ejecutable dotnet. La última línea define el comando que ejecuta las pruebas func & regr en el contenedor usando nant.exe como acción por defecto al ejecutar el contenedor.
...
Además, falta 1 argumento en comparación con el mismo comando usado para el contenedor Linux:

*-add-host dev.slides.external.tool.server:192.168.1.48*

Porque el contenedor que se ejecuta en Windows no requiere external.tool.server.
