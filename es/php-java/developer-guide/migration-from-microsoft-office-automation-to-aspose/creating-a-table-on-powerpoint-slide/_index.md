---
title: Creando una Tabla en una Diapositiva de PowerPoint
type: docs
weight: 50
url: /es/php-java/creating-a-table-on-powerpoint-slide/
---

{{% alert color="primary" %}} 

Las tablas se utilizan ampliamente para mostrar datos en diapositivas de presentación. Este artículo muestra cómo crear una tabla de 15 x 15 con un tamaño de fuente de 10 programáticamente utilizando primero [VSTO 2008](/slides/es/php-java/creating-a-table-on-powerpoint-slide/) y luego [Aspose.Slides para PHP a través de Java](/slides/es/php-java/creating-a-table-on-powerpoint-slide/).

{{% /alert %}} 
## **Creando Tablas**
### **Ejemplo de VSTO 2008**
Los siguientes pasos añaden una tabla a una diapositiva de Microsoft PowerPoint utilizando VSTO:

1. Crear una presentación.
1. Se añade una diapositiva vacía a la presentación.
1. Añadir una tabla de 15 x 15 a la diapositiva.
1. Añadir texto a cada celda de la tabla con un tamaño de fuente de 10.
1. Guardar la presentación en el disco.



{{< gist "aspose-com-gists" "a56eda38c01ad33dc653116c7bae4293" "Examples-CSharp-VSTO-CreateTableUsingVSTO-CreateTableUsingVSTO.cs" >}}
### **Ejemplo de Aspose.Slides para PHP a través de Java**
Los siguientes pasos añaden una tabla a una diapositiva de Microsoft PowerPoint utilizando Aspose.Slides:

1. Crear una presentación.
1. Añadir una tabla de 15 x 15 a la primera diapositiva.
1. Añadir texto a cada celda de la tabla con un tamaño de fuente de 10.
1. Escribir la presentación en el disco.



{{< gist "aspose-com-gists" "1f55f0222bc39a382d831900e8de7400" "Examples-src-main-java-com-aspose-slides-examples-Slides-Table-CreateTable-CreateTable.java" >}}