...
Denna separering är användbar när samma bild visas mer än en gång. Lägg till bilden i presentationen en gång, behåll den returnerade [PPImage](https://reference.aspose.com/slides/sv/php-java/aspose.slides/ppimage/), och använd den bildresursen när du skapar bildramar.

Bildramar kan innehålla rasterbilder såsom PNG eller JPEG samt vektor‑SVG‑bilder. De kan också referera till länkade bilder istället för att lagra bild‑bytarna i presentationen. Valet påverkar portabilitet, filstorlek, extrahering och exportbeteende, så det är bra att bestämma hur bilden ska lagras innan du tillämpar formatering eller optimering.
...
Bildramen styr den visade geometrin; att ändra ramen storlek ändrar inte de ursprungliga pixel‑dimensionerna som lagras i den inbäddade bildresursen. Detta blir viktigt när man beskär eller komprimerar en bild senare.

## **Använd relativ skala**

[PictureFrame](https://reference.aspose.com/slides/sv/php-java/aspose.slides/pictureframe/) exponerar relativ bredd‑ och höjdskala för ramen via [setRelativeScaleWidth](https://reference.aspose.com/slides/sv/php-java/aspose.slides/pictureframe/setrelativescalewidth/) och [setRelativeScaleHeight](https://reference.aspose.com/slides/sv/php-java/aspose.slides/pictureframe/setrelativescaleheight/). Värdet `1.0` motsvarar 100 % av den ursprungliga bildstorleken. Relativ skala är användbar när ett arbetsflöde måste bevara relationen till källbildens storlek istället för att manuellt beräkna slutdimensioner.
...
Relativ skala ändrar ramens skalningsinställningar; den omprovklistrar eller komprimerar inte den inbäddade bilden.
...
Länkade bilder kan minska mängden bilddata som lagras i PPTX, men de introducerar ett externt beroende. Den länkade filen måste förbli tillgänglig för applikationen som öppnar eller renderar presentationen. Om sökvägen ändras, filen flyttas eller resursen är otillgänglig, visas den länkade bilden kanske inte som förväntat. För presentationer som måste skickas via e‑post, arkiveras eller renderas i isolerade miljöer är inbäddade bilder vanligtvis mer pålitliga.

### **Lägg till en länkad bild**

Följande exempel skapar en bildram och pekar den på en lokal bildfil. Det hanterar endast bildlänkning; videolänkning är ett separat medie‑arbetsflöde och blandas medvetet inte in i detta exempel.
...
Att spara via [IImage::save](https://reference.aspose.com/slides/sv/php-java/aspose.slides/iimage/#save) konverterar den extraherade bilden till det begärda output‑formatet. Om du behöver de kodade bytarna som lagras i presentationen snarare än en konverterad rasterfil, använd bildresursens binära data istället.
...
Att behålla SVG‑innehållet som SVG bevarar vektor‑källan i presentationen. Raster‑exporter som PNG eller JPEG renderar nödvändigtvis det vektoriserade innehållet till pixlar. PDF‑ eller SVG‑bildexport är också en renderingsoperation, så de exporterade grafik‑filerna bör inte betraktas som en exakt byte‑för‑byte‑kopiering av den inbäddade SVG:n; använd den inbäddade [SvgImage::getSvgData](https://reference.aspose.com/slides/sv/php-java/aspose.slides/svgimage/getsvgdata/) när den ursprungliga vektorresursen själv krävs.

## **Beskär en bild**

Beskärning förändrar vilken del av en bild som syns i ramen. Beskärningsvärdena på [PictureFillFormat](https://reference.aspose.com/slides/sv/php-java/aspose.slides/picturefillformat/) är procent av källbildens dimensioner. Beskärning tar initialt inte bort de dolda pixlarna från den inbäddade bilden; den ändrar bara den synliga regionen.
...
Eftersom de dolda bildbytarna fortfarande finns kvar kan beskärningen ändras senare utan att förlora de ursprungliga pixlarna. Om filstorlek är viktigare än återställningsmöjlighet kan de beskurna områdena fysiskt tas bort enligt nästa avsnitt.
...
Komprimering är avsedd för rasterbilder. SVG‑ och metafil‑innehåll reduceras inte av detta rasterkomprimerings‑arbetsflöde. Kom också ihåg att lägre upplösning och borttagna beskurna regioner inte kan återställas från den optimerade presentationen. Välj en mål‑upplösning baserat på den största storlek som bilden faktiskt kommer att visas eller exporteras i, snarare än att globalt tillämpa lägsta DPI.

## **Inspektera bildeffekter**

Bildeffekter lagras på bilden som används av ramen. Bildens transform‑samling kan innehålla effekter såsom fast alfa‑modulering för transparens och luminans för ljusstyrka och kontrast. Exemplet nedan läser säkert båda typer av effekter från den första bildramen på en bild:
...
Dessa effekter ändrar hur bilden renderas i ramen; de skriver inte om de ursprungliga inbäddade bild‑bytarna.

## **Lås bildramens geometri**

[PictureFrameLock](https://reference.aspose.com/slides/sv/php-java/aspose.slides/pictureframelock/)‑inställningarna styr vilka redigeringsåtgärder som inaktiveras för en bildram. Till exempel bevarar [setAspectRatioLocked](https://reference.aspose.com/slides/sv/php-java/aspose.slides/pictureframelock/setaspectratiolocked/) formens proportioner medan den ändras i storlek.
...
- **Inbäddade bilder** gör presentationen självständig och är de mest pålitliga för delning och server‑side rendering, men stora rasterbilder ökar PPTX‑storlek och minnesanvändning.
...
- **Beskärning** är initialt icke‑destruktiv. De dolda pixlarna förblir inbäddade tills beskurna områden tas bort explicit eller under komprimering.
- **Komprimering** kan minska filstorleken avsevärt för överdimensionerade rasterbilder, men den offrar källupplösning. Den bör tillämpas efter att den avsedda storleken på bilden på sliden är känd.
- **SVG‑bilder** bör förbli som SVG när vektor‑bevarande är viktigt. Extrahera den inbäddade SVG:n direkt när du behöver själva vektorresursen. Raster‑export av sliden konverterar alltid den renderade sliden till pixlar.
...
För stora presentationer är bildoptimering vanligtvis mest effektiv när den utförs selektivt: behåll logotyper och diagram som vektor­innehåll, komprimera foton efter deras faktiska visningsstorlek, ta bort beskurna pixlar endast när senare redigering inte krävs, och undvik externa länkar om inte beroendehantering är en del av driftsdesignen.
...
Nej. Komprimering kan reducera den lagrade rasterupplösningen, och borttagning av beskurna regioner kastar bilddata. Behåll originalkällbilden utanför presentationen om hög‑upplöst redigering senare kan behövas.

**Hur ska SVG‑bilder hanteras?**

Behåll SVG‑innehållet som SVG när vektor‑fidelity är viktigt. Den inbäddade [SvgImage](https://reference.aspose.com/slides/sv/php-java/aspose.slides/svgimage/) kan extraheras direkt. Att rendera en bild till ett rasterformat såsom PNG eller JPEG rasteriserar SVG:n som en del av bildens grafik.

**Hur kan jag undvika osäkra type‑cast när jag läser befintliga bilder?**

Kontrollera formtypen innan du använder bildram‑specifika medlemmar. En `java_instanceof`‑kontroll mot [PictureFrame](https://reference.aspose.com/slides/sv/php-java/aspose.slides/pictureframe/) undviker ogiltiga cast‑ar och låter koden hantera bilder som inte innehåller bildramar.