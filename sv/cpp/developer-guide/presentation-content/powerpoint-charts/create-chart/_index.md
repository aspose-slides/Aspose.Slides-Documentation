---
title: Skapa eller uppdatera PowerPoint-diagram i C++
...
* härleda riktning och momentum för data över tid eller i förhållande till en specifik måttenhet
* identifiera avvikelser, aberrationer, avvikelser, fel, meningslös data etc.
...
1. Lägg till ett diagram med viss data och ange den diagramtyp du föredrar. 
...
// Sökvägen till dokumentkatalogen.
	...
	// Den första etiketten är inställd på att visa kategorinamn
...
Spridda diagram (även kallade spridningsplotar eller x‑y‑grafer) används ofta för att kontrollera mönster eller demonstrera korrelationer mellan två variabler. 
...
Denna C++‑kod visar hur du skapar spridda diagram med olika serie‑markörer: 
...
### **Skapa cirkeldiagram**
Cirkeldiagram är bäst för att visa förhållandet del‑till‑helhet i data, särskilt när data innehåller kategoriska etiketter med numeriska värden. Om dina data däremot innehåller många delar eller etiketter kan du överväga att använda ett stapeldiagram istället. 

1. Skapa en instans av klassen [Presentation](https://reference.aspose.com/slides/sv/cpp/class/aspose.slides.presentation). 
1. Hämta en bilds referens via dess index. 
1. Lägg till ett diagram med standarddata samt önskad typ (i detta fall `ChartType.Pie`). 
1. Åtkomst till diagrammets data IChartDataWorkbook. 
1. Rensa standardserier och -kategorier. 
1. Lägg till nya serier och kategorier. 
1. Lägg till ny diagramdata för diagramserierna. 
1. Lägg till nya punkter för diagrammen och lägg till anpassade färger för cirkeldiagrammets sektorer. 
1. Ange etiketter för serier. 
1. Ange ledarlinjer för serieetiketter. 
1. Ange rotationsvinkeln för cirkeldiagrammets bilder. 
...
Som standard förenas punkter i ett linjediagram med raka kontinuerliga linjer. Om du vill att punkterna ska förenas med streck istället kan du ange önskad strecktyp på följande sätt:
...
Trädkartsdiagram är bäst för försäljningsdata när du vill visa den relativa storleken på datakategorier och (samtidigt) snabbt dra uppmärksamhet till de poster som är stora bidragsgivare till varje kategori. 
...
using namespace System;
	...
	SharedPtr<IChartSeries> series = chart->get_ChartData()->get_Series()->idx_get(0);
	// Fyller på den första seriedatan
	...
	series = chart->get_ChartData()->get_Series()->idx_get(1);
	// Fyller på den andra seriedatan
...
Denna C++‑kod visar hur du skapar ett flerkategori‑diagram:
...
4. Åtkomst till diagrammets datablad. 
5. Modifiera diagramserie‑data genom att ändra serievärden. 
...
// Hämtar den första slideMarker och lägger till ett diagram med standarddata
...
Denna C++‑kod visar hur du automatiskt sätter en diagramserie‑markör:
...
Aspose.Slides stödjer ett brett spektrum av diagramtyper, inklusive stapel, linje, cirkel, område, spridning, histogram, radar och många fler. Denna flexibilitet låter dig välja den mest lämpliga diagramtypen för dina datavisualiseringsbehov.

**Hur lägger jag till ett nytt diagram på en bild?**

För att lägga till ett diagram skapar du först en instans av [Presentation]‑klassen, hämtar önskad bild med dess index och anropar sedan metoden för att lägga till ett diagram, där du specificerar diagramtyp och initiala data. Denna process integrerar diagrammet direkt i din presentation.

**Hur kan jag uppdatera data som visas i ett diagram?**

Du kan uppdatera ett diagrams data genom att komma åt dess dataarbetsbok ([IChartDataWorkbook]), rensa eventuella standardserier och -kategorier och sedan lägga till dina egna data. Detta gör det möjligt att programatiskt uppdatera diagrammet så att det återspeglar de senaste data.

**Är det möjligt att anpassa diagrammets utseende?**

Ja, Aspose.Slides erbjuder omfattande anpassningsmöjligheter. Du kan ändra färger, typsnitt, etiketter, förklaringar och andra formateringselement för att skräddarsy diagrammets utseende efter dina specifika designkrav.