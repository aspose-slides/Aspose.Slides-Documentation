---
title: Skapa eller uppdatera diagram i PowerPoint-presentationer i C++
...
* identifiera avvikande värden, avvikelser, fel, nonsensdata etc. 
* kommunicera eller presentera komplex data

I PowerPoint kan du skapa diagram via infogningsfunktionen, som erbjuder mallar för många diagramtyper. Med Aspose.Slides kan du skapa vanliga diagram (baserade på populära diagramtyper) och anpassade diagram. 

{{% alert color="primary" %}} 

För att låta dig skapa diagram tillhandahåller Aspose.Slides [ChartType](https://reference.aspose.com/slides/sv/cpp/namespace/aspose.slides.charts#a23ba9ea390f5be4c8f5ab18baf4f8c05)‑enumklassen under namnutrymmet [Aspose::Slides::Charts](https://reference.aspose.com/slides/sv/cpp/namespace/aspose.slides.charts/). Värdena i denna enumklass motsvarar olika diagramtyper. 
...
1. Lägg till ett diagram med vissa data och ange önskad diagramtyp.  
...
// Sökvägen till dokumentkatalogen.
	...
	// Anger fyllnadsfärgen för serien
	...
	// Den första etiketten sätts att visa kategorinamn
...
Denna C++‑kod visar hur du skapar ett spritt diagram med olika markörer för serierna: 
...
// Sökvägen till dokumentkatalogen.
	...
	// Tar bort den standardgenererade serien 
	chart->get_ChartData()->get_Series()->Clear();
	...
	// Visar ledarlinjerna för diagrammet
	series->get_Labels()->get_DefaultDataLabelFormat()->set_ShowLeaderLines(true);

	// Anger rotationsvinkeln för sektorer i cirkeldiagram
...
1. Lägg till nya serier och kategorier.  
1. Lägg till nya diagramdata för diagramserierna.  
1. Lägg till nya punkter för diagrammen och anpassade färger för sektorerna i cirkeldiagrammet.  
1. Ställ in etiketter för serierna.  
1. Ställ in ledarstreck för serietiketter.  
...
```c++
	...
	// Tar bort den standardgenererade serien och kategorierna
	chart->get_ChartData()->get_Series()->Clear();
	chart->get_ChartData()->get_Categories()->Clear();
	...
	// Anger att serien ska visa ledarlinjer för diagrammet
	series->get_Labels()->get_DefaultDataLabelFormat()->set_ShowLeaderLines ( true);

	// Anger rotationsvinkeln för sektorerna i cirkeldiagrammet
	chart->get_ChartData()->get_SeriesGroups()->idx_get(0)->set_FirstSliceAngle ( 180);
...
Linjediagram (även kända som linjediagram) är bäst för situationer där du vill demonstrera förändringar i värde över tid. Med ett linjediagram kan du jämföra mycket data samtidigt, spåra förändringar och trender över tid, markera avvikelser i dataserier osv. 
...
Trädkartsdiagram är bäst för försäljningsdata när du vill visa den relativa storleken på datakategorier och samtidigt snabbt rikta uppmärksamheten mot poster som bidrar mest till varje kategori. 
...
Denna C++‑kod visar hur du skapar ett låd- och whisker‑diagram:
...
Denna C++‑kod visar hur du skapar ett soluppgångsdiagram:
...
1. Skapa en instans av [Presentation](https://reference.aspose.com/slides/sv/cpp/class/aspose.slides.presentation)-klassen.  
1. Hämta en bilds referens via dess index.  
1. Lägg till ett diagram med viss data och ange önskad diagramtyp (`ChartType.Histogram` i detta fall).  
1. Åtkomst till diagramdata `IChartDataWorkbook`.  
1. Rensa standardserier och kategorier.  
1. Lägg till nya serier och kategorier.  
1. Skriv den modifierade presentationen till en PPTX‑fil.  
...
1. Skapa en instans av [Presentation](https://reference.aspose.com/slides/sv/cpp/class/aspose.slides.presentation)-klassen.  
1. Hämta en bilds referens via dess index.  
1. Lägg till ett diagram med viss data och ange önskad diagramtyp (`ChartType.Radar` i detta fall).  
1. Skriv den modifierade presentationen till en PPTX‑fil.  
...
Ett kombinationsdiagram (eller combo‑diagram) kombinerar två eller fler diagramtyper i ett enda diagram. Detta diagram låter dig framhäva, jämföra eller undersöka skillnader mellan två eller fler dataset, vilket hjälper dig att identifiera samband mellan dem.
...
static SharedPtr<IChart> CreateChartWithFirstSeries(SharedPtr<ISlide> slide)
{
    ...
    // Ta bort den standardgenererade serien och kategorierna.
    chart->get_ChartData()->get_Series()->Clear();
    chart->get_ChartData()->get_Categories()->Clear();
...
static void SetPrimaryAxesFormat(SharedPtr<IChart> chart)
{
    ...
    // Sätt färgen för den vertikala huvudrutnätlinjen.
...
{
    // Sätt den sekundära horisontella axeln.
    ...
    // Sätt den sekundära vertikala axeln.
...
5. Modifiera diagramseriernas data genom att ändra serievärden.  
...
Denna C++‑kod visar hur du anger datapaketet för ett diagram:
...
Denna C++‑kod visar hur du automatiskt sätter en markör för en diagramserie:
...
Aspose.Slides stöder ett brett spektrum av diagramtyper, inklusive stapel, linje, cirkel, område, spridd, histogram, radar och många fler. Denna flexibilitet låter dig välja den mest lämpliga diagramtypen för dina visualiseringsbehov.

**Hur lägger jag till ett nytt diagram på en bild?**

För att lägga till ett diagram skapar du först en instans av [Presentation](https://reference.aspose.com/slides/sv/cpp/aspose.slides/presentation/)-klassen, hämtar den önskade bilden via dess index och anropar sedan metoden för att lägga till ett diagram, där du specificerar diagramtyp och initiala data. Detta integrerar diagrammet direkt i din presentation.

**Hur kan jag uppdatera data som visas i ett diagram?**

Du kan uppdatera ett diagrams data genom att få åtkomst till dess datarbok ([IChartDataWorkbook](https://reference.aspose.com/slides/sv/cpp/aspose.slides.charts/ichartdataworkbook/)), rensa eventuella standardserier och -kategorier och sedan lägga till din egen data. Detta gör att du programatiskt kan uppdatera diagrammet så att det speglar de senaste uppgifterna.

**Är det möjligt att anpassa diagrammets utseende?**

Ja, Aspose.Slides erbjuder omfattande anpassningsalternativ. Du kan ändra färger, typsnitt, etiketter, legender och andra formateringselement för att skräddarsy diagrammets utseende efter dina specifika designkrav.