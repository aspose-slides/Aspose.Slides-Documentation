...
- C++
- Aspose.Slides
description: "Aspose.Slides for C++ が開く・保存・変換できるすべてのファイル形式—PPT、PPTX、ODP など—を、明確なインポート/エクスポートサポート情報とともにご確認ください。"
---
## **概要**

Aspose.Slides は Microsoft PowerPoint 97 から Office 365 まで、Microsoft PowerPoint for Mac を含むプレゼンテーション ファイルをサポートします。本記事では、ライブラリがサポートする PowerPoint のバージョンと、読み込み・保存またはその両方が可能なファイル形式の表を示します。

また、PDF のコンプライアンス、フォント埋め込み、パスワード保護されたファイルの検出、カスタム フォント、フォントフォールバック、および XPS エクスポート オプションに関する一般的な質問にも回答します。

## **サポート対象の Microsoft PowerPoint バージョン**
...
## **サポート対象のファイル形式**
この表は Aspose.Slides for C++ が読み込みおよび保存できるファイル形式を示しています。
...
**アーカイブとアクセシビリティの基準（PDF/A と PDF/UA）を満たす PDF にプレゼンテーションを保存できますか？**

はい。Aspose.Slides は PDF エクスポート オプションの `compliance` 設定を使用して、PDF/A-2a、PDF/A-2b、PDF/A-2u、PDF/A-3a、PDF/A-3b および PDF/UA などのコンプライアンス レベルでのエクスポートをサポートします。

**PDF へのエクスポート時にフォント埋め込みをサポートし、埋め込むフォントを細かく制御できますか？**

はい。フォントを完全に埋め込むかサブセット化（使用したグリフのみ）するかを制御でき、一般的なシステム フォントの取り扱いを指定し、ASCII テキストの動作を PDF エクスポート オプションで設定できます。

**ファイルがパスワードで保護されているかどうかを、実際に読み込む前に検出できますか？**

はい。`PresentationFactory` のファクトリベース検査 API を使用すると、プレゼンテーション ファイルを完全に開かずにパスワード保護の有無を問い合わせることができます。

**フォントフォールバック機構やカスタム フォントのサポートはありますか？**

はい。ライブラリはカスタム フォントの [読み込み](/slides/ja/cpp/custom-font/) と [埋め込み](/slides/ja/cpp/embedded-font/) をサポートし、レンダリングや変換時に欠損グリフが発生しないようにフォント [フォールバック ルール](/slides/ja/cpp/fallback-font/) を提供します。

**スライドを XPS にエクスポートできますか、また XPS 出力を調整するオプションはありますか？**

はい。[XPS へのエクスポート](/slides/ja/cpp/convert-powerpoint-to-xps/) がサポートされており、関連する [保存オプション]((https://reference.aspose.com/slides/ja/cpp/aspose.slides.export/xpsoptions/)) を調整して XPS ドキュメントの品質や内容をコントロールできます。