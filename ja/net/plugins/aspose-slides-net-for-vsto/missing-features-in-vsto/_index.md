---
title: VSTOの不足している機能
type: docs
weight: 20
url: /ja/net/missing-features-in-vsto/
---

このセクションには以下のトピックが含まれています

- [プレゼンテーションにレイアウトスライドを追加](/slides/ja/net/add-layout-slide-to-presentation/)
- [PPTからPPTX形式への変換](/slides/ja/net/conversion-from-ppt-to-pptx-format/)
- [OpenOfficeドキュメントの変換](/slides/ja/net/conversion-of-openoffice-document/)
- [プレゼンテーションをHTMLに変換](/slides/ja/net/convert-presentation-to/)
- [ノート付きプレゼンテーションをTiffに変換](/slides/ja/net/convert-presentation-to-tiff-with-notes/)
- [プレゼンテーションをXPSに変換](/slides/ja/net/convert-presentation-to-xps/)
- [スライドをSVG画像として作成](/slides/ja/net/create-slide-as-svg-image/)
- [メディアファイルをHTMLファイルにエクスポート](/slides/ja/net/export-media-files-into-file/)
- [スライドのサムネイルをJPEGとして生成](/slides/ja/net/generate-slide-thumbnail-as-jpeg/)
- [ユーザー定義の寸法を持つスライドからサムネイルを生成](/slides/ja/net/generating-a-thumbnail-from-a-slide-with-user-defined-dimensions/)
- [プレゼンテーションのロック](/slides/ja/net/presentation-locking/)
- [スライド上のシェイプを画像としてレンダリング](/slides/ja/net/rendering-shapes-on-slide-as-images/)