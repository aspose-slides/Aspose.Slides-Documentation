---
title: Python でプレゼンテーションのチャート データ ラベルを管理する
linktitle: データ ラベル
type: docs
...
- Python
- Aspose.Slides
description: "Aspose.Slides for Python via .NET を使用して、PowerPoint および OpenDocument のプレゼンテーションにチャート データ ラベルを追加および書式設定し、スライドをより魅力的にする方法を学びます。"
---

## **概要**

チャートのデータ ラベルは、チャート データ系列または個々のデータ ポイントに関する詳細を示します。これにより、読者はデータ系列をすばやく識別でき、チャートの理解が容易になります。Aspose.Slides for Python では、任意のチャートに対してデータ ラベルを有効化、カスタマイズ、書式設定できます。表示内容（値、パーセンテージ、系列名またはカテゴリ名）、ラベルの配置位置、外観（フォント、数値書式、区切り文字、リーダーラインなど）を選択できます。本記事では、チャートに明確で有益なラベルを追加するために必要な主要 API とサンプルを紹介します。

## **データ ラベルの精度を設定する**

チャート データ ラベルは数値を表示することが多く、精度を一定に保つ必要があります。このセクションでは、適切な数値書式を適用して Aspose.Slides のデータ ラベルの小数点以下桁数を制御する方法を示します。

次の Python の例は、チャート データ ラベルの数値精度を設定する方法を示しています。
...
## **チャート データ ラベルにパーセント記号を表示する**

このセクションでは、チャート データ ラベルにパーセンテージを表示し、パーセント記号を含める方法を示します。シリーズ全体または個別のポイントに対してパーセンテージ値を有効化する方法（円グラフ、ドーナツ グラフ、100％ スタック ドチャートに最適）と、ラベルオプションやカスタム数値書式を介して書式設定を制御する方法を学びます。

次の Python の例は、チャートのデータ ラベルにパーセント記号を追加する方法を示しています。
...
次の Python コードは、軸ベースのチャートでカテゴリ軸からのラベル距離を設定する方法を示しています。
...
軸を使用しないチャート（例：円グラフ）を作成すると、データ ラベルがエッジに近すぎることがあります。その場合は、リーダーラインがはっきり表示されるようにラベル位置を調整します。
...
![ラベル位置を変更した画像](changed_label_position.png)
...
**ゼロ、負、または空の値に対してラベルだけを無効にするにはどうすればよいですか？**

ラベルを有効化する前にデータポイントをフィルタリングし、0、負の値、または欠損値に対して表示をオフにするルールを適用します。

**PDF/画像へエクスポートする際にラベルスタイルを一貫させるにはどうすればよいですか？**

フォント（ファミリー、サイズ）を明示的に設定し、レンダリング側にフォントが存在することを確認してフォールバックを防止します。