...
تشرح هذه المقالة كيفية تحويل عرض تقديمي PowerPoint إلى تنسيق HTML باستخدام C++. وتغطي المواضيع التالية.
...
* تحويل عرض تقديمي PowerPoint إلى HTML مع خطوط أصلية أو مضمنة.
...
1. إنشاء مثيل من الفئة [Presentation](https://reference.aspose.com/slides/cpp/class/aspose.slides.presentation).
   * حمّل **.ppt** في الفئة _Presentation_ لـ **Convert PPT to HTML in C++**
   * حمّل **.pptx** في الفئة _Presentation_ لـ **Convert PPTX to HTML in C++**
   * حمّل **.odp** في الفئة _Presentation_ لـ **Convert ODP to HTML in C++**
...
يعرض لك هذا الكود كيفية تحويل PowerPoint إلى HTML في C++:
```cpp
...
توفر Aspose.Slides الفئة [ResponsiveHtmlController](https://reference.aspose.com/slides/cpp/class/aspose.slides.export.responsive_html_controller) التي تسمح لك بإنشاء ملفات HTML متجاوبة. يعرض لك هذا الكود كيفية تحويل عرض تقديمي PowerPoint إلى HTML متجاوب في C++:
```cpp
// إنشاء كائن Presentation يمثل ملف عرض تقديمي
auto presentation = System::MakeObject<Presentation>(u"Convert_HTML.pptx");
...
يعرض لك هذا الكود كيفية تحويل PowerPoint إلى HTML مع الملاحظات في C++:
```cpp
...
توفر Aspose.Slides الفئة [EmbedAllFontsHtmlController](https://reference.aspose.com/slides/cpp/class/aspose.slides.export.embed_all_fonts_html_controller) التي تسمح لك بدمج جميع الخطوط في العرض أثناء تحويله إلى HTML.

لمنع دمج بعض الخطوط، يمكنك تمرير مصفوفة من أسماء الخطوط إلى مُنشئ برامتر من الفئة [EmbedAllFontsHtmlController](https://reference.aspose.com/slides/cpp/class/aspose.slides.export.embed_all_fonts_html_controller). الخطوط الشائعة مثل Calibri أو Arial، عند استخدامها في عرض تقديمي، لا تحتاج إلى دمجها لأن معظم الأنظمة تحتوي على هذه الخطوط مسبقًا. عندما يتم دمج هذه الخطوط، يصبح مستند HTML الناتج كبيرًا بشكل غير ضروري.

تدعم الفئة [EmbedAllFontsHtmlController](https://reference.aspose.com/slides/cpp/class/aspose.slides.export.embed_all_fonts_html_controller) الوراثة وتوفر الطريقة [WriteFont](https://reference.aspose.com/slides/cpp/class/aspose.slides.export.embed_all_fonts_html_controller#a1dfd1c26bb181c8581ec67d270ce0b77) التي يُقصد أن يتم تجاوزها.
```cpp
...
بشكل افتراضي، عند تحويل PowerPoint إلى HTML، تُصدر Aspose.Slides ملف HTML صغير مع صور بدقة 72 DPI وتُحذف المناطق المقتطعة. للحصول على ملفات HTML بصور ذات جودة أعلى، يجب ضبط خاصية `PicturesCompression` (من فئة `HtmlOptions`) إلى 96 (أي `PicturesCompression::Dpi96`) أو قيم أعلى [القيم](https://reference.aspose.com/slides/cpp/namespace/aspose.slides.export#adc51ca67b7e5c99f6fad75b02ebfd6d8).

يعرض لك هذا الكود C++ كيفية تحويل عرض تقديمي PowerPoint إلى HTML مع الحصول على صور عالية الجودة بدقة 150 DPI (أي `PicturesCompression::Dpi150`):
```cpp
...
يعرض لك هذا الكود في C++ كيفية إخراج HTML بصور ذات جودة كاملة:
```cpp
auto pres = System::MakeObject<Presentation>(u"InputDoc.pptx");
...
## **حفظ CSS والملفات عندما يتم التصدير إلى HTML**

باستخدام ملفات نمط CSS الجديدة، يمكنك بسهولة تغيير نمط ملف HTML الناتج من عملية تحويل PowerPoint إلى HTML.

يعرض لك كود C++ في هذا المثال كيفية استخدام طرق قابلة للتجاوز لإنشاء مستند HTML مخصص مع رابط إلى ملف CSS:
``` cpp
...
إذا كنت لا ترغب في دمج الخطوط (لتجنب زيادة حجم HTML الناتج)، يمكنك ربط جميع الخطوط بتنفيذ نسخة `LinkAllFontsHtmlController` الخاصة بك.

يعرض لك هذا الكود C++ كيفية تحويل PowerPoint إلى HTML مع ربط جميع الخطوط واستثناء "Calibri" و "Arial" (لأنهما موجودان بالفعل في النظام):
```cpp
...
يعرض لك هذا الكود C++ كيفية تحويل عرض تقديمي PowerPoint إلى HTML متجاوب:
```cpp
...
يعرض لك هذا الكود C++ كيفية إضافة فيديو إلى العرض ثم حفظه كـ HTML:
```cpp
 // يحمّل عرض تقديمي
...
يعتمد الأداء على حجم وتعقيد العروض التقديمية. Aspose.Slides فعال للغاية وقابل للتوسع للعمليات الدُفعية. لتحقيق الأداء الأمثل عند تحويل عدد كبير من العروض، يُنصح باستخدام تعدد الخيوط أو المعالجة المتوازية كلما كان ذلك ممكنًا.

**هل يدعم Aspose.Slides تصدير الروابط التشعبية إلى HTML؟**

نعم، يدعم Aspose.Slides بالكامل تصدير الروابط التشعبية المدمجة إلى HTML. عند تحويل العروض إلى تنسيق HTML، يتم الحفاظ على الروابط تلقائيًا وتظل قابلة للنقر.

**هل هناك أي حد لعدد الشرائح عند تحويل العروض إلى HTML؟**

لا يوجد حد لعدد الشرائح عند استخدام Aspose.Slides. يمكنك تحويل العروض من أي حجم. ومع ذلك، بالنسبة للعروض التي تحتوي على عدد كبير جدًا من الشرائح، قد يعتمد الأداء على الموارد المتاحة على الخادم أو النظام.