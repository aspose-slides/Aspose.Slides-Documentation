...
- C++
- Aspose.Slides
description: "تحويل عروض PowerPoint إلى HTML متجاوب في C++. الحفاظ على التخطيط والروابط والصور باستخدام دليل تحويل Aspose.Slides للحصول على نتائج سريعة وخالية من الأخطاء."
---

## **نظرة عامة**

تشرح هذه المقالة كيفية تحويل عرض PowerPoint إلى صيغة HTML باستخدام C++. يغطي المقال المواضيع التالية.
...
للحصول على عينة كود C++ لتحويل PowerPoint إلى HTML، يرجى الاطلاع على القسم أدناه أي [تحويل PowerPoint إلى HTML](#convert-powerpoint-to-html). يمكن للرمز تحميل عدد من الصيغ مثل PPT و PPTX و ODP في كائن Presentation وحفظه بصيغة HTML.
...
**Aspose.Slides** يقدم العديد من الخيارات (معظمها من فئة [**HtmlOptions**](https://reference.aspose.com/slides/cpp/class/aspose.slides.export.html_options)) التي تحدد عملية التحويل من PowerPoint إلى HTML:
...
إلى جانب عمليات التحويل المذكورة هنا، يدعم Aspose.Slides عمليات تحويل أخرى تتعلق بصيغة HTML:
...
1. أنشئ مثيلاً من فئة [Presentation](https://reference.aspose.com/slides/cpp/class/aspose.slides.presentation).
   * حمّل **.ppt** في فئة _Presentation_ لـ **Convert PPT to HTML in C++**
   * حمّل **.pptx** في فئة _Presentation_ لـ **Convert PPTX to HTML in C++**
   * حمّل **.odp** في فئة _Presentation_ لـ **Convert ODP to HTML in C++**
...
هذا الكود يوضح كيفية تحويل PowerPoint إلى HTML في C++:
```cpp
...
يوفر Aspose.Slides فئة [ResponsiveHtmlController](https://reference.aspose.com/slides/cpp/class/aspose.slides.export.responsive_html_controller) التي تسمح بإنشاء ملفات HTML متجاوبة. هذا الكود يوضح كيفية تحويل عرض PowerPoint إلى HTML متجاوب في C++:
```cpp
// إنشاء كائن Presentation يمثل ملف عرض تقديمي
auto presentation = System::MakeObject<Presentation>(u"Convert_HTML.pptx");
...
هذا الكود يوضح كيفية تحويل PowerPoint إلى HTML مع الملاحظات في C++:
```cpp
...
يوفر Aspose.Slides فئة [EmbedAllFontsHtmlController](https://reference.aspose.com/slides/cpp/class/aspose.slides.export.embed_all_fonts_html_controller) التي تسمح بدمج جميع الخطوط في العرض أثناء تحويله إلى HTML.

لمنع دمج خطوط معينة، يمكنك تمرير مصفوفة من أسماء الخطوط إلى مُنشئ بارامتر من فئة [EmbedAllFontsHtmlController](https://reference.aspose.com/slides/cpp/class/aspose.slides.export.embed_all_fonts_html_controller). الخطوط الشائعة مثل Calibri أو Arial لا تحتاج إلى دمجها لأنها موجودة مسبقًا في معظم الأنظمة. عندما يتم دمج هذه الخطوط، يصبح مستند HTML كبيرًا غير ضروري.

تدعم فئة [EmbedAllFontsHtmlController](https://reference.aspose.com/slides/cpp/class/aspose.slides.export.embed_all_fonts_html_controller) الوراثة وتوفر طريقة [WriteFont](https://reference.aspose.com/slides/cpp/class/aspose.slides.export.embed_all_fonts_html_controller#a1dfd1c26bb181c8581ec67d270ce0b77) التي من المفترض أن تُعاد كتابةها.
```cpp
...
افتراضيًا، عند تحويل PowerPoint إلى HTML، ينتج Aspose.Slides ملفات HTML صغيرة مع صور بدقة 72 DPI ومساحات مقصوصة محذوفة. للحصول على ملفات HTML بصور عالية الجودة، عليك تعيين خاصية `PicturesCompression` (من فئة `HtmlOptions`) إلى 96 (أي `PicturesCompression::Dpi96`) أو قيم أعلى.

هذا الكود C++ يوضح كيفية تحويل عرض PowerPoint إلى HTML مع الحصول على صور عالية الجودة بدقة 150 DPI (أي `PicturesCompression::Dpi150`):
```cpp
...
هذا الكود في C++ يوضح كيفية إخراج HTML بصور بجودة كاملة:
```cpp
auto pres = System::MakeObject<Presentation>(u"InputDoc.pptx");
...
الكود C++ في هذا المثال يوضح كيفية استخدام طرق قابلة للإعادة لتكوين مستند HTML مخصص مع رابط إلى ملف CSS:
``` cpp
...
إذا كنت لا تريد دمج الخطوط (لتجنب زيادة حجم HTML الناتج)، يمكنك ربط جميع الخطوط عن طريق تنفيذ نسختك الخاصة من `LinkAllFontsHtmlController`.
...
public:
        ...
        String fontStyle, String fontWeight, ArrayPtr<uint8_t> fontData)
    {
        ...
        String path = String::Format(u"{0}.woff", fontName); // قد تحتاج إلى تنقية المسار
...
**ما هو أداء Aspose.Slides عند تحويل عدة عروض إلى HTML؟**

يعتمد الأداء على حجم وتعقيد العروض. Aspose.Slides فعال للغاية وقابل للتوسع لعمليات الدفعة. لتحقيق أداء أمثل عند تحويل عدد كبير من العروض، يُنصح باستخدام تعدد الخيوط أو المعالجة المتوازية كلما أمكن ذلك.

**هل يدعم Aspose.Slides تصدير الروابط التشعبية إلى HTML؟**

نعم، يدعم Aspose.Slides تصدير الروابط التشعبية المدمجة إلى HTML بالكامل. عند تحويل العروض إلى صيغة HTML، تُحافظ على الروابط تلقائيًا وتبقى قابلة للنقر.
