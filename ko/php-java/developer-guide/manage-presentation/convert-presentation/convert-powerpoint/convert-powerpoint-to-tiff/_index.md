...
- PHP
- Aspose.Slides
description: "Aspose.Slides for PHP via Java을 사용하여 PowerPoint(PPT, PPTX) 프레젠테이션을 고품질 TIFF 이미지로 쉽게 변환하는 방법을 코드 예제와 함께 배웁니다."
---
## **소개**

TIFF (**Tagged Image File Format**)은 널리 사용되는 무손실 래스터 이미지 형식으로, 뛰어난 품질과 그래픽의 상세한 보존으로 알려져 있습니다. 디자이너, 사진작가 및 데스크톱 퍼블리셔는 종종 이미지의 레이어, 색 정확도 및 원본 설정을 유지하기 위해 TIFF를 선택합니다.

Aspose.Slides를 사용하면 PowerPoint 슬라이드(PPT, PPTX)와 OpenDocument 슬라이드(ODP)를 직접 고품질 TIFF 이미지로 손쉽게 변환할 수 있어 프레젠테이션이 최대한의 시각적 사실성을 유지합니다.

## **프레젠테이션을 TIFF로 변환**

제공되는 [save](https://reference.aspose.com/slides/ko/php-java/aspose.slides/presentation/#save) 메서드와 [Presentation](https://reference.aspose.com/slides/ko/php-java/aspose.slides/presentation/) 클래스를 사용하면 전체 PowerPoint 프레젠테이션을 빠르게 TIFF로 변환할 수 있습니다. 결과 TIFF 이미지는 기본 슬라이드 크기에 해당합니다.
...
특정 크기의 TIFF 이미지가 필요하면 [TiffOptions](https://reference.aspose.com/slides/ko/php-java/aspose.slides/tiffoptions/)에서 제공되는 메서드를 사용하여 원하는 값을 설정할 수 있습니다. 예를 들어, [setImageSize](https://reference.aspose.com/slides/ko/php-java/aspose.slides/tiffoptions/#getImageSize) 메서드를 사용하면 결과 이미지의 크기를 정의할 수 있습니다.
...
try {
    ...
    /*
    ImagePixelFormat은(는) 문서에 명시된 다음 값을 포함합니다:
        Format1bppIndexed - 1 비트(픽셀당), 인덱싱됨.
        Format4bppIndexed - 4 비트(픽셀당), 인덱싱됨.
        Format8bppIndexed - 8 비트(픽셀당), 인덱싱됨.
        Format24bppRgb    - 24 비트(픽셀당), RGB.
        Format32bppArgb   - 32 비트(픽셀당), ARGB.
    */
