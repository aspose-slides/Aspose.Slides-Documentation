...
이 문서는 Aspose.Slides for .NET을 사용하여 PowerPoint 및 OpenDocument 프레젠테이션에서 텍스트를 서식 지정하는 방법을 보여줍니다. 배경 색, 투명도, 문자 간격, 글꼴 속성, 회전, 단락 간격, 자동 맞춤 동작, 텍스트 앵커링, 탭 정지 및 언어 설정을 다룹니다.

아래 예제에서는 첫 번째 슬라이드에 단일 텍스트 상자가 포함된 "sample.pptx" 파일을 사용합니다. 해당 텍스트는 다음과 같습니다:

![샘플 텍스트](sample_text.png)

리터럴 텍스트 또는 정규식 매치를 찾고 강조 표시하려면 [텍스트 검색 및 교체](/slides/ko/net/search-and-replace-text/)를 참조하세요.

## **텍스트 배경 색 설정**

[IParagraphFormat.DefaultPortionFormat](https://reference.aspose.com/slides/ko/net/aspose.slides/iparagraphformat/defaultportionformat/)을 사용하여 단락의 기본 강조 색을 설정하거나, 개별 텍스트 부분에 대해 [IBasePortionFormat.HighlightColor](https://reference.aspose.com/slides/ko/net/aspose.slides/ibaseportionformat/highlightcolor/)을 사용할 수 있습니다.
...
[IParagraphFormat.Alignment](https://reference.aspose.com/slides/ko/net/aspose.slides/iparagraphformat/alignment/)을 사용하여 텍스트 프레임 내에서 단락 정렬을 설정합니다. 값은 가운데, 왼쪽 정렬, 오른쪽 정렬, 양쪽 정렬 등으로 설정할 수 있습니다.
...
텍스트 투명도는 [IBasePortionFormat.FillFormat](https://reference.aspose.com/slides/ko/net/aspose.slides/ibaseportionformat/fillformat/)에 할당된 색상의 알파 구성 요소를 통해 제어됩니다. 아래 예제에서 `alpha = 50`은 0–255 스케일의 ARGB 알파 채널 값이며, 투명도 비율이 아닙니다.
...
텍스트 상자에서 문자 간 간격을 확대하거나 축소하려면 [IBasePortionFormat.Spacing](https://reference.aspose.com/slides/ko/net/aspose.slides/ibaseportionformat/spacing/)을 사용합니다.

다음 C# 코드는 **전체 단락**에서 문자 간격을 확대하는 방법을 보여줍니다:
...
경우에 따라 Aspose.Slides가 렌더링한 텍스트가 PowerPoint에 표시된 동일한 텍스트보다 약간 더 조밀하게 보일 수 있습니다. 이는 PowerPoint가 해당 글꼴에 유효한 커닝 정보가 포함되어 있고 PowerPoint 설정에서 커닝이 활성화되어 있어도 특정 글꼴에 대한 커닝 데이터를 무시할 수 있기 때문입니다.

이러한 경우 렌더링된 출력이 PowerPoint와 더 가깝게 만들려면 영향을 받는 글꼴을 사용하는 텍스트 부분에 대해 커닝을 비활성화할 수 있습니다. [IBasePortionFormat.KerningMinimalSize](https://reference.aspose.com/slides/ko/net/aspose.slides/ibaseportionformat/kerningminimalsize/)을 실제 글꼴 크기보다 크게 설정합니다:
...
다음 코드는 전체 단락에 대한 글꼴 및 텍스트 스타일을 설정합니다. 여기서는 글꼴 크기, 굵게, 기울임꼴, 점선 밑줄 및 Times New Roman 글꼴을 단락의 모든 부분에 적용합니다.
...
아래 코드 예제는 **굵은 글꼴이 적용된 텍스트 부분**에 유사한 속성을 적용합니다:
...
다음 코드 예제는 도형 내 텍스트 방향을 `Vertical270`으로 설정합니다. 이는 텍스트를 **시계 반대 방향으로 90도** 회전시킵니다:
...
[ITextFrameFormat.AutofitType](https://reference.aspose.com/slides/ko/net/aspose.slides/itextframeformat/autofittype/)은 텍스트가 컨테이너 경계를 초과할 때 텍스트가 어떻게 동작하는지를 결정합니다. 이를 사용하여 �스트가 축소, 넘침, 또는 도형을 자동으로 크기 조정하도록 제어합니다.
...
자동 줄 바꿈 후 줄 수를 세고 텍스트 또는 도형 너비가 결과에 어떻게 영향을 미치는지 확인하려면 [렌더링된 줄 수 카운트](/slides/ko/net/manage-paragraph/)를 참조하십시오. 줄 수만으로는 텍스트가 컨테이너를 초과하는지 여부를 판단할 수 없습니다.
...
[IParagraphFormat.DefaultTabSize](https://reference.aspose.com/slides/ko/net/aspose.slides/iparagraphformat/defaulttabsize/)와 [IParagraphFormat.Tabs](https://reference.aspose.com/slides/ko/net/aspose.slides/iparagraphformat/tabs/)을 사용하여 단락의 탭 정지를 구성합니다.
...
Aspose.Slides는 [IBasePortionFormat.LanguageId](https://reference.aspose.com/slides/ko/net/aspose.slides/ibaseportionformat/languageid/)를 제공하여 텍스트 부분의 교정 언어를 설정할 수 있습니다. 교정 언어는 PowerPoint에서 맞춤법 및 문법 검사를 수행할 때 사용하는 언어를 결정합니다.
...
다음 코드 예제는 새 프레젠테이션에서 모든 슬라이드 텍스트에 대해 14포인트 크기의 기본 굵은 글꼴을 설정하는 방법을 보여줍니다.
...
PowerPoint에서 **All Caps** 글꼴 효과를 적용하면 원래 소문자로 입력된 경우에도 슬라이드에 텍스트가 대문자로 표시됩니다. Aspose.Slides를 사용하여 해당 텍스트 부분을 검색하면 라이브러리는 입력된 그대로의 텍스트를 반환합니다. 표시된 텍스트와 일치시키려면 [TextCapType](https://reference.aspose.com/slides/ko/net/aspose.slides/textcaptype/)을 확인하고 값이 `All`인 경우 반환된 문자열을 대문자로 변환하십시오.
