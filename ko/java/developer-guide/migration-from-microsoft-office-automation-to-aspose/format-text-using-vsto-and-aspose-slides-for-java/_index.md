...
- Java
- Aspose.Slides
description: "Microsoft Office 자동화에서 Aspose.Slides for Java로 마이그레이션하고 PowerPoint(PPT, PPTX) 프레젠테이션에서 텍스트를 정밀하게 제어하며 서식 지정합니다."
---
{{% alert color="info" %}} 

때때로 슬라이드의 텍스트를 프로그래밍 방식으로 서식 지정해야 할 때가 있습니다. 이 문서에서는 VSTO([VSTO](/slides/ko/java/format-text-using-vsto-and-aspose-slides-for-java/)) 또는 Aspose.Slides for Java([Aspose.Slides for Java](/slides/ko/java/format-text-using-vsto-and-aspose-slides-for-java/))를 사용하여 첫 번째 슬라이드에 텍스트가 있는 샘플 프레젠테이션을 읽는 방법을 보여줍니다. 코드는 슬라이드의 세 번째 텍스트 상자에 있는 텍스트를 마지막 텍스트 상자의 텍스트와 동일하게 서식 지정합니다.

{{% /alert %}} 
## **텍스트 서식 지정**
VSTO와 Aspose.Slides 두 방법 모두 다음 단계로 진행됩니다:
...
아래 스크린샷은 VSTO와 Aspose.Slides for Java 코드를 실행하기 전후의 샘플 슬라이드를 보여줍니다.
...
Aspose.Slides를 사용해 텍스트를 서식 지정하려면, 텍스트를 서식 지정하기 전에 글꼴을 추가합니다.
