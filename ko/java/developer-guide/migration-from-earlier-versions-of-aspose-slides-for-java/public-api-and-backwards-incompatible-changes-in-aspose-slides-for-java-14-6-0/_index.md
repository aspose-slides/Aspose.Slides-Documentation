---
title: Aspose.Slides for Java 14.6.0의 공개 API 및 이전 호환되지 않는 변경 사항
...
이 페이지에서는 Aspose.Slides for Java 14.6.0 API에 도입된 모든 [추가된](/slides/ko/java/public-api-and-backwards-incompatible-changes-in-aspose-slides-for-java-14-6-0/) 클래스, 메서드, 속성 등을 나열하고, 새로운 제한 사항 및 기타 변경 사항을 포함합니다.
...
IPresentation.getViewProperty() 메서드는 IViewProperties에 대한 액세스를 제공하며, 프레젠테이션이 Microsoft PowerPoint에서 열릴 때 프레젠테이션 보기 유형 및 노트 표시 여부를 변경할 수 있습니다.
...
이 인터페이스는 ChartDataPoint.ErrorBarsCustomValues 속성 목록에 있는 값 유형을 지정합니다.
...
IErrorBarsFormat.ValueType 속성이 Custom으로 설정된 경우, 시리즈의 DataPoints 컬렉션에 있는 특정 데이터 포인트의 ErrorBarCustomValues 속성을 사용하여 값을 지정합니다.
...
이 인터페이스는 차트 시리즈의 오류 막대를 나타냅니다. 사용자 지정 값 유형인 경우, 시리즈의 DataPoins 컬렉션에 있는 특정 데이터 포인트의 ErrorBarCustomValues 속성을 사용하여 값을 지정합니다.
