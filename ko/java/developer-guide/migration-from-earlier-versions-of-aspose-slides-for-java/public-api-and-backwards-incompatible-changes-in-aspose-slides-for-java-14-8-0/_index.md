---
title: Aspose.Slides for Java 14.8.0의 공개 API 및 뒤로 호환되지 않는 변경 사항
...
이 페이지에서는 Aspose.Slides for Java 14.8.0 API와 함께 도입된 모든 [added](/slides/ko/java/public-api-and-backwards-incompatible-changes-in-aspose-slides-for-java-14-8-0/) 클래스, 메서드, 속성 등을 나열하고, 새로운 제한 사항 및 기타 [changes](/slides/ko/java/public-api-and-backwards-incompatible-changes-in-aspose-slides-for-java-14-8-0/)를 설명합니다.
...
Aspose.Slides.Charts.IChartSeries.getOverlap() 메서드는 2D 차트에서 막대와 열이 얼마나 겹쳐야 하는지(-100에서 100 사이)의 값을 가져옵니다. 이 메서드는 특정 시리즈에만 적용되는 것이 아니라 상위 시리즈 그룹의 모든 시리즈에 적용되며, 해당 그룹 속성의 투영입니다.

- 상위 시리즈 그룹에 접근하려면 IChartSeries.getParentSeriesGroup() 메서드를 사용하십시오.
- 값을 관리하려면 IChartSeriesGroup.getOverlap() 및 setOverlap(byte) 메서드를 사용하십시오.
...
이 형태 썸네일 생성 방법을 사용하면 개발자가 해당 외관의 경계 내에서 형태 썸네일을 생성할 수 있습니다. 모든 형태 효과를 고려하며, 생성된 썸네일은 슬라이드 경계에 제한됩니다.
...
새 기능을 통해 개발자는 프레젠테이션에서 VBA 프로젝트를 생성하고 편집할 수 있습니다.
