---
title: Aspose.Slides for PHP via Java Docker ile Kurulum
type: docs
weight: 75
url: /tr/php-java/installing-slides-php-using-docker/
keywords:
...
- PHP
- Aspose.Slides
description: "Docker konteynerlerinde Aspose.Slides çalıştırın: görüntüleri, bağımlılıkları, yazı tiplerini ve lisanslamayı yapılandırarak PowerPoint ve OpenDocument işleyen ölçeklenebilir hizmetler oluşturun."
---
## **Önkoşullar**
* Makinenize Docker kurun. Resmi kurulum kılavuzunu [burada](https://docs.docker.com/get-docker/) bulabilirsiniz.
...
### **2. Docker Görüntüsünü Oluşturun** 
   Dockerfile'ın bulunduğu dizinde aşağıdaki komutu çalıştırarak Docker görüntüsünü oluşturun:
   ```bash
   docker build -t aspose-slides-php-java .
   ```

### **3. Docker Konteynerini Çalıştırın** 
   Görüntü oluşturulduktan sonra konteyneri başlatın:
   ```bash
   docker run -p 8080:8080 aspose-slides-php-java
   ```

### 4. **Docker'da Aspose.Slides'e Erişin** 
   Konteyneri başlattıktan sonra script bir PDF dosyası oluşturacaktır. Oluşturulan `output.pdf` dosyasını konteyner içindeki `/tmp` klasöründe bulabilirsiniz:
   ```bash
   docker exec -it <container-id> ls /tmp
   ```
   Oluşturulan PDF dosyasını yerel makinenize kopyalamak için aşağıdaki komutu çalıştırın:
   ```bash
