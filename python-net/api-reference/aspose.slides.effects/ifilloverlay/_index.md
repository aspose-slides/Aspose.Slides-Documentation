---
title: IFillOverlay Class - Aspose.Slides for Python via .NET - API Reference
type: docs
weight: 600
url: /python-net/api-reference/aspose.slides.effects/ifilloverlay/
---

Represents a Fill Overlay effect. A fill overlay may be used to specify<br/>            an additional fill for an object and blend the two fills together.

**Namespace:** [aspose.slides.effects](/python-net/api-reference/aspose.slides.effects/)

**Full Class Name:** aspose.slides.effects.IFillOverlay

**Assembly:**  Aspose.Slides Version: 21.12.0.0

The IFillOverlay type exposes the following members:
## **Properties**
|**Name**|**Description**|
| :- | :- |
|blend|FillBlendMode.<br/>            Read/write [FillBlendMode](/python-net/api-reference/aspose.slides/fillblendmode/).|
|fill_format|Fill format.<br/>            Read-only [IFillFormat](/python-net/api-reference/aspose.slides/ifillformat/).|
|as_iimage_transform_operation|Allows to get base IImageTransformOperation interface.<br/>            Read-only [IImageTransformOperation](/python-net/api-reference/aspose.slides.effects/iimagetransformoperation/).|
## **Methods**
|**Name**|**Description**|
| :- | :- |
|get_effective()|Gets effective object formatting data with the inheritance applied.|
