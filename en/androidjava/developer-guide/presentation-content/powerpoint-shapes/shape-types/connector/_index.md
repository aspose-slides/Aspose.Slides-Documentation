...
- Java
- Aspose.Slides
description: "Empower Java apps to draw, connect and auto-route lines in PowerPoint slides on Android—gain full control over straight, elbow and curved connectors."
---
...
1. Add two [AutoShape](https://reference.aspose.com/slides/androidjava/com.aspose.slides/AutoShape) to the slide using the `addAutoShape` method exposed by the `Shapes` object.
...
You can adjust an existing connector through its adjustment points. Only connectors with adjustment points can be altered in this manner. See the table under **[Types of connectors.](/slides/androidjava/connector/#types-of-connectors)**
...
* For calculations that determine the coordinates of a connector's adjustment points, you have to take the connector's rotation and its reflection into account. **Note** that the rotation angle for all connectors shown under **[Types of connectors](/slides/androidjava/connector/#types-of-connectors)** is 0.
...
Check that the shape exposes [connection sites](https://reference.aspose.com/slides/androidjava/com.aspose.slides/shape/#getConnectionSiteCount--). If there are none or the count is zero, gluing isn’t available; in that case, use free endpoints and position them manually. It’s sensible to check the site count before attaching.

**What happens to a connector if I delete one of the connected shapes?**

Its ends will be detached; the connector remains on the slide as an ordinary line with free start/end. You can either delete it or reassign the connections and, if needed, [reroute](https://reference.aspose.com/slides/androidjava/com.aspose.slides/connector/#reroute--).
