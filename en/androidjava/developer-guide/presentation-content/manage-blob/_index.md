...
Aspose.Slides for Android via Java allows you to use BLOBs for objects in a way that reduces memory consumption when large files are involved.
...
### **Export a Large File through BLOB from a Presentation**
Aspose.Slides for Android via Java allows you to export large files (in this case, an audio or video file) through a process involving BLOBs from presentations. For example, you may need to extract a large media file from a presentation but do not want the file to be loaded into your computer's memory. By exporting the file through the BLOB process, you get to keep memory consumption low.
...
### **Add an Image as BLOB in a Presentation**
...
When processing large presentations, ensure that the [Presentation](https://reference.aspose.com/slides/androidjava/com.aspose.slides/presentation/) instance is properly disposed so that the memory it occupied is released. Call `dispose()` after you have finished using the presentation to free unmanaged resources.
...
Use [LoadOptions](https://reference.aspose.com/slides/androidjava/com.aspose.slides/loadoptions/) with [BlobManagementOptions](https://reference.aspose.com/slides/androidjava/com.aspose.slides/blobmanagementoptions/). There you set the in-memory limit for BLOB, allow or disallow temporary files, choose the root path for temp files, and select source locking behavior.

**Do BLOB settings affect performance, and how do I balance speed vs memory?**

Yes. Keeping BLOB in memory maximizes speed but increases RAM consumption; lowering the memory limit shifts more work to temporary files, reducing RAM at the cost of additional I/O. Use the [setMaxBlobsBytesInMemory](https://reference.aspose.com/slides/androidjava/com.aspose.slides/blobmanagementoptions/#setMaxBlobsBytesInMemory-long-) method to reach the right balance for your workload and environment.

**Do BLOB options help when opening extremely large presentations (e.g., gigabytes)?**

Yes. [BlobManagementOptions](https://reference.aspose.com/slides/androidjava/com.aspose.slides/blobmanagementoptions/) are designed for such scenarios: enabling temporary files and using source locking can significantly reduce peak RAM use and stabilize processing for very large decks.
