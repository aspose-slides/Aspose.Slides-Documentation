...
Aspose.Slides for Android via Java directly writes API information and Version Number in output documents. For example, when it converts a presentation to PDF, Aspose.Slides for Android via Java populates the Application field with the '*Aspose.Slides*' value and the PDF Producer field with a value in '*Aspose.Slides v XX.XX*'  form. **Note** that you cannot instruct Aspose.Slides for Android via Java to change or remove this information from output documents.
