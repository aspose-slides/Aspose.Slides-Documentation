---
title: Integrating Excel Data into PowerPoint Presentations with Aspose.Slides
linktitle: "Excel Integration"
type: docs
weight: 330
url: /net/developer-guide/technical-articles/excel-integration/
keywords:
  - Excel
  - Excel workbook
  - read Excel
...
# Integrating Excel Data into PowerPoint Presentations with Aspose.Slides
...
ExcelDataWorkbook wb = new ExcelDataWorkbook(@"templateData.xlsx");
...
for (int row = 1; row <= 4; row++)
{
    ...
    ISlide slide = pres.Slides.AddClone(templatePresentation.Slides[0]);

    // Get paragraphs from the target shape (assumes shape index 1 is used)
    IParagraphCollection paragraphs = (slide.Shapes[1] as AutoShape).TextFrame.Paragraphs;

    // Replace placeholders with data from Excel
    string employeeName = wb.GetCell(0, row, 0).Value.ToString();
    paragraphs[0].Portions[0].Text = paragraphs[0].Portions[0].Text.Replace("{{EmployeeName}}", employeeName);

    string department = wb.GetCell(0, row, 1).Value.ToString();
    paragraphs[1].Portions[0].Text = paragraphs[1].Portions[0].Text.Replace("{{Department}}", department);

    string yearsOfService = wb.GetCell(0, row, 2).Value.ToString();
    ...
    pres.Save($@"{employeeName} Report.pptx", SaveFormat.Pptx);
}
```
...
// Add a table shape to the first slideunits
...
for (int rowIndex = 0; rowIndex < 5; rowIndex++)
{
    ...
    {
        string cellValue = wb.GetCell(0, rowIndex, colIndex).Value.ToString();
        table[colIndex, rowIndex].TextFrame.Text = cellValue;
    }
}
