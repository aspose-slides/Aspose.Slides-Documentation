...
type: docs
weight: 30
url: /php-java/extracting-text-on-cloud-platforms-using-aspose-slides/
keywords:
- text extraction
...
$presentationText = PresentationFactory::getInstance()->getPresentationText($filePath, $arrangingMode);
$slidesText = $presentationText->getSlidesText();

foreach ($slidesText as $slideText) {
    echo "Slide text: " . $slideText->getText() . "\n";
...
```php
$filePath = "presentation.pptx";
$arrangingMode = TextExtractionArrangingMode::Arranged;

$presentationText = PresentationFactory::getInstance()->getPresentationText($filePath, $arrangingMode);
$slidesText = $presentationText->getSlidesText();

foreach ($slidesText as $slideText) {
    echo "Slide text (preserving order): " . $slideText->getText() . "\n";
}
```
