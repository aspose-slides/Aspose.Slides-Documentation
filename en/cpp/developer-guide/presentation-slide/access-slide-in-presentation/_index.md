...
You will also learn how to change a slide’s position by using the `set_SlideNumber` method and how to define the starting slide number for a presentation with the `set_FirstSlideNumber` method. The examples demonstrate loading a presentation, getting slide references, updating slide order or numbering, and saving the modified presentation.
