...
- C++
- Aspose.Slides
description: "了解如何在 C++ 中打开 PowerPoint 和 OpenDocument 演示文稿，提供打开密码，控制资源加载，并使用 Aspose.Slides for C++ 减少内存使用。"
---
## **介绍**

[Aspose.Slides for C++](https://products.aspose.com/slides/zh/cpp/) 可以从文件和流加载 PowerPoint 和 OpenDocument 演示文稿。加载演示文稿后，您可以检查其结构、编辑幻灯片、管理资源，并以原始格式或其他支持的格式保存。
...
加载文件或流后，您可以 [确定其原始演示文稿格式](/slides/zh/cpp/detect-presentation-source-format/) 以决定应用程序如何处理它。

要打开现有演示文稿，请将其文件路径传递给 [Presentation](https://reference.aspose.com/slides/zh/cpp/aspose.slides/presentation/) 构造函数。使用后释放 Presentation，以便及时释放文件句柄、临时数据和其他资源。

以下 C++ 示例演示了如何打开演示文稿并获取其幻灯片计数：
...
打开密码会对演示文稿内容进行加密。要加载完整的演示文稿，请将正确的密码传递给 [LoadOptions::set_Password](https://reference.aspose.com/slides/zh/cpp/aspose.slides/loadoptions/set_password/) 并将这些选项传递给 [Presentation](https://reference.aspose.com/slides/zh/cpp/aspose.slides/presentation/) 构造函数。如果密码缺失或不正确，加载将失败。
...
有关密码检测、验证和加密工作流，请参阅 [Password-Protect Presentations](/slides/zh/cpp/password-protected-presentation/)。如果加密的演示文稿被有意保存了公共文档属性，则可以在不提供密码的情况下读取这些属性；请参阅 [Manage Presentation Properties](/slides/zh/cpp/presentation-properties/)。
...
使用 `PresentationLockingBehavior::KeepLocked` 时，源文件将保持锁定状态，直至 `Presentation` 对象被释放。在该对象存活期间，请勿移动、覆盖或删除源文件。

Aspose.Slides 可能在加载时复制输入流的内容。对于大型演示文稿，文件路径通常比流更高效。有关其他存储和内存管理选项，请参阅 [Manage BLOBs](/slides/zh/cpp/manage-blob/)。
{{% /alert %}}

## **控制外部资源**

[LoadOptions::set_ResourceLoadingCallback](https://reference.aspose.com/slides/zh/cpp/aspose.slides/loadoptions/set_resourceloadingcallback/) 接受一个 [IResourceLoadingCallback](https://reference.aspose.com/slides/zh/cpp/aspose.slides/iresourceloadingcallback/) 实现。回调可以提供替换数据、重定向资源、使用默认加载器或跳过资源。当演示文稿包含必须根据特定应用安全或存储规则解析的外部图像时，这非常有用。
...
演示文稿可能包含嵌入的二进制数据，应用程序可能不需要或不想保留这些数据。例如：
...
在加载时，将 `true` 传递给 [LoadOptions::set_DeleteEmbeddedBinaryObjects](https://reference.aspose.com/slides/zh/cpp/aspose.slides/loadoptions/set_deleteembeddedbinaryobjects/) 以删除这些二进制数据。保存加载后的演示文稿以保留已清理的结果。

此选项可减少不需要的嵌入负载的暴露风险，但它并不是完整的恶意软件检测或内容清理系统。
...
Aspose.Slides 在加载期间会抛出解析或格式异常。应将此失败与密码错误的异常分开处理，以便应用程序能够准确报告原因。

**如果缺少必需的字体会怎样？**

演示文稿仍然可以加载，但渲染和导出时可能会使用替代字体。您可以 [配置字体替代](/slides/zh/cpp/font-substitution/) 或 [提供自定义字体](/slides/zh/cpp/custom-font/) 以使输出更可预期。

**加载演示文稿是否也会加载其嵌入的媒体？**

嵌入的音频和视频可通过演示文稿对象模型获取。外部资源根据配置的资源加载行为进行解析，如果无法访问其位置，则可能不可用。