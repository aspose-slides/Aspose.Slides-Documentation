...
PNG（便携式网络图形）格式虽然没有 JPEG（联合图像专家组）那样流行，但仍然非常受欢迎。 

**使用场景：** 当你拥有复杂图像且尺寸不是问题时，PNG 是比 JPEG 更好的图像格式。 

{{% alert title="Tip" color="primary" %}} 您可能想查看 Aspose 免费的 **PowerPoint to PNG Converters**：[PPTX to PNG](https://products.aspose.app/slides/conversion/pptx-to-png) 和 [PPT to PNG](https://products.aspose.app/slides/conversion/ppt-to-png)。它们是本页所述过程的实时实现。 {{% /alert %}}
...
下面的 C++ 代码演示了如何将 PowerPoint 演示文稿转换为 PNG：
```cpp
...
如果您希望获得特定大小的 PNG 文件，可以为 `ImageSize` 传入您偏好的 `width` 和 `height` 参数。 

下面的代码演示了在指定图像尺寸的情况下将 PowerPoint 转换为 PNG：
```cpp
auto pres = System::MakeObject<Presentation>(u"pres.pptx");
...
**如何只导出特定形状（例如图表或图片）而不是整张幻灯片？**

Aspose.Slides 支持 [为单个形状生成缩略图](/slides/zh/cpp/create-shape-thumbnails/)；您可以将形状渲染为 PNG 图像。

**服务器上是否支持并行转换？**

是的，但请 [不要共享](/slides/zh/cpp/multithreading/) 同一演示文稿实例跨线程。每个线程或进程使用单独的实例。

**导出为 PNG 时试用版有什么限制？**

评估模式会在输出图像上添加水印，并在授予许可证之前强制执行 [其他限制](/slides/zh/cpp/licensing/)。