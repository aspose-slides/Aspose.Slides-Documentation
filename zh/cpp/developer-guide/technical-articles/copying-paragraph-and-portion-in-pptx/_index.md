...
为了格式化演示文本，我们需要在**段落**和**部分**级别进行格式化。 
某些文本属性可以在段落级别设置，而某些则可以在部分级别设置。 
如果文本中有需要复制到新添加的段落或部分的段落或部分，我们需要将相应段落或部分的所有属性复制到新添加的段落或部分中。

{{% /alert %}} 

## **复制段落**
段落属性可以通过**段落**类的**ParagraphFormat**实例访问。 
我们需要将源段落的所有属性复制到目标段落。在下面的例子中，分享了**CopyParagraph**方法，该方法接受一个要复制的段落作为参数。它将源段落的所有属性复制到一个临时段落并返回相同的。目标段落获取复制的值。
...
## **复制部分**
部分属性可以通过**部分**类的**PortionFormat**实例访问。 
我们需要将源部分的所有属性复制到目标部分。在下面的例子中，分享了**CopyPortion**方法，该方法接受一个要复制的部分作为参数。它将源部分的所有属性复制到一个临时部分并返回相同的。目标部分获取复制的值。
