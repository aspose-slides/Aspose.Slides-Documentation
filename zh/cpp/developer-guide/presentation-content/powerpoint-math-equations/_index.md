...
在 PowerPoint 中，用户通常通过 **插入 > 方程式** 添加公式：

![PowerPoint 插入选项卡中选中的“方程式”命令](powerpoint-math-equations_1.png)
...
- 一个数学形状，由 [AddMathShape](https://reference.aspose.com/slides/zh/cpp/aspose.slides/shapecollection/) 创建，是包含公式的形状。
- [MathPortion](https://reference.aspose.com/slides/zh/cpp/aspose.slides.mathtext/mathportion/) 在形状文本框中存储数学内容。
...
下面的大多数示例使用 [MathematicalText](https://reference.aspose.com/slides/zh/cpp/aspose.slides.mathtext/mathematicaltext/) 和来自 [IMathElement](https://reference.aspose.com/slides/zh/cpp/aspose.slides.mathtext/imathelement/) 的流式方法，以保持代码简短且易读。

有关 MathML 导出场景，请参阅 [Export Math Equations from Presentations in C++](/slides/zh/cpp/exporting-math-equations/)。

## **创建公式**

此示例创建一个数学形状并添加勾股定理：

![c² = a² + b² 的公式](powerpoint-math-equations_3.png)
...
![倾斜的数学分数，显示 1 除以 x](powerpoint-math-equations_4.png)
...
![n 次根根式表达式，x 在根号下](powerpoint-math-equations_5.png)
...
使用 `Nary` 进行求和、并集、交集等大型运算符。使用 `Integral` 添加积分。两种方法都允许设置上下限。
...
N 元运算符用于带可选上下限的大型运算符。像 `+`、`-`、`=` 这样的简单运算符通常作为 `MathematicalText` 添加并连接到表达式中。
...
使用 [MathMatrix](https://reference.aspose.com/slides/zh/cpp/aspose.slides.mathtext/mathmatrix/) 定义行和列。默认情况下矩阵不包含括号，因此在需要圆括号、方括号或大括号时自行添加。

![一个两行的数学矩阵，包含一个空单元格](powerpoint-math-equations_10.png)
...
![垂直的数学数组，x 在 y 上方](powerpoint-math-equations_11.png)
...
使用下标和上标助手添加索引和幂。当索引必须出现在基数的左侧时，使用 `SetSubSuperscriptOnTheLeft`。

![大写 Y，左侧下标 1 和上标 n](powerpoint-math-equations_9.png)
...
使用 `Group` 在表达式上方或下方放置分组字符。添加上下限以标注分组的项。

![表达式 x + y，下面带有标签 any text](powerpoint-math-equations_15.png)
...
仅在能澄清公式时使用格式化助手。例如，`Overbar` 在数学元素上方放置一条横线。
...
| 组合元素 | [IMathElement.Join](https://reference.aspose.com/slides/zh/cpp/aspose.slides.mathtext/imathelement/join/) |
...
| 添加方程数组 | [ToMathArray](https://reference.aspose.com/slides/zh/cpp/aspose.slides.mathtext/imathelement/tomatharray/) |
...
是的。打开演示文稿，找到包含 `MathPortion` 的形状，获取其 `MathParagraph`，并更新该段落中的数学块。
