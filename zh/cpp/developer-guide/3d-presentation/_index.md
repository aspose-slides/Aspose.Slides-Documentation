...
- C++
- Aspose.Slides
description: "使用 Aspose.Slides 在 C++ 中轻松生成交互式 3D 演示文稿，可快速导出为 PowerPoint 和 OpenDocument 格式，灵活运用。"
---

## **概述**
自 Aspose.Slides 20.9 起，能够创建和修改 PowerPoint 3D 模型。这可以通过为 2D 形状添加一组 3D 效果来实现。通过在形状上创建相机视图，您可以沿轴旋转它。为形状创建拉伸或深度，将 2D 形状转换为 3D 模型。设置 3D 形状的光照效果或更改材质可以使其更具活力。将 3D 模型的颜色更改为 3D 渐变、修改形状轮廓、添加斜角可使 3D 模型更具体积感。所有 3D 效果均可应用于 PowerPoint 3D 模型和文本。

让我们观察创建 3D 模型的第一个示例，其中包括上述所有功能：
``` cpp
...
要旋转 PowerPoint 3D 模型，需要在形状上创建相机视图。这可以使用[IThreeDFormat.get_Camera()](https://reference.aspose.com/slides/cpp/class/aspose.slides.three_d_format#ad2f989bd1fd64fd4136e1f17660035d4)方法完成。旋转方法从相机类调用，就像您在旋转相机一样。事实上，当您相对于形状旋转相机时，实际上是在 3D 平面上旋转形状。
``` cpp
auto shape = slide->get_Shapes()->AddAutoShape(ShapeType::Rectangle, 200.0f, 150.0f, 200.0f, 200.0f);
shape->get_ThreeDFormat()->get_Camera()->SetRotation(20.0f, 30.0f, 40.0f);
// ... 设置其他 3D 场景 参数
...
## **3D 深度和拉伸**
要为 PowerPoint 3D 模型添加深度和拉伸，请使用[IThreeDFormat.set_ExtrusionHeight()](https://reference.aspose.com/slides/cpp/class/aspose.slides.three_d_format#adf0bad4894b1c36d9e4b044ef4978295)方法。  
要修改拉伸颜色，请使用[IThreeDFormat.get_ExtrusionColor()](https://reference.aspose.com/slides/cpp/class/aspose.slides.three_d_format#aa7db8859d23a9b4eb2f35f3a42025e9e)方法：
``` cpp
auto shape = slide->get_Shapes()->AddAutoShape(ShapeType::Rectangle, 200.0f, 150.0f, 200.0f, 200.0f);
shape->get_ThreeDFormat()->get_Camera()->SetRotation(20.0f, 30.0f, 40.0f);
...
## **3D 渐变**
在 PowerPoint 3D 模型上绘制 3D 渐变可以通过[Shape.get_FillFormat().get_GradientFormat()](https://reference.aspose.com/slides/cpp/class/aspose.slides.fill_format#a1f075336cb7a0e05cd5d7a706b6f4f58)方法完成：
``` cpp
using namespace Aspose::Slides;
...
是的。Slides 提供了用于[读取有效值](/slides/zh/cpp/shape-effective-properties/)的 API（包括 3D‑灯光、斜角等），以便查看最终应用的设置。

**在将演示文稿转换为视频时，3D 效果会工作吗？**

是的。当[为视频生成帧](/slides/zh/cpp/convert-powerpoint-to-video/)时，3D 效果的渲染方式与[导出的图像](/slides/zh/cpp/convert-powerpoint-to-png/)相同。