...
Aspose.Slides 支持 Microsoft PowerPoint 97 到 Office 365 的演示文稿文件，包括 Microsoft PowerPoint for Mac。本文列出了库支持的 PowerPoint 版本，并提供了可以加载、保存或两者兼备的文件格式表。

本文还回答了关于 PDF 合规性、字体嵌入、受密码保护的文件、自定义字体、字体回退以及 XPS 导出选项的常见问题。
...
## **支持的文件格式**
此表包含 Aspose.Slides for C++ 能够加载和保存的文件格式：
...
可以。Aspose.Slides 支持通过 [PDF 导出选项](https://reference.aspose.com/slides/zh/cpp/aspose.slides.export/pdfoptions/) 中的 [compliance](https://reference.aspose.com/slides/zh/cpp/aspose.slides.export/pdfoptions/set_compliance/) 设置导出符合 PDF/A-2a、PDF/A-2b、PDF/A-2u、PDF/A-3a、PDF/A-3b 以及 PDF/UA 等合规级别的 PDF。

**库在导出为 PDF 时是否支持字体嵌入，并且可以细粒度控制嵌入内容？**

可以。您可以控制是完整嵌入字体还是子集嵌入（仅使用的字形），指定常见系统字体的处理方式，并通过 [PDF 导出选项](https://reference.aspose.com/slides/zh/cpp/aspose.slides.export/pdfoptions/) 配置 ASCII 文本的行为。

**我能在实际加载文件之前检测文件是否受密码保护吗？**

可以。使用 [基于工厂的检查 API](https://reference.aspose.com/slides/zh/cpp/aspose.slides/presentationfactory/) ，您可以查询演示文稿文件以确定其是否受密码保护，而无需完整打开。

**是否提供字体回退机制并支持自定义字体？**

可以。库支持 [加载](/slides/zh/cpp/custom-font/) 和 [嵌入](/slides/zh/cpp/embedded-font/) 自定义字体，并提供字体 [回退规则](/slides/zh/cpp/fallback-font/) 以防止在渲染和转换过程中出现缺失字形。

**我可以将幻灯片导出为 XPS 吗？是否有调优 XPS 输出的选项？**

可以。支持 [导出为 XPS](/slides/zh/cpp/convert-powerpoint-to-xps/)，并且您可以调整相关的 [保存选项](https://reference.aspose.com/slides/zh/cpp/aspose.slides.export/xpsoptions/) 以控制 XPS 文档的输出质量和内容。