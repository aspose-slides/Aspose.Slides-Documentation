---
title: Aspose.Slides for Java 15.10.0 的公共 API 及向后不兼容的更改
...
---
{{% alert color="info" %}} 
此页面列出所有 [added](/slides/zh/java/public-api-and-backwards-incompatible-changes-in-aspose-slides-for-java-15-10-0/) 或 [removed](/slides/zh/java/public-api-and-backwards-incompatible-changes-in-aspose-slides-for-java-15-10-0/) 的类、方法、属性等，以及 Aspose.Slides for Java 15.10.0 API 引入的其他更改。
{{% /alert %}} 
## **公共 API 更改**
#### **已向 ISequence 添加图表系列动画 API**
已向 com.aspose.slides.ISequence 接口添加了 2 个新方法。
...
要向图表添加系列动画，可以使用以下代码。示例文件中的图表有三个系列，因此为每个索引 0 到 2 各添加一个效果：
...
```
#### **已新增 com.aspose.slides.VideoPlayerHtmlController，以支持将媒体文件导出为 HTML**
已添加新的公共类 com.aspose.slides.VideoPlayerHtmlController。使用该类的实例，用户可以将视频和音频文件导出为 HTML。

VideoPlayerHtmlController 构造函数接受以下参数：

- path: 将生成视频和音频文件的路径（文件夹必须已存在）
- fileName: HTML 文件的名称
