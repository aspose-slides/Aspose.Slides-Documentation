...
- Java
- Aspose.Slides
description: "在 Aspose.Slides for Java 中精通演示主题，以创建、定制并转换具有一致品牌形象的 PowerPoint 文件。"
---
演示主题定义了设计元素的属性。当您选择演示主题时，实际上是选择了一套特定的视觉元素及其属性。
...
PowerPoint 主题为幻灯片上的不同元素使用一套特定的颜色。如果您不喜欢这些颜色，可以通过为主题应用新颜色来更改它们。为了让您选择新的主题颜色，Aspose.Slides 在 [SchemeColor](https://reference.aspose.com/slides/zh/java/com.aspose.slides/SchemeColor) 枚举中提供了相应的值。

下面的 Java 代码演示了如何更改主题的强调色：
...
当您对主主题颜色（1）进行亮度变换时，会生成来自附加调色板（2）的颜色。然后您可以设置和获取这些主题颜色。
...
这种差异仅在于命名。这些值对应相同的主题颜色槽位，映射是固定的：
...
`Text`/`Background` 与 `Dark`/`Light` 之间没有动态转换。它们仅是同一主题颜色的替代名称。

这种命名差异来源于 Microsoft Office 的术语。较早的 Office 版本使用 `Dark 1`、`Light 1`、`Dark 2` 和 `Light 2`，而较新的 UI 版本则将相同的槽位显示为 `Text 1`、`Background 1`、`Text 2` 和 `Background 2`。

## **更改主题字体**

Aspose.Slides 使用以下特殊标识符（类似于 PowerPoint 中使用的）来让您为主题及其他目的选择字体：
...
默认情况下，PowerPoint 应用提供 12 种预定义背景，但在普通演示文稿中仅保存其中的 3 种背景。
...
PowerPoint 主题通常为每个样式数组包含 3 个值。这些数组组合成 3 种效果：细致、适中和强烈。例如，当对特定形状应用这些效果时的结果如下：
...
下面的 Java 代码演示了如何通过更改元素的部分属性来更改主题效果：
...
是的。Aspose.Slides 支持幻灯片级别的主题覆盖，您可以仅在该幻灯片上应用局部主题，同时保持母版主题不变（通过 [SlideThemeManager](https://reference.aspose.com/slides/zh/java/com.aspose.slides/slidethememanager/)）。

**将主题从一个演示文稿迁移到另一个演示文稿的最安全方式是什么？**

[Clone slides](/slides/zh/java/clone-slides/) 与其母版一起复制到目标演示文稿。这会保留原始的母版、布局以及关联的主题，从而保持外观一致。

**如何查看所有继承和覆盖后的“实际”值？**

使用 API 的 ["effective" 视图](/slides/zh/java/shape-effective-properties/) 来查看主题、颜色、字体、效果等的实际值。这些视图在应用母版以及任何局部覆盖后返回解析后的最终属性。