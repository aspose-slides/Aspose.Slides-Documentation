...
当在 Linux 上使用 Aspose.Slides 时，如果 Java 进程无法访问所需的字体文件夹或临时目录，系统未安装任何字体，或者缺少诸如 fontconfig 或 libfreetype 等必需的系统库，可能会出现与字体相关的问题。

本文描述了 Linux 上与字体相关的常见错误和异常，并提供了解决方案。它解释了如何检查对字体和 TEMP 目录的访问权限，安装所需的字体和库，以及使用 `FontsLoader` 在不全局安装字体的情况下加载字体。
...
{{% alert color="warning" %}}
在某些情况下，由于环境或安全策略的限制，您可能无法授予文件夹访问权限。请尝试以下变通方法：
{{% /alert %}}

**变通方法**

使用 [FontsLoader](https://reference.aspose.com/slides/zh/java/com.aspose.slides/FontsLoader) 加载所需字体而无需安装它们：

```
FontsLoader.loadExternalFonts(pathToFontsFolders);
```

如果无法访问 TEMP 目录，请使用以下代码为 Java 指定其他目录作为 TEMP：
...
## **异常：InvalidOperationException：在系统上找不到已安装的任何字体**
