...
- Java
- Aspose.Slides
description: "在 Android 上通过 Java 使用 Aspose.Slides 将 PowerPoint 和 OpenDocument 演示文稿导出为 HTML，图像和其他资源保存为外部链接文件。"
---
## **概述**

默认情况下，Aspose.Slides 将演示文稿导出为一个独立的 HTML 文件。图像和其他资源直接写入 HTML，通常以 Base64 数据的形式。这样在需要单一可移植文件时很方便，但它并不总是适合网页显示、CMS 或随后发布输出的服务器端转换管道的最佳格式。
...
有关通用的 HTML 转换工作流，请参阅[Convert PowerPoint Presentations to HTML](/slides/zh/androidjava/convert-powerpoint-to-html/)。本文重点介绍导出过程中的资源链接部分。

## **链接资源导出工作原理**

[ILinkEmbedController](https://reference.aspose.com/slides/zh/androidjava/com.aspose.slides/ilinkembedcontroller/) 让您的应用程序可以逐个资源决定导出器是将数据嵌入 HTML 还是外部保存并写入链接。

该接口有三个方法：

- [ILinkEmbedController.getObjectStoringLocation](https://reference.aspose.com/slides/zh/androidjava/com.aspose.slides/ilinkembedcontroller/) 决定资源是应链接还是嵌入。
- [ILinkEmbedController.getUrl](https://reference.aspose.com/slides/zh/androidjava/com.aspose.slides/ilinkembedcontroller/) 返回将写入生成的 HTML 或另一个链接资源的 URL。
- [ILinkEmbedController.saveExternal](https://reference.aspose.com/slides/zh/androidjava/com.aspose.slides/ilinkembedcontroller/) 将链接资源数据写入磁盘或其他存储目标。

文件系统路径和浏览器 URL 是相互独立的。例如，下面的示例将资源文件写入应用程序文件存储中的 `html-output/assets`，而 HTML 中包含相对 URL，例如 `assets/resource-1.svg`。浏览器会相对于包含链接的文件解析这些 URL。因此，`presentation.html` 到 SVG 文件的链接使用 `assets/resource-1.svg`，而该 SVG 文件到同一 `assets` 文件夹中保存的图像的链接使用 `resource-4.jpg`。

## **导出带链接资源的 HTML**

以下 Android Java 示例创建一个输出目录，将 HTML 文件保存到该目录，并将链接资源存放在 `assets` 子目录中。将 `context.getFilesDir()` 等应用拥有的目录作为 `applicationFilesDirectory` 传入。代码避免使用 `java.nio.file` API，因而兼容 Android `minSdk` 19。

控制器在 Aspose.Slides 提供或能够推断安全文件扩展名时，会链接常见的图像、字体、音频、视频和 CSS 资源。未被识别的资源仍会嵌入。
...
具体文件取决于演示文稿的内容和导出选项。例如，光栅图像通常导出为 JPEG 或 PNG。当使用不同的图像编解码器能够生成更小或更合适的文件时，Aspose.Slides 可能会选择与源演示文稿不同的编解码器。带透明度的图像会导出为 PNG。

## **部署时的 URL 选择**

示例使用相对 URL 前缀：`assets/`。如果从 `html-output/presentation.html` 打开 `presentation.html`，浏览器会加载 `html-output/assets/resource-1.svg`。

当一个链接资源引用另一个链接资源时，示例在[ILinkEmbedController.getUrl](https://reference.aspose.com/slides/zh/androidjava/com.aspose.slides/ilinkembedcontroller/) 中使用 `referrer` 参数，并仅返回文件名。例如，如果 `resource-1.svg` 和 `resource-4.jpg` 都位于 `assets` 文件夹中，SVG 文件应引用 `resource-4.jpg`，而不是 `assets/resource-4.jpg`。

如果文件部署到其他位置，请使用不同的 URL 前缀：

- 当资源目录与 HTML 文件相邻时使用 `assets/`。
- 当资源目录在 HTML 文件上一级时使用 `../assets/`。
- 当文件上传到 CDN 或静态文件服务器时使用 `https://cdn.example.com/presentations/job-123/assets/`。

[ILinkEmbedController.getUrl](https://reference.aspose.com/slides/zh/androidjava/com.aspose.slides/ilinkembedcontroller/) 返回的 URL 必须与 [ILinkEmbedController.saveExternal](https://reference.aspose.com/slides/zh/androidjava/com.aspose.slides/ilinkembedcontroller/) 写入的文件的最终部署位置相匹配。在 Android 应用中，根据发布工作流使用应用专属存储、缓存目录或通过存储访问框架获得的目录。在服务器应用中，为每个转换任务使用唯一的输出目录或对象存储前缀，以避免覆盖其他导出的文件。

## **何时采用嵌入方式**

当输出必须是单个文件时（例如电子邮件附件、离线预览或需要在没有支持资源文件夹的情况下移动的文档），嵌入的 Base64 HTML 仍然有用。HTML 将由 Web 应用提供、存储在 CMS 中、经过构建管道优化或由浏览器独立缓存时，使用链接资源更为合适。

## **常见问题**

**我可以只对图像进行外部化，而保持其他资源嵌入吗？**

可以。在 [ILinkEmbedController.getObjectStoringLocation](https://reference.aspose.com/slides/zh/androidjava/com.aspose.slides/ilinkembedcontroller/) 中，仅对想要保存为单独文件的内容类型返回来自 [LinkEmbedDecision](https://reference.aspose.com/slides/zh/androidjava/com.aspose.slides/linkembeddecision/) 的 `Link`，其余返回 `Embed`。

**为什么导出图像的扩展名与源演示文稿不同？**

Aspose.Slides 可能在 HTML 导出过程中重新编码光栅图像，以提升大小或浏览器兼容性。例如，源文件中的图像可能会根据渲染结果写成 JPEG 或 PNG。

**移动 HTML 文件后相对 URL 还能工作吗？**

相对 URL 仅在保持相同的相对文件夹结构时才有效。如果 HTML 引用 `assets/resource-1.png`，则 `assets` 文件夹必须与 HTML 文件同级，除非您生成了不同的 URL 前缀。

**我可以在 Android 上将资源写入公共外部存储吗？**

可以，只要您的应用拥有针对目标 Android 版本的有效目的地和权限模型。对于仅供您的应用使用的生成 HTML，应用专属文件或缓存目录通常更简便。对于面向用户的输出，请使用用户选择的位置或其他适合您应用的存储方式。

**服务器应用应该复用同一输出文件夹吗？**

不应该。为每个转换任务使用唯一的输出目录或存储前缀。这可以避免文件名冲突，并防止一次导出覆盖另一项导出生成的资源。