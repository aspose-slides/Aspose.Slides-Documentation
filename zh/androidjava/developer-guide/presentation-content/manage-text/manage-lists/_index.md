...
Aspose.Slides for Android via Java 让您在 PowerPoint 和 OpenDocument 演示文稿中创建和格式化项目符号列表和编号列表。列表项是段落，其项目符号设置通过段落格式进行控制。
...
使用[IParagraphFormat.setDepth](https://reference.aspose.com/slides/zh/androidjava/com.aspose.slides/iparagraphformat/#setDepth-short-) 将列表项放在不同层级。层级 0 为顶层，层级 1 为其下的嵌套层，以此类推。

下面的 Java 代码展示了如何创建多级列表：
...
下面的 Java 代码将文本框中的第一段落更改为使用编号列表样式：
...
### 可以将项目符号和编号列表导出为 PDF 或图像吗？

可以。Aspose.Slides 在目标格式支持相应的文本布局和项目符号功能时，会保留列表格式。
...
可以。列表项文本可以包含 Unicode 字符，您可以在多语言演示文稿中创建列表。请确保演示文稿使用的字体支持所需字符。