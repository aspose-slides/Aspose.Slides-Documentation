...
## **为环形图设置标注**
Aspose.Slides for Android via Java 提供了为环形图设置系列数据标签标注形状的支持。以下示例代码给出。
```java
Presentation pres = new Presentation();
try {
...
是的。标注是图表渲染的一部分，因此在导出为 [PDF](/slides/zh/androidjava/convert-powerpoint-to-pdf/)、[HTML5](/slides/zh/androidjava/export-to-html5/)、[SVG](/slides/zh/androidjava/render-a-slide-as-an-svg-image/) 或 [raster images](/slides/zh/androidjava/convert-powerpoint-to-png/) 时，它们会与幻灯片的格式一起被保留。

**自定义字体在标注中可用吗？导出时能保留其外观吗？**

是的。Aspose.Slides 支持将[embedding fonts](/slides/zh/androidjava/embedded-font/) 嵌入到演示文稿中，并在导出为[PDF](/slides/zh/androidjava/convert-powerpoint-to-pdf/)等格式时控制字体嵌入，确保标注在不同系统上保持相同的外观。