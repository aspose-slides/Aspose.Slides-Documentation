...
[Aspose.Slides](/slides/zh/python-net/) for .NET 允许您通过涉及 BLOB 的流程添加大文件（本例中为大型视频文件），以降低内存消耗。
...
使用 [**ImageCollection**](https://reference.aspose.com/slides/zh/python-net/aspose.slides/imagecollection/) 类的方法，您可以将大型图像作为流添加，以使其被视为 BLOB。
...
当使用 `temp_files_root_path` 时，Aspose.Slides 不会自动创建用于存放临时文件的文件夹。您必须手动创建该文件夹。
{{% /alert %}}
...
使用 [LoadOptions](https://reference.aspose.com/slides/zh/python-net/aspose.slides/loadoptions/) 搭配 [BlobManagementOptions](https://reference.aspose.com/slides/zh/python-net/aspose.slides/blobmanagementoptions/)。在此可以设置 BLOB 的内存限制，是否允许临时文件，临时文件的根路径，以及源锁定行为。

**BLOB 设置会影响性能吗？我该如何在速度与内存之间取得平衡？**

是的。将 BLOB 保持在内存中可最大化速度，但会增加 RAM 消耗；降低内存限制会将更多工作转移到临时文件，从而降低 RAM 占用，但会产生额外的 I/O 开销。通过调节 [max_blobs_bytes_in_memory](https://reference.aspose.com/slides/zh/python-net/aspose.slides/blobmanagementoptions/max_blobs_bytes_in_memory/) 阈值，以在您的工作负载和环境中达到合适的平衡。

**在打开极大型演示文稿（例如数 GB）时，BLOB 选项有帮助吗？**

是的。[BlobManagementOptions](https://reference.aspose.com/slides/zh/python-net/aspose.slides/blobmanagementoptions/) 专为此类场景设计：启用临时文件并使用源锁定可以显著降低峰值 RAM 使用，并在处理极大文稿时保持稳定。
