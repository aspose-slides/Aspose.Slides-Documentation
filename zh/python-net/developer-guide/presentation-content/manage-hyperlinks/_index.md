...
type: docs
weight: 20
url: /zh/python-net/developer-guide/presentation-content/manage-hyperlinks/
keywords:
- 添加 URL
- 添加 超链接
- 创建 超链接
...
- 演示文稿
- Python
description: "通过 Aspose.Slides for Python via .NET 轻松管理 PowerPoint 和 OpenDocument 演示文稿中的超链接——在几分钟内提升交互性和工作流程。"
---

## **概述**

超链接是指向外部资源、对象或数据项，或文件内部特定位置的引用。PowerPoint 演示文稿中常见的超链接类型包括：

* 嵌入文本、形状或媒体中的网站链接
* 链接到幻灯片

Aspose.Slides for Python via .NET 提供了在演示文稿中进行各种超链接相关操作的广泛功能。

## **添加 URL 超链接**

本节说明在使用 Aspose.Slides 时如何向幻灯片元素添加 URL 超链接。内容涵盖将链接地址分配给文本、形状和图片，以确保演示期间的流畅导航。
...
使用 [Hyperlink](https://reference.aspose.com/slides/python-net/aspose.slides/hyperlink/) 类的 [color_source](https://reference.aspose.com/slides/python-net/aspose.slides/hyperlink/color_source/) 属性，您可以设置超链接的颜色并读取其颜色信息。此功能在 PowerPoint 2019 中引入，因此通过此属性进行的更改不适用于更早版本的 PowerPoint。
...
本节解释在使用 Aspose.Slides 时如何从演示文稿中移除超链接。您将学习在保留原始内容和格式的同时，如何清除文本、形状和图片中的链接目标。

### **从文本中移除超链接**

以下示例代码展示了如何从演示文稿幻灯片的文本中移除超链接：
...
**如何创建不仅指向幻灯片，还指向“节”或该节的第一张幻灯片的内部导航？**

PowerPoint 中的节是幻灯片的分组；导航在技术上仍然针对特定幻灯片。要“导航到节”，通常链接到该节的第一张幻灯片。

**我可以将超链接附加到母版幻灯片元素，以便在所有幻灯片上生效吗？**

可以。母版幻灯片和布局元素支持超链接。这些链接会出现在子幻灯片上，并在放映期间可点击。

**导出为 PDF、HTML、图片或视频时，超链接会被保留吗？**

在 [PDF](/slides/zh/python-net/convert-powerpoint-to-pdf/) 和 [HTML](/slides/zh/python-net/convert-powerpoint-to-html/) 中会保留——链接通常会被保持。导出为 [图片](/slides/zh/python-net/convert-powerpoint-to-png/) 和 [视频](/slides/zh/python-net/convert-powerpoint-to-video/) 时，由于这些格式的本质（光栅帧/视频不支持超链接），点击功能不会保留。