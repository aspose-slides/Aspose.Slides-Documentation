...
- 演示文稿
- Python
description: "借助 Aspose.Slides for Python via .NET，轻松管理 PowerPoint 和 OpenDocument 演示文稿中的超链接——几分钟内提升交互性和工作流。"
---

## **概览**

超链接是指向外部资源、对象或数据项，或文件中某个特定位置的引用。PowerPoint 演示文稿中常见的超链接类型包括：
...
本节说明在使用 Aspose.Slides 时，如何向幻灯片元素添加 URL 超链接。内容涵盖将链接地址分配给文本、形状和图片，以确保演示过程中顺畅的导航。

### **向文本添加 URL 超链接**

以下代码示例演示如何向文本添加网站超链接：
...
Aspose.Slides 允许为图像、音频和视频文件添加超链接。

以下代码示例演示如何为 **图像** 添加超链接：
...
您可能还想查看 [使用 Python 管理演示文稿中的 OLE](/slides/zh/python-net/manage-ole/)。
{{% /alert %}}

## **使用超链接创建目录**

因为超链接可以引用对象或位置，您可以使用它们来构建目录。
...
本节展示如何在 Aspose.Slides 中格式化超链接的外观。您将学习控制颜色和其他样式选项，以在文本、形状和图片之间保持超链接格式的一致性。

### **超链接颜色**

使用 [Hyperlink](https://reference.aspose.com/slides/python-net/aspose.slides/hyperlink/) 类的 [color_source](https://reference.aspose.com/slides/python-net/aspose.slides/hyperlink/color_source/) 属性，您可以设置超链接的颜色并读取其颜色信息。此功能在 PowerPoint 2019 中引入，因而通过该属性进行的更改不适用于更早版本的 PowerPoint。

下面的示例演示如何在同一幻灯片上添加不同颜色的超链接：
...
本节解释在使用 Aspose.Slides 时，如何从演示文稿中移除超链接。您将学习在保持原始内容和格式的同时，清除文本、形状和图片中的链接目标。

### **从文本中移除超链接**

下面的示例代码展示如何从演示文稿幻灯片上的文本中移除超链接：
...
下面的代码片段演示如何向幻灯片添加超链接，然后编辑其提示文本：
...
{{% alert color="primary" %}}
您可能想查看 Aspose 免费的在线 [PowerPoint 编辑器](https://products.aspose.app/slides/editor)。
{{% /alert %}}

## **常见问题**

**如何创建不仅指向幻灯片，还指向“章节”或该章节首张幻灯片的内部导航？**

PowerPoint 中的章节是幻灯片的分组；导航技术上仍然指向具体的幻灯片。若要“跳转到章节”，通常链接到该章节的第一张幻灯片。

**我可以将超链接附加到母版幻灯片元素上，以便在所有幻灯片上都有效吗？**

可以。母版幻灯片和布局元素支持超链接。此类链接会出现在子幻灯片上，并在放映时可点击。

**在导出为 PDF、HTML、图像或视频时，超链接会被保留吗？**

在 [PDF](/slides/zh/python-net/convert-powerpoint-to-pdf/) 和 [HTML](/slides/zh/python-net/convert-powerpoint-to-html/) 中会保留——链接通常会被保留。导出为 [图像](/slides/zh/python-net/convert-powerpoint-to-png/) 和 [视频](/slides/zh/python-net/convert-powerpoint-to-video/) 时，由于这些格式的本质（光栅帧/视频不支持超链接），点击功能将不会保留。