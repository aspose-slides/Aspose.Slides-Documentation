...
- 演示文稿
- Python
description: 轻松地使用 Aspose.Slides for Python via .NET 在 PowerPoint 和 OpenDocument 演示文稿中管理超链接——在几分钟内提升交互性和工作流。
---

## **概览**

超链接是对外部资源、对象或数据项，或文件内部特定位置的引用。PowerPoint 演示文稿中常见的超链接类型包括：
...
本节说明在使用 Aspose.Slides 时如何向幻灯片元素添加 URL 超链接。它涵盖了将链接地址分配给文本、形状和图片，以确保演示期间的顺畅导航。
...
您可能想查看[使用 Python 在演示文稿中管理 OLE](/slides/zh/python-net/manage-ole/)。
{{% /alert %}}
...
使用 [Hyperlink](https://reference.aspose.com/slides/python-net/aspose.slides/hyperlink/) 类的 [color_source](https://reference.aspose.com/slides/python-net/aspose.slides/hyperlink/color_source/) 属性，您可以设置超链接的颜色并读取其颜色信息。此功能在 PowerPoint 2019 中首次引入，因此通过此属性进行的更改不适用于早期版本的 PowerPoint。
...
本节说明在使用 Aspose.Slides 时如何从演示文稿中移除超链接。您将学习如何在保留原始内容和格式的前提下，清除文本、形状和图片的链接目标。
...
以下代码片段展示了如何向幻灯片添加超链接，然后编辑其工具提示：
...
{{% alert color="primary" %}}
您可能想尝试 Aspose 简单、免费且在线的 PowerPoint 编辑器。
{{% /alert %}}

## **常见问题**

**如何创建不仅指向幻灯片而且指向“章节”或章节首张幻灯片的内部导航？**

PowerPoint 中的章节是幻灯片的分组；导航技术上定位到特定幻灯片。要“导航到章节”，通常链接到该章节的第一张幻灯片。
...
在 [PDF](/slides/zh/python-net/convert-powerpoint-to-pdf/) 和 [HTML](/slides/zh/python-net/convert-powerpoint-to-html/) 中会保留链接。导出为 [图像](/slides/zh/python-net/convert-powerpoint-to-png/) 和 [视频](/slides/zh/python-net/convert-powerpoint-to-video/) 时，由于这些格式本质上是光栅帧/视频，不支持超链接，点击功能将不被保留。