...
- Python
- Aspose.Slides
description: "使用 Aspose.Slides for Python via .NET 在 PowerPoint 和 OpenDocument 幻灯片中创建和编辑表格。了解简易代码示例，简化表格工作流。"
---

## **概述**

PowerPoint 中的表格是一种高效的信息呈现方式。信息以网格（行和列）的形式排列，直观且易于理解。
...
本节展示如何在 Aspose.Slides 中通过向幻灯片添加表格形状、定义行列以及设置精确尺寸来从头创建表格。您还将看到如何向单元格填充文本、调整对齐和边框，以及自定义表格外观。
...
6. 遍历每个 [Cell](https://reference.aspose.com/slides/python-net/aspose.slides/cell/)，格式化其上、下、左、右边框。  
7. 合并表格第一行的前两个单元格。  
...
在标准表格中，单元格编号直观且基于零索引。表格中的第一个单元格索引为 (0, 0)（列 0，行 0）。

例如，拥有 4 列 4 行的表格，单元格编号如下：
...
本节说明如何在演示文稿中定位并操作已有表格。您将学习如何在幻灯片上查找表格、访问其行、列和单元格，以及更新内容或格式。

1. 创建一个 [Presentation](https://reference.aspose.com/slides/python-net/aspose.slides/presentation/) 类的实例。  
2. 根据索引获取包含该表格的幻灯片引用。  
3. 遍历所有 [Shape](https://reference.aspose.com/slides/python-net/aspose.slides/shape/) 对象，直到找到表格。  
4. 使用 [Table](https://reference.aspose.com/slides/python-net/aspose.slides/table/) 对象操作表格。  
5. 保存修改后的演示文稿。
...
本节展示如何使用 Aspose.Slides 控制表格单元格内部的文本对齐方式。您将学习为单元格设置水平和垂直对齐，以保持内容清晰一致。
...
以下 Python 示例演示如何对表格中的文本应用首选的格式化选项：
...
Aspose.Slides 让您通过代码使用预定义样式格式化表格。下面的示例演示创建表格、应用内置样式并保存结果——这是一种高效确保一致、专业格式的方式。
...
形状的长宽比是其尺寸的比例。Aspose.Slides 提供 `aspect_ratio_locked` 属性，允许您锁定表格和其他形状的长宽比。

以下 Python 示例演示如何锁定表格的长宽比：
...
可以。表格公开了 [right_to_left](https://reference.aspose.com/slides/python-net/aspose.slides/table/right_to_left/) 属性，段落拥有 [ParagraphFormat.right_to_left](https://reference.aspose.com/slides/python-net/aspose.slides/paragraphformat/right_to_left/)。同时使用可确保单元格内部正确的 RTL 顺序和渲染。
