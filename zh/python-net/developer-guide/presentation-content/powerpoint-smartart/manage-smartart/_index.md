...
type: docs
weight: 10
url: /zh/python-net/developer-guide/presentation-content/powerpoint-smartart/manage-smartart/
keywords:
- SmartArt
- SmartArt 文本
- 布局类型
...
- Python
- Aspose.Slides
description: "学习使用 Aspose.Slides for Python via .NET 构建和编辑 PowerPoint SmartArt，提供清晰的代码示例，加速幻灯片设计和自动化。"
---

## **概述**

本指南展示了如何在 Aspose.Slides for Python 中创建和操作 SmartArt。您将学习如何从 SmartArt 中提取文本（包括节点形状内部的 [TextFrame](https://reference.aspose.com/slides/python-net/aspose.slides/textframe/) 内容），向幻灯片添加 SmartArt 并切换其布局，检测并处理隐藏节点，配置组织结构图布局，以及创建图片组织结构图——所有示例都简洁、可直接复制粘贴的 Python 代码，能够打开一个 [Presentation](https://reference.aspose.com/slides/python-net/aspose.slides/presentation/)，对幻灯片和 SmartArt 节点进行操作，并将结果保存为 PPTX。

## **从 SmartArt 获取文本**

`text_frame` 属性 of the [SmartArtShape](https://reference.aspose.com/slides/python-net/aspose.slides.smartart/smartartshape/) 允许您检索 SmartArt 形状中的所有文本——不仅仅是节点中包含的文本。以下示例代码展示了如何从 SmartArt 节点获取文本。
...
`SmartArtNode.is_hidden` 属性如果节点在数据模型中被隐藏则返回 `True`。要检查 SmartArt 节点是否隐藏，请按以下步骤操作：
...
是的。`[is_reversed](https://reference.aspose.com/slides/python-net/aspose.slides.smartart/smartart/is_reversed/)` 属性如果所选的 SmartArt 类型支持反转，则切换图表方向（LTR/RTL）。

**如何在保留格式的情况下将 SmartArt 复制到同一幻灯片或其他演示文稿？**

您可以通过形状集合中的 [克隆 SmartArt 形状](/slides/zh/python-net/shape-manipulations/)（[ShapeCollection.add_clone](https://reference.aspose.com/slides/python-net/aspose.slides/shapecollection/add_clone/)）或 [克隆包含该形状的整个幻灯片](/slides/zh/python-net/clone-slides/) 来实现。两种方式都能保留大小、位置和样式。

**如何将 SmartArt 渲染为栅格图像以进行预览或网页导出？**

您可以通过 API 将幻灯片（或整个演示文稿）转换为 PNG/JPEG（[渲染幻灯片](/slides/zh/python-net/convert-powerpoint-to-png/)），SmartArt 将作为幻灯片的一部分绘制。

**如果幻灯片上有多个 SmartArt，如何以编程方式选择特定的 SmartArt？**

常用做法是使用[替代文本]([alternative text](https://reference.aspose.com/slides/python-net/aspose.slides.smartart/smartart/alternative_text/))或[名称]([name](https://reference.aspose.com/slides/python-net/aspose.slides.smartart/smartart/name/))，并在 [Slide.shapes](https://reference.aspose.com/slides/python-net/aspose.slides/slide/shapes/) 中按该属性搜索形状，然后检查其类型以确认是 [SmartArt](https://reference.aspose.com/slides/python-net/aspose.slides.smartart/smartart/)。文档中描述了查找和操作形状的典型技术。