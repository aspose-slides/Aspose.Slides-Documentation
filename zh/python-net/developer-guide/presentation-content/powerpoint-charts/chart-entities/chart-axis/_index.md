...
linktitle: 图表坐标轴
type: docs
url: /zh/python-net/developer-guide/presentation-content/powerpoint-charts/chart-entities/chart-axis/
keywords:
- 图表坐标轴
- 垂直坐标轴
- 水平坐标轴
...
## **交换坐标轴之间的数据**
Aspose.Slides 允许您快速交换坐标轴之间的数据——垂直坐标轴（y 轴）上的数据将移动到水平坐标轴（x 轴），反之亦然。

下面的 Python 代码演示了如何在图表上执行坐标轴数据交换：
...
下面的 Python 代码演示了如何在折线图中隐藏垂直坐标轴：
...
使用 **CategoryAxisType** 属性，您可以指定首选的类目坐标轴类型（**date** 或 **text**）。下面的 Python 代码演示了该操作：
...
## **为类目坐标轴的值设置日期格式**
Aspose.Slides for Python via .NET 允许您为类目坐标轴的值设置日期格式。下面的 Python 代码演示了该操作：
...
## **在类目或数值坐标轴中设置位置坐标轴**
Aspose.Slides for Python via .NET 允许您在类目或数值坐标轴中设置位置坐标轴。下面的 Python 代码展示了如何执行此任务：
...
## **在图表数值坐标轴上启用显示单位标签**
Aspose.Slides for Python via .NET 允许您配置图表以在其数值坐标轴上显示单位标签。下面的 Python 代码演示了该操作：
...
**如何设置两个坐标轴相交的数值（坐标轴交叉点）？**

坐标轴提供了一个 [交叉设置](https://reference.aspose.com/slides/python-net/aspose.slides.charts/axis/cross_type/)：您可以选择在零点、在最大类目/数值处，或在特定数值处相交。此功能有助于将 X 轴上下移动或强调基准线。

**如何相对于坐标轴定位刻度标签（旁边、外部、内部）？**

设置 [标签位置](https://reference.aspose.com/slides/python-net/aspose.slides.charts/axis/major_tick_mark/) 为 “cross”、 “outside” 或 “inside”。这会影响可读性，并帮助在小型图表上节省空间。