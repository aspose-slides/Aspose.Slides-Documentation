...
- Python
- Aspose.Slides
description: "了解如何使用 Aspose.Slides for Python via .NET 在 PowerPoint 和 OpenDocument 演示文稿中添加和格式化图表数据标签，以打造更具吸引力的幻灯片。"
---

## **概述**

图表上的数据标签显示有关图表数据系列或单个数据点的详细信息。它们使读者能够快速识别数据系列，并且使图表更易于理解。在 Aspose.Slides for Python 中，您可以为任何图表启用、定制和格式化数据标签——选择显示的内容（值、百分比、系列或类别名称）、标签的位置以及它们的外观（字体、数字格式、分隔符、引线等）。本文概述了添加清晰、信息丰富标签所需的关键 API 和示例。

## **设置数据标签精度**

图表数据标签通常显示需要统一精度的数值。本节展示如何通过应用适当的数字格式来控制 Aspose.Slides 中数据标签的小数位数。

以下 Python 示例展示了如何为图表数据标签设置数值精度：
...
本节展示如何在图表数据标签中显示百分比并附加百分号。您将学习如何为整个系列或特定数据点（适用于饼图、环形图和 100% 堆叠图）启用百分比值，并通过标签选项或自定义数字格式控制其显示。

以下 Python 示例演示了如何在图表的数据标签中添加百分号：
...
本节展示如何在 Aspose.Slides 中控制数据标签与图表坐标轴之间的距离。调整此偏移量可以防止标签重叠，提高密集视觉的可读性。
...
当创建不使用坐标轴的图表（例如饼图）时，数据标签可能会贴近图形边缘。此时，需要调整标签位置以便引线清晰可见。

以下 Python 代码演示了如何在饼图上调整标签位置：
...
![更改的标签位置](changed_label_position.png)

## **常见问题**

**如何防止在密集图表上标签重叠？**

结合自动标签布局、引线以及减小字体大小；必要时隐藏部分字段（例如类别），或仅对极值/关键点显示标签。

**如何仅对零、负值或空值禁用标签？**

在启用标签之前过滤数据点，并根据预定义规则关闭对值为 0、负数或缺失的显示。

**导出为 PDF/图像时，如何确保标签样式一致？**
