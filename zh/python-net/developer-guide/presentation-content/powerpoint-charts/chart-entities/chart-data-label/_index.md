...
linktitle: 数据标签
type: docs
url: /zh/python-net/developer-guide/presentation-content/powerpoint-charts/chart-entities/chart-data-label/
keywords:
- 图表
- 数据标签
- 数据精度
...
图表中的数据标签显示有关图表数据系列或单个数据点的详细信息。它们使读者能够快速识别数据系列，并使图表更易于理解。在 Aspose.Slides for Python 中，您可以为任何图表启用、定制和格式化数据标签——选择显示的内容（数值、百分比、系列或类别名称）、标签的位置以及外观（字体、数字格式、分隔符、引导线等）。本文概述了添加清晰、信息丰富标签所需的关键 API 和示例。

## **设置数据标签精度**

图表数据标签通常显示需要统一精度的数值。本节展示如何通过应用适当的数字格式来控制 Aspose.Slides 中数据标签的小数位数。

以下 Python 示例演示如何为图表数据标签设置数值精度：
...
使用 Aspose.Slides，您可以在图表的数据标签中显示百分比。下面的示例计算每个点在其类别中的占比，并将标签格式化为显示百分比。
...
本节展示如何在图表数据标签中显示百分比并加入百分号。您将学习如何为整个系列或特定数据点启用百分比值（适用于饼图、环形图以及 100% 堆叠图），以及如何通过标签选项或自定义数字格式来控制其显示方式。

以下 Python 示例演示如何在图表的数据标签中添加百分号：
...
本节展示如何在 Aspose.Slides 中控制数据标签与图表坐标轴之间的距离。调整此偏移量可避免标签重叠，提高密集视觉内容的可读性。

以下 Python 代码演示在基于坐标轴的图表中设置标签与类别轴的距离：
...
当创建不使用坐标轴的图表（如饼图）时，数据标签可能太靠近边缘。此时，需要调整标签位置以使引导线清晰可见。
...
![已更改标签位置](changed_label_position.png)

## **常见问题**

**如何防止密集图表中的数据标签重叠？**

结合自动标签布局、引导线以及减小字体大小；必要时隐藏某些字段（例如类别），或仅对极值/关键点显示标签。

**如何仅对零、负数或空值禁用标签？**

在启用标签之前过滤数据点，并依据定义的规则关闭对值为 0、负数或缺失值的显示。

**导出为 PDF/图片时，如何确保标签样式保持一致？**
