...
FontFallBackRule 类的实例可以组织到 [FontFallBackRulesCollection](https://reference.aspose.com/slides/python-net/aspose.slides/fontfallbackrulescollection/) 中，后者实现 [IFontFallBackRulesCollection](https://reference.aspose.com/slides/python-net/aspose.slides/ifontfallbackrulescollection/) 接口。可以向集合中添加或删除规则。

然后可以将此集合分配给 [FontFallBackRulesCollection](https://reference.aspose.com/slides/python-net/aspose.slides/fontsmanager/)属性，属于 [FontsManager](https://reference.aspose.com/slides/python-net/aspose.slides/fontsmanager/) 类。FontsManager 控制整个演示文稿的字体。阅读更多 [关于 FontsManager 和 FontsLoader](/slides/zh/python-net/about-fontsmanager-and-fontsloader/)。
...
以下是一个示例，展示如何创建回退字体规则集合并将其分配给特定演示文稿的 FontsManager：
```py
...
**缺失字体的 replacement/substitution 与缺失字形的回退可以一起使用吗？**

可以。它们是同一字体解析流程中的独立阶段：首先引擎解析字体可用性（[replacement](/slides/zh/python-net/font-replacement/)/[substitution](/slides/zh/python-net/font-substitution/)），然后回退在可用字体中填补缺失字形的空白。