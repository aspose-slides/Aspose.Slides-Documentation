...
- Python
- Aspose.Slides
description: "使用 Aspose.Slides for Python via .NET 快速克隆或复制 PowerPoint 幻灯片。遵循我们清晰的代码示例和技巧，在几秒钟内实现 PPT 自动生成，提高生产力，消除手动工作。"
---
...
在 Aspose.Slides for Python via .NET 中，`Presentation` 对象公开的[幻灯片集合](https://reference.aspose.com/slides/python-net/aspose.slides/slidecollection/)提供 `add_clone` 和 `insert_clone` 方法以执行这些克隆操作。
...
1. 从 `Presentation` 对象获取幻灯片集合。
1. 在 `SlideCollection` 上调用 `add_clone` 方法，传入要克隆的幻灯片。
1. 保存修改后的演示文稿。
...
1. 创建 `Presentation` 类的实例。
1. 从 `Presentation` 对象获取幻灯片集合。
1. 在 `SlideCollection` 上调用 `insert_clone` 方法，传入要克隆的幻灯片以及目标索引。
1. 保存修改后的演示文稿。
...
1. 为目标演示文稿（将添加幻灯片的地方）创建 `Presentation` 类的实例。
1. 从目标演示文稿获取幻灯片集合。
1. 在目标 `SlideCollection` 上调用 `add_clone`，传入源演示文稿中的幻灯片。
1. 保存修改后的目标演示文稿。
...
1. 为目标演示文稿（将添加幻灯片的地方）创建 `Presentation` 类的实例。
1. 从目标演示文稿获取幻灯片集合。
1. 在目标 `SlideCollection` 上调用 `insert_clone`，传入源幻灯片以及期望的目标索引。
1. 保存修改后的目标演示文稿。
...
1. 为源演示文稿（包含要克隆的幻灯片）创建 `Presentation` 类的实例。
1. 为目标演示文稿创建 `Presentation` 类的实例。
1. 访问要克隆的源幻灯片及其母版。
1. 从目标演示文稿的母版集合中获取 `MasterSlideCollection`。
1. 在目标 `MasterSlideCollection` 上调用 `add_clone`，传入源母版以将其克隆到目标中。
1. 从目标演示文稿获取 `SlideCollection`。
1. 在目标 `SlideCollection` 上调用 `add_clone`，传入源幻灯片和已克隆的目标母版。
1. 保存修改后的目标演示文稿。
...
会。备注页和审阅评论会随克隆一起复制。如果不需要它们，请在插入后[删除它们](/slides/zh/python-net/presentation-notes/)。

**图表及其数据源如何处理？**

图表对象、格式以及嵌入的数据都会被复制。如果图表链接到外部源（例如 OLE 嵌入的工作簿），该链接会保留为[OLE 对象](/slides/zh/python-net/manage-ole/)。在文件之间移动后，请验证数据可用性并刷新行为。

**我可以控制克隆的插入位置和章节吗？**

可以。您可以在具体的幻灯片索引处插入克隆，并将其放入选定的[章节](/slides/zh/python-net/slide-section/)。如果目标章节不存在，请先创建章节，然后再将幻灯片移动进去。