...
由于 CentOS 7 附带 GLIBC 2.14，而 Aspose.Slides for .NET 6 和 .NET 7（包括跨平台构建）需要 Linux x86_64 且 GLIBC 2.23 或更高版本，所以可以在此类系统上使用 Aspose.Slides for .NET Standard。
...
### **Mono Framework**
- 在 Mac 和 Linux 平台上支持 MONO

## **开发环境**
Aspose.Slides for .NET 可用于在任何面向 .NET 平台的开发环境中开发应用程序，但以下环境明确受支持：
...
这是 Aspose.Slides 使用 Aspose.Slides 团队开发的自定义跨平台图形引擎的版本。 在非 Windows 平台上，可能需要 `fontconfig` 库。
...
**是否需要安装 Microsoft PowerPoint 进行转换和渲染？**
...
实际上，演示文稿中使用的字体或相应的[替代字体](/slides/zh/net/font-substitution/)必须可用。为确保在 Linux/macOS 上渲染一致，建议安装常见的字体包。