...
- C#
- Aspose.Slides
description: "发现 Aspose.Slides for .NET：轻松管理 PowerPoint 和 OpenDocument 格式中的图表工作簿，简化演示文稿数据。"
---
## **概述**

本文说明了如何在 Aspose.Slides 中使用图表工作簿。它展示了如何通过工作簿流读取和写入图表数据、使用工作簿单元格作为图表数据标签、访问工作表集合以及为图表值指定数据源类型。

还涵盖了使用外部工作簿作为图表数据源的内容。示例演示了如何创建并分配外部工作簿、检索链接到图表的外部工作簿路径，以及在工作簿可用时编辑图表数据。

## **从工作簿读取和写入图表数据**
Aspose.Slides 提供了 [ReadWorkbookStream](https://reference.aspose.com/slides/zh/net/aspose.slides.charts/ichartdata/readworkbookstream/) 和 [WriteWorkbookStream](https://reference.aspose.com/slides/zh/net/aspose.slides.charts/ichartdata/writeworkbookstream/) 方法，允许您读取和写入图表数据工作簿（包含使用 Aspose.Cells 编辑的图表数据）。**注意**，图表数据必须以相同方式组织，或具有类似于源的结构。
...
5. 根据源类型与外部工作簿数据源类型相同的条件，指定相应的条件。
...
以下 C# 示例打开一个图表引用不可用外部工作簿的演示文稿，并通过 [IChart.ChartData](https://reference.aspose.com/slides/zh/net/aspose.slides.charts/ichart/chartdata/) 和 [IChartData.ChartDataWorkbook](https://reference.aspose.com/slides/zh/net/aspose.slides.charts/ichartdata/chartdataworkbook/) 访问恢复的数据：
...
不会。演示文稿仅存储指向外部文件的 [链接](https://reference.aspose.com/slides/zh/net/aspose.slides.charts/chartdata/externalworkbookpath/)，并在读取数据时使用该链接。保存演示文稿时不会修改外部文件本身。

**如果外部文件受密码保护该怎么办？**

Aspose.Slides 在链接时不接受密码。常见做法是事先移除保护或准备一个已解密的副本（例如使用 [Aspose.Cells](/cells/net/)），然后链接到该副本。

**多个图表可以引用同一个外部工作簿吗？**

可以。每个图表都会存储自己的链接。如果它们指向相同的文件，更新该文件后，下次加载数据时各图表都会反映出更改。