...
description: "PowerPoint 演示文稿中的表格单元格（使用 C# 或 .NET）"
---
...
{
    ITable table = pres.Slides[0].Shapes[0] as ITable; // 假设 Slide#0.Shape#0 是表格
...
## **合并单元格中的编号**
如果我们合并两个单元格对 (1, 1) x (2, 1) 和 (1, 2) x (2, 2)，得到的表格将会编号。下面的 C# 代码演示了该过程：
```c#
// 实例化表示 PPTX 文件的 Presentation 类
...
## **拆分单元格中的编号**
在前面的示例中，当表格单元格合并时，其他单元格的编号或编号系统保持不变。

这次，我们使用一个普通表格（即没有合并单元格的表格），然后尝试拆分单元格 (1,1) 以得到一个特殊的表格。您可能需要关注该表格的编号方式，可能会显得奇怪。但这正是 Microsoft PowerPoint 对表格单元格进行编号的方式，Aspose.Slides 也采用相同的行为。
...
下面的 C# 代码演示了在创建表格时如何在表格单元格中放置图像：
```c#
// 实例化表示 PPTX 文件的 Presentation 类
using (Presentation presentation = new Presentation())
{
...
可以。单元格的 [上](https://reference.aspose.com/slides/net/aspose.slides/cellformat/bordertop/)/[下](https://reference.aspose.com/slides/net/aspose.slides/cellformat/borderbottom/)/[左](https://reference.aspose.com/slides/net/aspose.slides/cellformat/borderleft/)/[右](https://reference.aspose.com/slides/net/aspose.slides/cellformat/borderright/) 边框各自拥有独立的属性，因此每一侧的粗细和样式可以不同。这与文章中演示的对单元格的逐边框控制逻辑一致。

**如果在将图片设为单元格背景后更改列/行大小，图像会怎样？**

其行为取决于 [填充模式](https://reference.aspose.com/slides/net/aspose.slides/picturefillmode/)（stretch/tile）。在拉伸模式下，图像会适应新的单元格；在平铺模式下，平铺会重新计算。文章中提到了单元格内图像的显示模式。

**我能为单元格的全部内容分配超链接吗？**

[超链接](/slides/zh/net/manage-hyperlinks/) 可以在单元格文本框内的文字（段落）级别或在整个表格/形状级别设置。实际操作中，您可以将链接分配给某段文字或单元格中的全部文字。

**我能在单个单元格内使用不同的字体吗？**

可以。单元格的文本框支持 [段落](https://reference.aspose.com/slides/net/aspose.slides/portion/)（运行），可以独立设置格式——字体、样式、大小和颜色。