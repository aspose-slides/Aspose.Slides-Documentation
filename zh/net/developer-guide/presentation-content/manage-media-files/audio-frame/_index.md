...
- C#
- Aspose.Slides
description: "在 Aspose.Slides for .NET 中创建和控制音频帧——示例 C# 用于嵌入、裁剪、循环以及在 PPT、PPTX 和 ODP 演示文稿中配置播放。"
---

## **创建音频帧**

Aspose.Slides for .NET 允许您向幻灯片添加音频文件。这些音频文件以音频帧的形式嵌入到幻灯片中。
...
当您向演示文稿添加音频文件时，音频会以带有标准默认图像的帧形式显示（请参见下节中的图片）。您可以更改音频帧的缩略图（设置您喜欢的图像）。

以下 C# 代码演示了如何更改音频帧的缩略图或预览图像：
```c#
...
PowerPoint **编辑** 选项对应 Aspose.Slides [AudioFrame](https://reference.aspose.com/slides/net/aspose.slides/audioframe) 属性：
...
PowerPoint 音频控制面板上的 **Volume 控件** 对应 [AudioFrame.VolumeValue](https://reference.aspose.com/slides/net/aspose.slides/audioframe/volumevalue/) 属性。它允许您以百分比的形式更改音频音量。
...
以下 C# 示例展示了如何添加带嵌入音频的新音频帧、对其进行裁剪并设置淡入淡出时长：
```c#
using (Presentation pres = new Presentation())
{
...
Aspose.Slides for .NET 允许您提取幻灯片放映过渡时使用的声音。例如，您可以提取特定幻灯片使用的声音。

1. 创建一个 [Presentation](https://reference.aspose.com/slides/net/aspose.slides/presentation) 类的实例，并加载包含音频的演示文稿。
2. 通过索引获取相关幻灯片的引用。
3. 访问该幻灯片的放映过渡。
4. 将声音提取为字节数据。
...
是的。只需将音频一次添加到演示文稿的共享 [audio collection](https://reference.aspose.com/slides/net/aspose.slides/presentation/audios/) 中，然后创建引用该现有资源的其他音频帧。这样可避免重复媒体数据，使演示文稿大小保持在可控范围内。

**我能在不重新创建形状的情况下替换已有音频帧中的声音吗？**

是的。对于链接的声音，更新 [link path](https://reference.aspose.com/slides/net/aspose.slides/audioframe/linkpathlong/) 指向新文件。对于嵌入的声音，将 [embedded audio](https://reference.aspose.com/slides/net/aspose.slides/audioframe/embeddedaudio/) 对象替换为演示文稿的 [audio collection](https://reference.aspose.com/slides/net/aspose.slides/presentation/audios/) 中的其他音频。帧的格式和大多数播放设置保持不变。
