---
title: Aspose.Slides for .NET 14.8.0 中的公共 API 及不兼容的向后更改
...
此页面列出所有[已添加](/slides/zh/net/public-api-and-backwards-incompatible-changes-in-aspose-slides-for-net-14-8-0/)或[已删除](/slides/zh/net/public-api-and-backwards-incompatible-changes-in-aspose-slides-for-net-14-8-0/)的类、方法、属性等，以及 Aspose.Slides for .NET 14.8.0 API 引入的其他更改。
...
此外，您还可以使用实现 IVbaProject 接口的 VbaProject 类创建新的 VBA 项目。

下面的示例展示了如何创建包含一个模块的简单 VBA 项目，并向库添加两个必需的引用。
...
该属性不仅适用于本系列，还适用于父系列组中的所有系列——它是相应组属性的投影。因此此属性为只读。

- 使用 ParentSeriesGroup 属性访问父系列组。
- 使用 ParentSeriesGroup.Overlap 可读写属性来更改其值。
...
此形状缩略图生成方法允许您在形状外观的边界内生成缩略图，考虑所有形状效果。生成的形状缩略图受幻灯片边界限制。
