---
title: Aspose.Slides for .NET 14.5.0 公共 API 及向后不兼容的更改
...
Aspose.Slides.IShape.ParentGroup 属性在形状被分组时返回其父 GroupShape 对象，否则返回 null。
...
Aspose.Slides.IShapeCollection.InsertGroupShape(int) 方法创建一个新的 GroupShape 并将其插入到集合中的指定索引位置。
- 当添加新形状时，GroupShape 的框架大小和位置将适配内容。
...
此属性允许开发人员在无需实例化的情况下使用工厂功能。

### **限制**
#### **IShape.Frame 的限制**
已添加对 IShape.Frame 使用未定义值的限制。尝试将未定义的框架分配给 IShape.Frame 的代码在大多数情况下没有意义（特别是当父 GroupShape 多次嵌套到其他 {{GroupShape}} 中时）。例如：
...
此类代码可能导致不明确的情况。因此已对 IShape.Frame 使用未定义值添加了限制。x、y、width、height、flipH、flipV 和 rotationAngle 的值必须已定义（且不能设为 float.NaN 或 NullableBool.NotDefined）。上述示例代码现在会抛出 ArgumentException 异常。
...
但 IShape.RawFrame 的框架属性可以未定义。当形状链接到占位符时，这很合理。此时未定义的形状框架值将从父占位符形状覆盖。如果没有父占位符形状，则该形状在基于其 IShape.RawFrame 计算有效框架时使用默认值。默认值为 x、y、width、height、flipH、flipV 和 rotationAngle 的 0 和 NullableBool.False。例如：
...
- Aspose.Slides.IShapeCollection.Parent 属性的类型已从 ISlideComponent 更改为新的 IGroupShape 接口。IGroupShape 接口是 ISlideComponent 的子接口，因此现有代码无需改动。
...
- IShape.Frame 属性返回一个有效的 IShapeFrame 实例（其所有属性都有已定义的有效值）。
- IShape.RawFrame 属性返回一个 IShapeFrame 实例，其中每个属性都可以是未定义值（尤其是 FlipH 或 FlipV 可以是 NullableBool.NotDefined）。