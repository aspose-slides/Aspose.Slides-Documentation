...
如果您想降低存储成本，可以将 Microsoft PowerPoint 演示文稿转换为 XPS 格式。这样，您会发现保存、共享和打印文档更加简便。

Microsoft 继续在 Windows（甚至在 Windows 10 中）中实现对 XPS 的强力支持，因此您可能需要考虑将文件保存为此格式。如果您使用的是 Windows 8.1、Windows 8、Windows 7 或 Windows Vista，则 XPS 可能是某些操作的最佳选择。
...
Microsoft 最终在 Windows 10 中通过“打印到 PDF”功能实现了对 PDF 打印操作的支持。在此之前，用户通常通过 XPS 格式打印文档。

## **使用 Aspose.Slides 进行 XPS 转换**

在适用于 .NET 的 [**Aspose.Slides**](https://products.aspose.com/slides/net/) 中，您可以使用由 [Presentation](https://reference.aspose.com/slides/net/aspose.slides/presentation) 类公开的 [**Save**](https://reference.aspose.com/slides/net/aspose.slides/presentation/methods/save/index) 方法将整个演示文稿转换为 XPS 文档。
...
可以——Aspose.Slides 允许您直接导出到流，这对于 Web API、服务器端流水线或任何希望在不触及文件系统的情况下发送 XPS 的场景都非常理想。

**隐藏的幻灯片会被导出到 XPS 吗？我可以排除它们吗？**

默认情况下，仅渲染常规（可见）幻灯片。您可以通过在保存为 XPS 之前的 [导出设置](https://reference.aspose.com/slides/net/aspose.slides.export/xpsoptions/) 来 [包含或排除隐藏幻灯片](https://reference.aspose.com/slides/net/aspose.slides.export/xpsoptions/showhiddenslides/)，确保输出仅包含您期望的页面。