...
- C#
- Aspose.Slides
description: "了解如何在 C# 中自动化 PowerPoint 图表的创建。本分步指南展示了为何 Aspose.Slides for .NET 是 Microsoft.Office.Interop 更快、更强大的替代方案。"
---

## **概述**

本文演示如何使用 C# 在 Microsoft PowerPoint 演示文稿中以编程方式创建和自定义图表。借助 Aspose.Slides for .NET，您可以在无需 Microsoft Office 或 Interop 库的情况下自动生成专业的数据驱动图表。该 API 提供了丰富的功能，可用于构建柱状图、饼图、折线图等——全部可完全控制外观、数据和布局。无论是生成报告、仪表板还是商务演示，Aspose.Slides 都能帮助您直接从 .NET 应用程序中交付高质量可视化。

## **VSTO 示例**

本节演示如何使用 **VSTO (Visual Studio Tools for Office)** 在 Microsoft PowerPoint 演示文稿中创建图表。通过 VSTO，您可以结合 PowerPoint 与 Excel 自动化，以编程方式生成和自定义图表。示例展示了如何添加 **3D 群集柱形图**，从 Excel 工作表填充数据，调整格式和布局，并保存最终演示文稿——全部在 .NET 应用程序中完成。

1. 创建 Microsoft PowerPoint 演示文稿实例。  
2. 向演示文稿添加一个空白幻灯片。  
3. 添加 3D 群集柱形图并获取其引用。  
4. 创建新的 Microsoft Excel 工作簿实例并加载图表数据。  
5. 使用 Excel 工作簿实例访问图表数据工作表。  
6. 设置工作表中的图表范围，并从图表中移除系列 2 和 3。  
7. 修改图表数据工作表中的类别数据。  
8. 修改图表数据工作表中的系列 1 数据。  
9. 访问图表标题并设置其字体相关属性。  
10. 访问图表的值轴并设置主单位、次单位、最大值和最小值。  
11. 访问图表的深度（系列）轴并将其移除——本示例仅使用一个系列。  
...
以下示例展示如何使用 Aspose.Slides for .NET 在 PowerPoint 演示文稿中创建一个简单图表。该代码演示了如何添加 **3D 群集柱形图**，使用示例数据填充并自定义外观。仅需几行代码，即可动态生成图表并将其集成到演示文稿中，无需使用 Microsoft Office。
...
可以。Aspose.Slides for .NET 支持广泛的 [图表类型](https://docs.aspose.com/slides/net/create-chart/)，包括饼图、折线图、条形图、散点图、气泡图等。添加图表时，可使用 [ChartType](https://reference.aspose.com/slides/net/aspose.slides.charts/charttype/) 枚举指定所需的图表类型。

**我可以为图表应用自定义样式或主题吗？**

可以。您可以完全自定义图表的外观，包括颜色、字体、填充、轮廓、网格线和布局。但要完全复现 PowerPoint 中的 Office 主题，需要手动设置各项样式。

**我可以将图表单独导出为图像吗？**

可以，Aspose.Slides 允许您使用图表 [shape](https://reference.aspose.com/slides/net/aspose.slides/ishape/) 上的 `GetImage` 方法，将任何形状（包括图表）导出为单独的图像（例如 PNG、JPEG）。