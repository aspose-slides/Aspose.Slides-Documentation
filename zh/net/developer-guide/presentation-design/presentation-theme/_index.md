...
PowerPoint 主题为幻灯片上的不同元素使用一组特定的颜色。如果您不喜欢这些颜色，可以通过为主题应用新颜色来更改它们。 为帮助您选择新的主题颜色，Aspose.Slides 在 [SchemeColor](https://reference.aspose.com/slides/zh/net/aspose.slides/schemecolor/) 枚举中提供了相应的值。

此 C# 代码演示了如何更改主题的强调颜色：
...
为了进一步演示颜色更改操作，我们创建了另一个元素并将（来自初始操作的）强调颜色分配给它。然后我们在主题中更改颜色：
...
这段 C# 代码演示了从主主题颜色获取附加调色板颜色并在形状中使用它们的操作：
...
当您使用 [SchemeColor](https://reference.aspose.com/slides/zh/net/aspose.slides/schemecolor/) 时，可能会注意到它包含以下主题颜色值：

`Background1`, `Background2`, `Text1`, and `Text2`.

然而，`Presentation.MasterTheme.ColorScheme` 返回 [IColorScheme](https://reference.aspose.com/slides/zh/net/aspose.slides.theme/icolorscheme/)，它将相应的颜色公开为：

`Dark1`, `Dark2`, `Light1`, and `Light2`.

这种差异仅在命名上。这些值指向相同的主题颜色槽，映射是固定的：
...
不存在在 `Text`/`Background` 与 `Dark`/`Light` 之间的动态转换。它们仅是同一主题颜色的备用名称。

这种命名差异来源于 Microsoft Office 的术语。较早的 Office 版本使用 `Dark 1`、`Light 1`、`Dark 2` 和 `Light 2`，而较新的 UI 版本则将相同的槽显示为 `Text 1`、`Background 1`、`Text 2` 和 `Background 2`。
...
* **+mj-ea** - 正文字体东亚（Minor East Asian Font）
...
是的。Aspose.Slides 支持幻灯片级别的主题覆盖，您可以仅对该幻灯片应用本地主题，同时保持母版主题不变（通过 [SlideThemeManager](https://reference.aspose.com/slides/zh/net/aspose.slides.theme/slidethememanager/)）。

### 将主题从一个演示文稿迁移到另一个演示文稿的最安全方法是什么？

将 [克隆幻灯片](/slides/zh/net/clone-slides/) 与其母版一起复制到目标演示文稿中。这会保留原始的母版、布局以及关联的主题，从而保持外观一致。

### 如何查看在所有继承和覆盖之后的 “实际” 值？

使用 API 的 [“实际”视图](/slides/zh/net/shape-effective-properties/) 来查看主题、颜色、字体、效果等的实际值。这些视图在应用母版及任何本地覆盖后返回解析后的最终属性。