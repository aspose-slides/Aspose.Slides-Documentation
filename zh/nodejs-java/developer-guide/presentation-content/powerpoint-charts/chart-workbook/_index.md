...
- JavaScript
- Aspose.Slides
description: "通过 Java 了解 Aspose.Slides for Node.js：轻松管理 PowerPoint 和 OpenDocument 格式中的图表工作簿，以简化演示文稿数据。"
---
...
1. 创建 [Presentation](https://apireference.aspose.com/slides/nodejs-java/aspose.slides/presentation) 类的实例。
2. 通过索引获取幻灯片的引用。
3. 添加带有一些数据的气泡图。
4. 访问图表系列。
5. 将工作簿单元格设置为数据标签。
...
可以。图表具有 [数据源类型](https://reference.aspose.com/slides/nodejs-java/aspose.slides/chartdata/getdatasourcetype/) 和指向外部工作簿的 [路径](https://reference.aspose.com/slides/nodejs-java/aspose.slides/chartdata/getexternalworkbookpath/)；如果源是外部工作簿，您可以读取完整路径以确认使用了外部文件。
...
不会。演示文稿会存储指向外部文件的 [链接](https://reference.aspose.com/slides/nodejs-java/aspose.slides/chartdata/getexternalworkbookpath/)，并在读取数据时使用它。保存演示文稿时不会修改外部文件本身。

**如果外部文件受密码保护，我该怎么办？**

Aspose.Slides 在链接时不接受密码。常见做法是预先移除保护或准备一个已解密的副本（例如，使用 [Aspose.Cells](/cells/nodejs-java/)），并链接到该副本。
