...
是的。Aspose.Slides 支持通过 [compliance](https://reference.aspose.com/slides/nodejs-java/aspose.slides/pdfoptions/setcompliance/) 设置在 [PDF export options](https://reference.aspose.com/slides/nodejs-java/aspose.slides/pdfoptions/) 中导出符合 PDF/A-2a、PDF/A-2b、PDF/A-2u、PDF/A-3a、PDF/A-3b 以及 PDF/UA 等合规级别的 PDF。

**库在导出为 PDF 时是否支持字体嵌入，并能够细粒度控制嵌入内容？**

是的。您可以控制是完整嵌入字体还是子集嵌入（仅使用的字形），指定常见系统字体的处理方式，并通过 [PDF export options](https://reference.aspose.com/slides/nodejs-java/aspose.slides/pdfoptions/) 配置 ASCII 文本的行为。

**我可以在实际加载文件之前检测文件是否受密码保护吗？**

是的。使用 [factory-based inspection API](https://reference.aspose.com/slides/nodejs-java/aspose.slides/presentationfactory/) ，您可以查询演示文稿文件以确定其是否受密码保护，而无需完整打开。

**是否提供字体回退机制并支持自定义字体？**

是的。库支持[加载](/slides/zh/nodejs-java/custom-font/)和[嵌入](/slides/zh/nodejs-java/embedded-font/)自定义字体，并提供字体[回退规则](/slides/zh/nodejs-java/fallback-font/)以防止在渲染和转换期间出现缺失字符。

**我可以将幻灯片导出为 XPS 吗？是否有调节 XPS 输出的选项？**

是的。支持[导出为 XPS](/slides/zh/nodejs-java/convert-powerpoint-to-xps/)，您可以调整相关的[保存选项](https://reference.aspose.com/slides/nodejs-java/aspose.slides/xpsoptions/)来控制 XPS 文档的输出质量和内容。