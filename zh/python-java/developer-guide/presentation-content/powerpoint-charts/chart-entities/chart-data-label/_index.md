...
- Java
- Aspose.Slides
description: "学习使用 Aspose.Slides for Python via Java 在 PowerPoint 演示文稿中添加和格式化图表数据标签，以创建更具吸引力的幻灯片。"
---
## **简介**

图表上的数据标签显示有关图表数据系列或单个数据点的详细信息。它们可以让读者快速识别数据系列，并使图表更易于理解。

## **设置图表数据标签的数据精度**

下面的 Python 代码演示如何在图表数据标签中设置数据精度：
...
## **将百分比显示为标签**
Aspose.Slides for Python via Java 允许在显示的图表上设置百分比标签。下面的 Python 代码演示此操作：
...
## **设置标签与坐标轴的距离**
下面的 Python 代码演示在从坐标轴绘制的图表中如何设置标签与类别坐标轴的距离：
...
当创建不依赖任何坐标轴的图表（例如饼图）时，图表的数据标签可能会距离边缘过近。此时，需要调整数据标签的位置，以便清晰显示引线。

下面的 Python 代码演示如何在饼图上调整标签位置：
...
结合自动标签放置、引线以及减小字体大小；如有必要，可隐藏某些字段（例如类别），或仅对极端/关键点显示标签。
