...
- Python
- Aspose.Slides
description: "在 Aspose.Slides for Python via Java 中创建和控制音频帧——嵌入、剪辑、循环以及在 PPT、PPTX 和 ODP 演示文稿中配置播放的代码示例。"
---
## **概述**

本文说明了如何在 Aspose.Slides 中使用音频帧。展示了如何向幻灯片添加嵌入式音频、定制音频帧缩略图、配置播放选项（如音量、循环、隐藏、剪辑和淡入淡出时长），以及提取幻灯片放映过渡中使用的音频。

## **创建音频帧**

Aspose.Slides for Python via Java 允许您向幻灯片添加音频文件。音频文件会以音频帧的形式嵌入到幻灯片中。
...
5. 设置由 [AudioFrame](https://reference.aspose.com/slides/zh/python-java/aspose.slides/audioframe/) 对象公开的 [setPlayMode](https://reference.aspose.com/slides/zh/python-java/aspose.slides/audioframe/#setPlayMode) 和 [setVolume](https://reference.aspose.com/slides/zh/python-java/aspose.slides/audioframe/#setVolume)。
6. 保存修改后的演示文稿。

下面的 Python 代码演示了如何向幻灯片添加嵌入式音频帧：
...
当您向演示文稿添加音频文件时，音频会显示为带有标准默认图像的帧（见下图）。您可以更改音频帧的预览图像（设置您喜欢的图像）。
...
Aspose.Slides for Python via Java 允许您更改控制音频播放或属性的选项。例如，您可以调节音频音量、设置音频循环播放，甚至隐藏音频图标。
...
- **Trim Audio End Time** 的值等于音频时长减去 [setTrimFromEnd](https://reference.aspose.com/slides/zh/python-java/aspose.slides/audioframe/#setTrimFromEnd) 方法的值

PowerPoint 音频控制面板上的 **Volume control** 对应 [setVolumeValue](https://reference.aspose.com/slides/zh/python-java/aspose.slides/audioframe/#setVolumeValue) 方法，可让您以百分比方式更改音量。
...
下面的 Python 示例展示了如何添加带嵌入音频的新音频帧、剪辑它并设置淡入淡出时长：
...
下面的代码示例展示了如何获取带嵌入音频的音频帧并将音量设置为 85%：
...
Aspose.Slides 允许您通过 [getCaptionTracks](https://reference.aspose.com/slides/zh/python-java/aspose.slides/audioframe/#getCaptionTracks) 方法为音频帧添加闭合字幕。该方法返回一个 [CaptionsCollection](https://reference.aspose.com/slides/zh/python-java/aspose.slides/captionscollection/)，您可以在其中添加 WebVTT 字幕轨道、遍历已有轨道，并在需要时将其移除。

**添加音频字幕**

使用 [getCaptionTracks](https://reference.aspose.com/slides/zh/python-java/aspose.slides/audioframe/#getCaptionTracks) 方法将一个或多个字幕轨道附加到音频帧。以下示例中，先向幻灯片添加音频文件，然后从 `.vtt` 文件加载新的字幕轨道。
...
您可以遍历与音频帧关联的字幕轨道，并将它们保存为 `.vtt` 文件。每个字幕轨道都公开其二进制数据和唯一标识符，可在导出字幕时使用。
...
**我可以在多张幻灯片之间重复使用相同的音频资源而不会增加文件大小吗？**

可以。将音频一次性添加到演示文稿的共享 [audio collection](https://reference.aspose.com/slides/zh/python-java/aspose.slides/presentation/#getAudios)，并创建引用该已有资产的额外音频帧。这样可避免媒体数据重复，从而控制演示文稿的体积。

**我可以在不重新创建形状的情况下替换现有音频帧中的声音吗？**

可以。对于链接的声音，更新 [link path](https://reference.aspose.com/slides/zh/python-java/aspose.slides/audioframe/#setLinkPathLong) 指向新文件。对于嵌入的声音，将 [embedded audio](https://reference.aspose.com/slides/zh/python-java/aspose.slides/audioframe/#setEmbeddedAudio) 对象替换为演示文稿的 [audio collection](https://reference.aspose.com/slides/zh/python-java/aspose.slides/presentation/#getAudios) 中的另一对象。帧的格式以及大多数播放设置都会保持不变。
