...
- PHP
- Aspose.Slides
description: "了解 Aspose.Slides for PHP via Java 可以打开、保存和转换的所有文件格式——包括 PPT、PPTX 和 ODP——并查看明确的导入/导出支持说明。"
---
...
## **支持的文件格式**
此表列出了 Aspose.Slides for PHP via Java 可以加载和保存的文件格式：
...
**我能将演示文稿保存为符合存档和可访问性标准（PDF/A 和 PDF/UA）的 PDF 吗？**

是的。Aspose.Slides 支持通过 [compliance](https://reference.aspose.com/slides/php-java/aspose.slides/pdfoptions/setcompliance/) 设置在 [PDF export options](https://reference.aspose.com/slides/php-java/aspose.slides/pdfoptions/) 中导出符合 PDF/A-2a、PDF/A-2b、PDF/A-2u、PDF/A-3a、PDF/A-3b 以及 PDF/UA 等合规级别的 PDF。

**库在导出为 PDF 时是否支持字体嵌入，并能对嵌入内容进行细粒度控制？**

是的。您可以通过 [PDF export options](https://reference.aspose.com/slides/php-java/aspose.slides/pdfoptions/) 控制字体是完整嵌入还是子集化（仅使用的字形），指定常见系统字体的处理方式，并配置 ASCII 文本的行为。

**我能在实际加载文件之前检测它是否受密码保护吗？**

是的。使用 [factory-based inspection API](https://reference.aspose.com/slides/php-java/aspose.slides/presentationfactory/)，您可以查询演示文稿文件以判断其是否受密码保护，而无需完整打开文件。

**是否有字体回退机制并支持自定义字体？**

是的。库支持 [loading](/slides/zh/php-java/custom-font/) 和 [embedding](/slides/zh/php-java/embedded-font/) 自定义字体，并提供字体 [fallback rules](/slides/zh/php-java/fallback-font/) 以防止在渲染和转换过程中出现缺失字形。

**我能将幻灯片导出为 XPS 吗？是否有调节 XPS 输出的选项？**

是的。支持 [Export to XPS](/slides/zh/php-java/convert-powerpoint-to-xps/)，您可以调整相关的 [save options](https://reference.aspose.com/slides/php-java/aspose.slides/xpsoptions/) 以控制 XPS 文档的输出质量和内容。