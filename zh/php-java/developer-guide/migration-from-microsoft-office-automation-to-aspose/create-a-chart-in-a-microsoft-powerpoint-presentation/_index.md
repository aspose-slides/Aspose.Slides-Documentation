...
{{% /alert %}} 
## **创建图表**
下面的代码示例描述了使用 VSTO 添加简单 3D 聚类柱状图的过程。您将创建一个演示文稿实例，向其中添加一个默认图表。然后使用 Microsoft Excel 工作簿访问和修改图表数据，并设置图表属性。最后，保存演示文稿。
### **VSTO 示例**
使用 VSTO，执行以下步骤：

1. 创建 Microsoft PowerPoint 演示文稿的实例。
1. 向演示文稿添加一个空白幻灯片。
1. 添加一个 **3D 聚类柱状图** 并访问它。
...
1. 访问图表深度或系列轴，并删除它，因为在此示例中仅使用一个系列。
...
使用 Aspose.Slides for PHP via Java，执行以下步骤：
