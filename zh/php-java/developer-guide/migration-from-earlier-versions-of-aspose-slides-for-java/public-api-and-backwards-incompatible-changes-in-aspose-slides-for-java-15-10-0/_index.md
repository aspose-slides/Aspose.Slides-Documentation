---
title: Aspose.Slides for PHP via Java 15.10.0中的公共API和不兼容更改
type: docs
weight: 180
...
此页面列出了在Aspose.Slides for PHP via Java 15.10.0 API中添加或删除的所有[class](/slides/php-java/public-api-and-backwards-incompatible-changes-in-aspose-slides-for-java-15-10-0/)类、方法、属性等，以及其他更改。

{{% /alert %}} 
## **公共API更改**
#### **为ISequence添加了图表系列动画API**
新的两个方法已添加到com.aspose.slides.ISequence接口。
...
引入了与图表元素动画相关的两个新枚举EffectChartMajorGroupingType和EffectChartMinorGroupingType。

要将系列动画添加到图表，可以使用以下代码：
...
```
#### **新增com.aspose.slides.VideoPlayerHtmlController以支持将媒体文件导出为HTML**
新增公共类com.aspose.slides.VideoPlayerHtmlController。用户可以使用此类的实例将视频和音频文件导出为HTML。

VideoPlayerHtmlController构造函数接受以下参数：

path：将生成视频和音频文件的路径
fileName：HTML文件的名称
baseUri：将用于生成链接的基本URI
