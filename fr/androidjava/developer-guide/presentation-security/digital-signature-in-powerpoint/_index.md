...
La présentation peut contenir plus d'une signature numérique. Après que la signature numérique soit ajoutée à la présentation, un message spécial apparaîtra dans PowerPoint :
...
1. Ouvrir le fichier PFX et passer le mot de passe PFX à l'objet [**DigitalSignature**](https://reference.aspose.com/slides/androidjava/com.aspose.slides/DigitalSignature).
    ...
    if (pres.getDigitalSignatures().size() > 0)
    {
        ...
        for (IDigitalSignature signature : pres.getDigitalSignatures())
        {
            System.out.println(signature.getComments() + ", "
                    + signature.getSignTime().toString() + " -- " + (signature.isValid() ? "VALIDE" : "INVALID"));
