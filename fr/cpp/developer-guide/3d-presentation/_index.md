...
## **Aperçu**
Depuis Aspose.Slides 20.9, il est possible de créer et de modifier des modèles 3D PowerPoint. Cela peut être réalisé en appliquant à des formes 2D un ensemble d'effets 3D. En créant une vue caméra sur la forme, vous pouvez la faire pivoter selon l'axe. Créez une extrusion ou une profondeur sur la forme, ce qui transformera la forme 2D en modèle 3D. Définir l'effet de lumière sur la forme 3D ou changer les matériaux peut la rendre plus vivante. Modifier les couleurs des modèles 3D en un dégradé 3D, modifier le contour des formes, ajouter un biseau donne plus de volume au modèle 3D. Tous les effets 3D peuvent être appliqués aux modèles 3D PowerPoint et aux textes.
...
## **Dégradé 3D**
Créer un dégradé 3D sur un modèle 3D PowerPoint peut être fait via la méthode [Shape.get_FillFormat().get_GradientFormat()](https://reference.aspose.com/slides/cpp/class/aspose.slides.fill_format#a1f075336cb7a0e05cd5d7a706b6f4f58) :
``` cpp
using namespace Aspose::Slides;
...
Oui. Slides fournit des API pour [lire les valeurs effectives](/slides/fr/cpp/shape-effective-properties/) (y compris pour le 3D — éclairage, biseaux, etc.) afin que vous puissiez voir les paramètres appliqués finaux.

**Les effets 3D fonctionnent-ils lors de la conversion d’une présentation en vidéo ?**

Oui. Lors de la [génération des images‑clés pour la vidéo](/slides/fr/cpp/convert-powerpoint-to-video/), les effets 3D sont rendus de la même façon que pour les [images exportées](/slides/fr/cpp/convert-powerpoint-to-png/).