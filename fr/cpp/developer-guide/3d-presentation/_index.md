...
- C++
- Aspose.Slides
description: "Générez des présentations 3D interactives en C++ avec Aspose.Slides sans effort. Exportez rapidement vers les formats PowerPoint et OpenDocument pour une utilisation polyvalente."
---

## **Vue d'ensemble**
Depuis Aspose.Slides 20.9 il est possible de créer et de modifier des modèles 3D PowerPoint. Cela peut être réalisé en appliquant à des formes 2D un ensemble d'effets 3D. En créant une vue caméra sur la forme, vous pouvez la faire pivoter autour d’un axe. Créez une extrusion ou une profondeur sur la forme, ce qui transformera la forme 2D en modèle 3D. Le réglage de l’effet de lumière sur la forme 3D ou la modification des matériaux peut la rendre plus vivante. Modifier les couleurs des modèles 3D avec un dégradé 3D, ajuster le contour des formes, ajouter un chanfrein donnent plus de volume au modèle 3D. Tous les effets 3D peuvent être appliqués aux modèles 3D PowerPoint ainsi qu’aux textes.

Observons le premier exemple de création de modèles 3D, qui comprend toutes les fonctionnalités décrites ci‑dessus :
``` cpp
...
auto shape = slide->get_Shapes()->AddAutoShape(ShapeType::Rectangle, 200.0f, 150.0f, 200.0f, 200.0f);
shape->get_ThreeDFormat()->get_Camera()->SetRotation(20.0f, 30.0f, 40.0f);
// ... définir les autres paramètres de scène 3D
...
Pour ajouter de la profondeur et une extrusion à un modèle 3D PowerPoint utilisez la méthode [IThreeDFormat.set_ExtrusionHeight()](https://reference.aspose.com/slides/cpp/class/aspose.slides.three_d_format#adf0bad4894b1c36d9e4b044ef4978295).  
Pour modifier la couleur de l'extrusion utilisez la méthode [IThreeDFormat.get_ExtrusionColor()](https://reference.aspose.com/slides/cpp/class/aspose.slides.three_d_format#aa7db8859d23a9b4eb2f35f3a42025e9e) :
``` cpp
auto shape = slide->get_Shapes()->AddAutoShape(ShapeType::Rectangle, 200.0f, 150.0f, 200.0f, 200.0f);
shape->get_ThreeDFormat()->get_Camera()->SetRotation(20.0f, 30.0f, 40.0f);
...
// .. configuration 3D: Camera, LightRig, Extrusion
...
## **Texte 3D (WordArt)**
Pour appliquer rotation, extrusion, lumière, dégradé au texte et en faire un texte 3D (WordArt), vous devez accéder à la méthode [IAutoShape.get_TextFrame().get_TextFrameFormat().get_ThreeDFormat()](https://reference.aspose.com/slides/cpp/class/aspose.slides.i_text_frame_format#a5e681109403c2e57aa76a500fe508b30) :
``` cpp
using namespace Aspose::Slides;
using namespace Aspose::Slides::Export;
...
Oui. Slides fournit des API pour [lire les valeurs effectives](/slides/fr/cpp/shape-effective-properties/) (y compris pour la 3D — éclairage, chanfreins, etc.) afin que vous puissiez voir les paramètres finaux appliqués.

**Les effets 3D fonctionnent-ils lors de la conversion d'une présentation en vidéo ?**

Oui. Lors de la [génération des images pour la vidéo](/slides/fr/cpp/convert-powerpoint-to-video/), les effets 3D sont rendus exactement comme ils le sont pour les [images exportées](/slides/fr/cpp/convert-powerpoint-to-png/).