...
## **Vue d'ensemble**
Depuis Aspose.Slides 20.9 il est possible de créer et de modifier des modèles 3D PowerPoint. Cela peut être réalisé en appliquant à des formes 2D un ensemble d’effets 3D. En créant une vue caméra sur la forme, vous pouvez la faire pivoter autour d’un axe. Créez une extrusion ou une profondeur sur la forme, ce qui la transforme d’une forme 2D en modèle 3D. 
Appliquer un effet lumineux sur la forme 3D ou modifier les matériaux permet de la rendre plus vivante. Modifier les couleurs des modèles 3D par un dégradé 3D, ajuster le contour des formes, ajouter un biseau donnent plus de volume au modèle 3D. Tous les effets 3D peuvent être appliqués tant aux modèles 3D PowerPoint qu’au texte.

Observons le premier exemple de création de modèles 3D, qui inclut toutes les fonctionnalités susmentionnées :
``` cpp
...
Oui. Slides propose des API pour [lire les valeurs effectives](/slides/fr/cpp/shape-effective-properties/) (y compris pour la 3D — éclairage, biseaux, etc.) afin de voir les paramètres appliqués finalement.
