...
## **Vue d'ensemble**
Depuis Aspose.Slides 20.9 il est possible de créer et de modifier des modèles 3D PowerPoint. Cela peut être réalisé en appliquant à des formes 2D un ensemble d’effets 3D. En créant une vue caméra sur la forme, vous pouvez la faire pivoter autour de l’axe. Créez une extrusion ou une profondeur sur la forme, ce qui transforme la forme 2D en modèle 3D. Le réglage de l’effet de lumière sur la forme 3D ou la modification des matériaux peut la rendre plus vivante. Modifier les couleurs des modèles 3D en un dégradé 3D, modifier le contour des formes, ajouter un chanfrein donne plus de volume au modèle 3D. Tous les effets 3D peuvent être appliqués aux modèles 3D PowerPoint et aux textes.

Observons le premier exemple de création de modèles 3D, qui comprend toutes les fonctionnalités mentionnées ci‑dessus:
``` cpp
...
## **Dégradé 3D**
Créer un dégradé 3D sur un modèle 3D PowerPoint peut être effectué via la méthode [Shape.get_FillFormat().get_GradientFormat()](https://reference.aspose.com/slides/cpp/class/aspose.slides.fill_format#a1f075336cb7a0e05cd5d7a706b6f4f58):
``` cpp
using namespace Aspose::Slides;
...
// .. configurer la 3D : Camera, LightRig, Extrusion
...
Oui. Le moteur 3D de Slides rend les effets 3D lors de l’exportation vers des formats pris en charge ([images](/slides/fr/cpp/convert-powerpoint-to-png/), [PDF](/slides/fr/cpp/convert-powerpoint-to-pdf/), [HTML](/slides/fr/cpp/convert-powerpoint-to-html/), etc.).

**Puis‑je récupérer les valeurs « effectives » (finales) des paramètres 3D qui tiennent compte des thèmes, de l’héritage, etc. ?**

Oui. Slides fournit des API pour [lire les valeurs effectives](/slides/fr/cpp/shape-effective-properties/) (y compris pour le 3D — éclairage, chanfreins, etc.) afin que vous puissiez voir les paramètres finaux appliqués.
