...
Aspose.Slides vous permet de configurer une collection de règles de polices de secours pour une présentation. Chaque règle de secours est représentée par la classe `FontFallBackRule` et peut être ajoutée à une `FontFallBackRulesCollection`, qui implémente l’interface `IFontFallBackRulesCollection`.

Après avoir créé la collection, vous pouvez l’assigner en utilisant la méthode `set_FontFallBackRulesCollection` du `FontsManager` de la présentation. Le `FontsManager` contrôle les polices dans toute la présentation, et chaque instance de `Presentation` possède son propre `FontsManager`.
...
{{% alert color="primary" %}} 
En savoir plus sur la façon de [Rendre la présentation avec une police de secours](/slides/fr/cpp/render-presentation-with-fallback-font/).
{{% /alert %}}
