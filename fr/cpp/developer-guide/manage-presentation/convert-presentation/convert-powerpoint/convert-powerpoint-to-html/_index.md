...
- C++
- Aspose.Slides
description: "Convertir des présentations PowerPoint en HTML responsive en C++. Conservez la mise en page, les liens et les images avec le guide de conversion Aspose.Slides pour des résultats rapides et impeccables."
---

## **Vue d'ensemble**

Cet article explique comment convertir une présentation PowerPoint au format HTML à l'aide de C++. Il couvre les sujets suivants.
...
Pour le code d'exemple C++ permettant de convertir PowerPoint en HTML, veuillez consulter la section ci-dessous, à savoir [Convertir PowerPoint en HTML](#convert-powerpoint-to-html). Le code peut charger plusieurs formats tels que PPT, PPTX et ODP dans l'objet Presentation et l'enregistrer au format HTML.
...
**Aspose.Slides** propose de nombreuses options (principalement de la classe [**HtmlOptions**](https://reference.aspose.com/slides/cpp/class/aspose.slides.export.html_options)) qui définissent le processus de conversion PowerPoint en HTML :
...
Aspose.Slides fournit la classe [ResponsiveHtmlController](https://reference.aspose.com/slides/cpp/class/aspose.slides.export.responsive_html_controller) qui permet de générer des fichiers HTML responsives. Ce code montre comment convertir une présentation PowerPoint en HTML responsive en C++ :
```cpp
// Instancier un objet Presentation qui représente un fichier de présentation
auto presentation = System::MakeObject<Presentation>(u"Convert_HTML.pptx");
...
Ce code montre comment convertir un PowerPoint en HTML avec notes en C++ :
```cpp
...
Ce code en C++ montre comment exporter du HTML avec des images en pleine qualité :
```cpp
auto pres = System::MakeObject<Presentation>(u"InputDoc.pptx");
...
Le code C++ de cet exemple montre comment utiliser des méthodes pouvant être remplacées pour créer un document HTML personnalisé avec un lien vers un fichier CSS :
``` cpp
...
Ce code C++ montre comment convertir un PowerPoint en HTML en liant toutes les polices et en excluant "Calibri" et "Arial" (puisqu'elles existent déjà dans le système) :
```cpp
...
Avec Aspose.Slides pour C++, vous pouvez exporter des fichiers multimédia de cette façon :
...
Il n’y a aucune limite au nombre de diapositives avec Aspose.Slides. Vous pouvez convertir des présentations de toute taille. Toutefois, pour des présentations contenant un très grand nombre de diapositives, les performances peuvent dépendre des ressources disponibles sur votre serveur ou système.