...
type: docs
weight: 10
url: /fr/python-net/developer-guide/presentation-content/powerpoint-table/manage-table/
keywords:
- ajouter table
- créer table
- accéder à la table
...
- Python
- Aspose.Slides
description: "Créez et modifiez des tables dans les diapositives PowerPoint et OpenDocument avec Aspose.Slides for Python via .NET. Découvrez des exemples de code simples pour simplifier vos flux de travail avec les tables."
---

## **Vue d'ensemble**

Une table dans PowerPoint est un moyen efficace de présenter des informations. Les informations organisées dans une grille de cellules (lignes et colonnes) sont simples et faciles à comprendre.

Aspose.Slides fournit la classe [Table](https://reference.aspose.com/slides/python-net/aspose.slides/table/), la classe [Cell](https://reference.aspose.com/slides/python-net/aspose.slides/cell/) et d'autres types associés pour vous aider à créer, mettre à jour et gérer des tables dans toute présentation.

## **Créer des tables à partir de zéro**

Cette section montre comment créer une table à partir de zéro dans Aspose.Slides en ajoutant une forme de table à une diapositive, en définissant ses lignes et colonnes, et en ajustant des tailles précises. Vous verrez également comment remplir les cellules avec du texte, ajuster l’alignement et les bordures, et personnaliser l’apparence de la table.
...
Dans une table standard, la numérotation des cellules est simple et commence à zéro. La première cellule d’une table porte l’indice (0, 0) (colonne 0, ligne 0).

Par exemple, dans une table de 4 colonnes et 4 lignes, les cellules sont numérotées ainsi :
...
{{% alert color="info" %}}
Si la diapositive contient plusieurs tables, il est préférable de rechercher la table dont vous avez besoin via sa propriété `alternative_text`.
{{% /alert %}}
...
Cette section montre comment appliquer un formatage du texte au niveau de la table dans Aspose.Slides afin que chaque cellule hérite d’un style cohérent et unifié. Vous apprendrez à définir la taille de police, les alignements et les marges globalement.
...
Aspose.Slides vous permet de formater les tables en utilisant des styles prédéfinis directement dans le code. L’exemple montre la création d’une table, l’application d’un style intégré, puis l’enregistrement du résultat — une méthode efficace pour garantir un formatage professionnel et cohérent.
...
## **Verrouiller le rapport d'aspect des tables**

Le rapport d’aspect d’une forme est le rapport entre ses dimensions. Aspose.Slides fournit la propriété `aspect_ratio_locked`, qui permet de verrouiller le rapport d’aspect pour les tables et les autres formes.

L’exemple Python suivant montre comment verrouiller le rapport d’aspect d’une table :
