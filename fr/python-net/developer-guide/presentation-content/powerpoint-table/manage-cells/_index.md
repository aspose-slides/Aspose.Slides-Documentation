...
Parfois, les bordures du tableau détournent l’attention du contenu ou créent un désordre visuel. Cette section montre comment supprimer les bordures des cellules sélectionnées — ou des côtés spécifiques d’une cellule — afin d’obtenir une mise en page plus épurée et mieux alignée avec le design de votre diapositive.
...
with slides.Presentation() as presentation:
    ...
    # Efface le remplissage de bordure pour chaque cellule.
...
Si vous fusionnez deux paires de cellules — par exemple (1, 1) × (2, 1) et (1, 2) × (2, 2) — le tableau résultant conserve le même numéro de cellule que le tableau sans fusion. Le code Python suivant illustre ce comportement :
...
with slides.Presentation() as presentation:
    ...
    # Fusionne les cellules (1,1) et (2,1).
    table.merge_cells(table.rows[1][1], table.rows[2][1], False)

    # Fusionne les cellules (1,2) et (2,2).
    table.merge_cells(table.rows[1][2], table.rows[2][2], False)

    # Affiche les indices des cellules.
    for row_index in range(len(table.rows)):
...
Dans l’exemple précédent, lorsque les cellules du tableau étaient fusionnées, la numérotation des autres cellules ne changeait pas. Cette fois‑ci, nous créons un tableau standard (sans cellules fusionnées) puis scindons la cellule (1, 1) pour obtenir un tableau spécial. Portez attention à la numérotation de ce tableau — elle peut sembler inhabituelle. Cependant, c’est ainsi que Microsoft PowerPoint numérote les cellules de tableau, et Aspose.Slides suit ce même comportement.
...
1. Appliquez l’image à la cellule du tableau et choisissez un mode de remplissage (par ex., `STRETCH`).
...
# Instancie un objet Presentation.
with slides.Presentation() as presentation:
    # Accède à la première diapositive.
    slide = presentation.slides[0]

    # Définit les largeurs de colonnes et les hauteurs de lignes.
    column_widths = [150, 150, 150, 150]
    ...
    # Charge l'image et l'ajoute à la présentation pour obtenir un PPImage.
    with slides.Images.from_file("image.png") as source_image:
        image = presentation.images.add_image(source_image)

    # Applique l'image à la première cellule du tableau.
    cell = table.rows[0][0]
...
Le comportement dépend du [mode de remplissage](https://reference.aspose.com/slides/python-net/aspose.slides/picturefillmode/) (stretch/tile). En mode étirement, l’image s’ajuste à la nouvelle taille de la cellule ; en mode carrelage, les carreaux sont recalculés. L’article décrit les modes d’affichage d’image dans une cellule.

**Puis‑je affecter un hyperlien à tout le contenu d’une cellule ?**

Les [hyperliens](/slides/fr/python-net/manage-hyperlinks/) sont définis au niveau du texte (portion) à l’intérieur du cadre texte de la cellule ou au niveau du tableau/forme entier. En pratique, vous affectez le lien à une portion ou à tout le texte de la cellule.

**Puis‑je définir des polices différentes au sein d’une même cellule ?**

Oui. Le cadre texte d’une cellule prend en charge les [portions](https://reference.aspose.com/slides/python-net/aspose.slides/portion/) (runs) avec un formatage indépendant — famille de police, style, taille et couleur.