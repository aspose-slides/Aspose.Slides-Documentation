...
- Python
- Aspose.Slides
description: "Créer et personnaliser des formes dans les présentations PowerPoint et OpenDocument avec Aspose.Slides pour Python via .NET : chemins de géométrie, coins arrondis, formes composites."
---
...
* déplacer le coin d’un carré vers l’intérieur ou vers l’extérieur,
* ajuster la courbure d’un coin ou d’un point,
* ajouter de nouveaux points au carré,
* modifier ses points.

Vous pouvez appliquer ces opérations à n’importe quelle forme. Avec **Edit Points**, vous pouvez modifier une forme ou en créer une nouvelle à partir d’une forme existante.

## **Conseils pour l'édition de formes**

!["Edit Points" command](custom_shape_0.png)

Avant de commencer à éditer les formes PowerPoint à l’aide de **Edit Points**, considérez ces notes sur les formes :

* Une forme (ou son chemin) peut être **fermée** ou **ouverte**.
* Une forme fermée n’a aucun point de départ ou d’arrivée ; une forme ouverte possède un début et une fin.
...
* Les points d’ancrage peuvent être **corner**, **smooth** ou **straight** :
  * Un point **corner** est l’endroit où deux segments droits se rencontrent sous un angle.
  * Un point **smooth** possède deux poignées colinéaires, et les segments adjacents forment une courbe lisse. Dans ce cas, les deux poignées sont à la même distance du point d’ancrage.
  * Un point **straight** possède également deux poignées colinéaires, et les segments adjacents forment une courbe lisse. Dans ce cas, les poignées n’ont pas besoin d’être à la même distance du point d’ancrage.
* En déplaçant ou en modifiant les points d’ancrage (modifiant ainsi les angles des segments), vous pouvez changer l’apparence de la forme.

Pour éditer les formes PowerPoint, Aspose.Slides fournit la classe [GeometryPath](https://reference.aspose.com/slides/python-net/aspose.slides/geometrypath/).

* Une instance de [GeometryPath](https://reference.aspose.com/slides/python-net/aspose.slides/geometrypath/) représente le chemin de géométrie d’un objet [GeometryShape](https://reference.aspose.com/slides/python-net/aspose.slides/geometryshape/).
...
* Utilisez les propriétés [GeometryPath.stroke](https://reference.aspose.com/slides/python-net/aspose.slides/geometrypath/stroke/) et [GeometryPath.fill_mode](https://reference.aspose.com/slides/python-net/aspose.slides/geometrypath/fill_mode/) pour contrôler l’apparence d’un chemin de géométrie.
* Utilisez la propriété [GeometryPath.path_data](https://reference.aspose.com/slides/python-net/aspose.slides/geometrypath/path_data/) pour récupérer le chemin de géométrie d’une forme sous forme de tableau de segments de chemin.
...
**Fermer la figure courante** dans un chemin :
...
Parfois, une forme personnalisée contient des points inutiles qui compliquent sa géométrie ou affectent son rendu. Cette section montre comment supprimer des points spécifiques du chemin d’une forme afin de simplifier le contour et d’obtenir des résultats plus propres et plus précis.
...
## **Créer des formes composées personnalisées**

Créer une forme personnalisée composite vous permet de combiner plusieurs chemins de géométrie en une seule forme réutilisable sur une diapositive. Définissez et fusionnez ces chemins pour construire des visuels complexes qui dépassent l’ensemble de formes standard.
...
Cette section montre comment dessiner une forme personnalisée avec des coins arrondis fluides à l’aide d’un chemin de géométrie. Vous combinerez des segments droits et des arcs circulaires pour former le contour et ajouterez la forme finale à votre diapositive.
...
Une forme fermée est définie comme celle dont tous les côtés se rejoignent, formant une seule frontière sans lacunes. Une telle forme peut être une forme géométrique simple ou un contour personnalisé complexe. L’exemple de code suivant montre comment vérifier si la géométrie d’une forme est fermée :
...
Utilisez la propriété [rotation](https://reference.aspose.com/slides/python-net/aspose.slides/geometryshape/rotation/) de la forme ; la géométrie tourne avec la forme car elle est liée au système de coordonnées de la forme elle‑même.

**Puis-je convertir une forme personnalisée en image pour « verrouiller » le résultat ?**

Oui. Exportez la zone de la [slide](/slides/fr/python-net/convert-powerpoint-to-png/) ou la [shape](/slides/fr/python-net/create-shape-thumbnails/) elle‑même vers un format raster ; cela simplifie le travail ultérieur avec des géométries lourdes.