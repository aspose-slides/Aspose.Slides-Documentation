...
- Python
- Aspose.Slides
description: "Créez et personnalisez des formes dans les présentations PowerPoint et OpenDocument avec Aspose.Slides for Python via .NET : chemins géométriques, coins arrondis, formes composites."
---
...
Avant de commencer l'édition des formes PowerPoint avec **Edit Points**, considérez ces notes concernant les formes :
...
* Les points d'ancrage peuvent être **corner**, **smooth** ou **straight** :
  * Un point **corner** est le point où deux segments droits se rencontrent sous un angle.
  * Un point **smooth** possède deux poignées collinéaires, et les segments adjacents forment une courbe lisse. Dans ce cas, les deux poignées sont à la même distance du point d'ancrage.
  * Un point **straight** possède également deux poignées collinaires, mais les segments adjacents forment une courbe lisse. Dans ce cas, les poignées n'ont pas besoin d'être à la même distance du point d'ancrage.
* En déplaçant ou en éditant les points d'ancrage (et ainsi en modifiant les angles des segments), vous pouvez changer l'apparence de la forme.
...
* Utilisez la propriété [GeometryPath.path_data](https://reference.aspose.com/slides/python-net/aspose.slides/geometrypath/path_data/) pour récupérer le chemin géométrique d’une forme sous forme de tableau de segments.
...
**Supprimer le segment du chemin** à un indice donné :
...
Dans cette section, vous apprendrez à définir une forme libre en ajoutant votre propre séquence de points. En spécifiant des points ordonnés et des types de segment (droit ou courbe) et éventuellement en fermant le chemin, vous pouvez dessiner des graphiques personnalisés précis — polygones, icônes, bulles d'appel ou logos—directement sur vos diapositives.
...
2. Obtenez une instance de [GeometryPath](https://reference.aspose.com/slides/python-net/aspose.slides/geometrypath/) depuis la forme.
...
Il arrive qu'une forme personnalisée contienne des points inutiles qui compliquent sa géométrie ou affectent son rendu. Cette section montre comment supprimer des points spécifiques du chemin d'une forme afin de simplifier le contour et d'obtenir des résultats plus propres et plus précis.
...
## **Créer des formes composites personnalisées**

Créer une forme composite personnalisée vous permet de combiner plusieurs chemins géométriques en une seule forme réutilisable sur une diapositive. Définissez et fusionnez ces chemins pour créer des visuels complexes qui dépassent l'ensemble de formes standard.
...
Cette section montre comment dessiner une forme personnalisée avec des coins arrondis de manière fluide en utilisant un chemin géométrique. Vous combinerez des segments droits et des arcs circulaires pour former le contour et ajouter la forme finalisée à votre diapositive.
...
Utilisez la propriété [rotation](https://reference.aspose.com/slides/python-net/aspose.slides/geometryshape/rotation/) de la forme ; la géométrie tourne avec la forme car elle est liée au système de coordonnées propre à la forme.

**Puis-je convertir une forme personnalisée en image pour « verrouiller » le résultat ?**

Oui. Exportez la zone de [diapositive](/slides/fr/python-net/convert-powerpoint-to-png/) requise ou la [forme](/slides/fr/python-net/create-shape-thumbnails/) elle‑même vers un format raster ; cela simplifie le travail ultérieur avec des géométries lourdes.