---
title: Appliquer des animations de formes dans les présentations avec Python
...
- Python
- Aspose.Slides
description: "Découvrez comment créer et personnaliser des animations de formes dans les présentations PowerPoint et OpenDocument avec Aspose.Slides for Python via .NET. Démarquez‑vous !"
---

Les animations sont des effets visuels qui peuvent être appliqués aux textes, images, formes ou aux [diagrammes](/slides/fr/python-net/animated-charts/). Elles donnent vie aux présentations ou à leurs constituants. 
...
PowerPoint propose de nombreuses options et outils pour les animations et effets d'animation dans les catégories **entrée**, **sortie**, **mise en évidence** et **chemins de mouvement**. 

## **Animations dans Aspose.Slides**

* Aspose.Slides fournit les classes et types dont vous avez besoin pour travailler avec les animations dans l'espace de noms [Aspose.Slides.Animation](https://reference.aspose.com/slides/python-net/aspose.slides.animation/).  
...
3. Ajouter une [IAutoShape] `rectangle`.  
4. Ajouter du texte au `IAutoShape.TextFrame`.  
5. Obtenir la séquence principale d'effets.  
6. Ajouter un effet d'animation à [IAutoShape](https://reference.aspose.com/slides/python-net/aspose.slides/iautoshape/).  
7. Définir la propriété `TextAnimation.BuildType` sur la valeur de l'énumération `BuildType`.  
...
En plus d'appliquer des animations au texte, vous pouvez également appliquer des animations à un seul [Paragraph](https://reference.aspose.com/slides/python-net/aspose.slides/iparagraph/). Voir [**Texte animé**](/slides/fr/python-net/animated-text/).
...
1. Créer une instance de la classe [Presentation](https://reference.aspose.com/slides/python-net/aspose.slides/presentation/).  
2. Obtenir une référence à une diapositive via son indice.  
3. Ajouter ou obtenir un [PictureFrame](https://reference.aspose.com/slides/python-net/aspose.slides/pictureframe/) sur la diapositive.  
4. Obtenir la séquence principale d'effets.  
5. Ajouter un effet d'animation à [PictureFrame](https://reference.aspose.com/slides/python-net/aspose.slides/pictureframe/).  
6. Enregistrer la présentation sur le disque au format PPTX.  
...
2. Obtenir une référence à une diapositive via son indice.  
3. Ajouter une [IAutoShape] `rectangle`.  
4. Ajouter un `Bevel` [IAutoShape] (lorsque cet objet est cliqué, l'animation se lance).  
5. Créer une séquence d'effets sur la forme à biseau.  
6. Créer un `UserPath` personnalisé.  
7. Ajouter des commandes de déplacement vers le `UserPath`.  
8. Enregistrer la présentation sur le disque au format PPTX.  
...
Les exemples suivants montrent comment utiliser la méthode `get_effects_by_shape` de la classe [Sequence](https://reference.aspose.com/slides/python-net/aspose.slides.animation/sequence/) pour obtenir tous les effets d'animation appliqués à une forme.
...
Cette fenêtre de **Timing** des animations dans Microsoft PowerPoint :

![example1_image](shape-animation.png)

Correspondances entre le minutage PowerPoint et les propriétés `Effect.Timing` :

- La liste déroulante **Start** du minutage PowerPoint correspond à la propriété [Effect.Timing.TriggerType](https://reference.aspose.com/slides/python-net/aspose.slides.animation/effecttriggertype/).  
- **Duration** du minutage PowerPoint correspond à la propriété `Effect.Timing.Duration`. La durée d'une animation (en secondes) est le temps total nécessaire à l'animation pour compléter un cycle.  
- **Delay** du minutage PowerPoint correspond à la propriété `Effect.Timing.TriggerDelayTime`.  

Voici comment modifier les propriétés de synchronisation :
...
Cette fenêtre d'effet d'animation et le menu étendu dans Microsoft PowerPoint :

![example1_image](shape-after-animation.png)

La liste déroulante **After animation** de l'effet PowerPoint correspond aux propriétés suivantes :

- la propriété `after_animation_type` qui décrit le type d'After animation :  
  ...
  * **Don't Dim** correspond au type [DO_NOT_DIM](https://reference.aspose.com/slides/python-net/aspose.slides.animation/afteranimationtype/) (type d'After animation par défaut).  
...
- la propriété `after_animation_color` qui définit le format de couleur après l'animation. Cette propriété fonctionne en conjonction avec le type [COLOR]. Si vous changez le type, la couleur après l'animation sera réinitialisée.  
...
- `delay_between_text_parts` définit un délai entre les parties de texte animées (mots ou lettres). Une valeur positive indique le pourcentage de la durée de l'effet. Une valeur négative indique le délai en secondes.  

Voici comment modifier les propriétés *Animate text* d’un effet :

1. [Appliquer](#apply-animation-to-shape) ou récupérer l'effet d'animation.  
2. Définir la propriété `build_type` sur la valeur [AS_ONE_OBJECT] pour désactiver le mode d'animation *Par paragraphes*.  
3. Attribuer de nouvelles valeurs aux propriétés `animate_text_type` et `delay_between_text_parts`.  
...
[Export to HTML5](/slides/fr/python-net/export-to-html5/) et activez les [options](/reference.aspose.com/slides/python-net/aspose.slides.export/html5options/) responsables des animations de [shape](/reference.aspose.com/slides/python-net/aspose.slides.export/html5options/animate_shapes/) et de [transition](/reference.aspose.com/slides/python-net/aspose.slides.export/html5options/animate_transitions/). Le HTML simple ne lit pas les animations de diapositive, alors que le HTML5 le fait.

**Comment la modification de l'ordre Z (ordre des calques) des formes affecte-t-elle l'animation ?**

L'ordre Z et l'ordre de dessin sont indépendants : un effet contrôle le moment et le type d'apparition/disparition, tandis que l'**order Z** détermine ce qui couvre quoi. Le résultat visible est défini par leur combinaison. (C’est le comportement général de PowerPoint ; le modèle d’effets et de formes d’Aspose.Slides suit la même logique.)

**Existe-t-il des limitations lors de la conversion des animations en vidéo pour certains effets ?**

En général, les [animations sont supportées](/slides/fr/python-net/convert-powerpoint-to-video/), mais des cas rares ou des effets spécifiques peuvent être rendus différemment. Il est recommandé de tester avec les effets que vous utilisez et avec la version de la bibliothèque.