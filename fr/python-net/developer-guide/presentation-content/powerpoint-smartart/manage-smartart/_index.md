...
- Python
- Aspose.Slides
description: "Apprenez à créer et modifier des SmartArt PowerPoint avec Aspose.Slides pour Python via .NET grâce à des exemples de code clairs qui accélèrent la conception de diapositives et l’automatisation."
---

## **Vue d’ensemble**

Ce guide montre comment créer et manipuler des SmartArt dans Aspose.Slides pour Python. Vous apprendrez comment extraire le texte d’un SmartArt (y compris le contenu du [TextFrame](https://reference.aspose.com/slides/python-net/aspose.slides/textframe/) à l’intérieur des formes de nœuds), ajouter du SmartArt aux diapositives et changer sa disposition, détecter et gérer les nœuds cachés, configurer les dispositions d’organigrammes, et créer des organigrammes image — le tout avec des exemples Python courts, prêts à copier‑coller, qui ouvrent une [Presentation](https://reference.aspose.com/slides/python-net/aspose.slides/presentation/), travaillent avec les diapositives et les nœuds SmartArt, puis enregistrent le résultat au format PPTX.
...
## **Modifier le type de mise en page du SmartArt**

Pour modifier le type de mise en page du SmartArt, suivez ces étapes :
...
with slides.Presentation() as presentation:
    ...
    # Ajoutez une forme SmartArt avec la disposition BASIC_BLOCK_LIST.
    smart = slide.shapes.add_smart_art(10, 10, 400, 300, smartart.SmartArtLayoutType.BASIC_BLOCK_LIST)

    # Changez le type de disposition en BASIC_PROCESS.
...
La propriété `SmartArtNode.is_hidden` renvoie `True` si le nœud est caché dans le modèle de données. Pour vérifier si un nœud SmartArt est caché, suivez ces étapes :
...
with slides.Presentation() as presentation:
    ...
    # Ajoutez une forme SmartArt avec la disposition RADIAL_CYCLE.
...
with slides.Presentation() as presentation:
    ...
    # Ajoutez une forme SmartArt avec la disposition ORGANIZATION_CHART.
...
Oui. La propriété [is_reversed](https://reference.aspose.com/slides/python-net/aspose.slides.smartart/smartart/is_reversed/) inverse la direction du diagramme (LTR/RTL) si le type de SmartArt sélectionné supporte l’inversion.
...
[Renderisez la diapositive](/slides/fr/python-net/convert-powerpoint-to-png/) (ou la présentation complète) en PNG/JPEG via l’API qui convertit les diapositives/présentations en images — le SmartArt sera dessiné comme partie de la diapositive.
