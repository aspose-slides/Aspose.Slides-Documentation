...
Pour les cellules de classeur qui représentent des données manquantes, consultez [Contrôler l'affichage des cellules vides](/slides/fr/python-net/chart-series/) pour connaître la différence entre une cellule vide et zéro, ainsi qu'une comparaison en graphique en ligne des modes d'affichage disponibles.

## **Lire et écrire des données de graphique depuis un classeur**

Aspose.Slides fournit des méthodes pour lire et écrire des classeurs de données de graphique (qui contiennent les données de graphique modifiées avec Aspose.Cells). **Remarque :** Les données du graphique doivent être organisées de la même manière ou présenter une structure similaire à la source.
...
Effacer les collections garantit que la structure des données du graphique est cohérente avec le nouveau classeur, permettant à `validate_chart_layout` de se terminer sans erreurs.

## **Définir une cellule de classeur comme libellé de données du graphique**

Parfois, vous avez besoin de libellés de graphique provenant directement des cellules du classeur de données sous‑jacent. Aspose.Slides vous permet de lier des libellés de données à des cellules de classeur spécifiques afin que le texte du libellé reflète toujours la valeur de la cellule. L'exemple ci‑dessous montre comment activer les libellés basés sur la valeur de la cellule et pointer les libellés sélectionnés vers des cellules personnalisées dans le classeur du graphique.
...
Le code Python suivant montre comment définir une cellule de classeur comme libellé de données du graphique :
```py
import aspose.slides as slides
...
Bien que vous ne puissiez pas modifier les données des classeurs stockés sur des emplacements ou ressources distants, vous pouvez toujours les utiliser comme sources de données externes. Si vous fournissez un chemin relatif pour un classeur externe, il est automatiquement converti en chemin complet.
...
with slides.Presentation() as presentation:
    ...
    # Passez False pour que seul le chemin soit stocké : le classeur cible n'a pas encore besoin d'exister.
...
- Lorsque `update_chart_data` est défini sur `False`, seul le chemin du classeur est mis à jour ; les données du graphique ne sont pas chargées ni rafraîchies depuis le classeur cible. Utilisez ce paramètre lorsque le classeur cible n'existe pas ou n'est pas disponible.
- Lorsque `update_chart_data` est défini sur `True` (par défaut), les données du graphique sont chargées et mises à jour depuis le classeur cible. Si ce classeur ne peut pas être ouvert, une exception avec le message "External workbook is not available" est levée.

### **Créer des classeurs externes**

En utilisant les méthodes [read_workbook_stream](https://reference.aspose.com/slides/fr/python-net/aspose.slides.charts/chartdata/read_workbook_stream/) et [set_external_workbook](https://reference.aspose.com/slides/fr/python-net/aspose.slides.charts/chartdata/set_external_workbook/), vous pouvez créer un classeur externe à partir de zéro ou convertir un classeur interne en classeur externe.
...
Parfois, les données d'un graphique sont liées à un classeur Excel externe plutôt qu'aux données intégrées de la présentation. Avec Aspose.Slides, vous pouvez examiner la source de données du graphique et, si c’est un classeur externe, lire le chemin complet du classeur.

1. Créez une instance de la classe [Presentation](https://docs.aspose.com/slides/fr/python-net/api-reference/aspose.slides/presentation/).
2. Obtenez une référence à la diapositive par son indice.
3. Obtenez une référence à la forme du graphique.
4. Obtenez la source ([ChartDataSourceType](https://reference.aspose.com/slides/fr/python-net/aspose.slides.charts/chartdatasourcetype/)) qui représente la source de données du graphique.
5. Vérifiez si le type de source correspond au type de source de données du classeur externe.
...
Vous pouvez modifier les données dans des classeurs externes de la même manière que dans des classeurs internes. Si un classeur externe ne peut pas être chargé, une exception est levée.
```python
import aspose.slides as slides
...
Seulement si vous avez modifié les données du graphique. La présentation stocke un [lien vers le fichier externe](https://reference.aspose.com/slides/fr/python-net/aspose.slides.charts/chartdata/external_workbook_path/) et l'utilise pour lire les données, ainsi l'ouverture et l'enregistrement d'une présentation laissent le classeur intact. Cependant, les valeurs que vous modifiez via les données du graphique (voir [Modifier les données du graphique](#edit-chart-data) ci‑dessus) sont réécrites dans le classeur externe lors de l'enregistrement de la présentation — travaillez sur une copie si l'original doit rester intact.

**Que faire si le fichier externe est protégé par un mot de passe ?**

Aspose.Slides n’accepte pas de mot de passe lors de la création du lien. Une approche courante consiste à enlever la protection au préalable ou à préparer une copie déchiffrée (par exemple, en utilisant [Aspose.Cells](/cells/python-net/)) et à créer le lien vers cette copie.
