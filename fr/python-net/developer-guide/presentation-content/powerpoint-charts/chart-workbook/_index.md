...
- Python
- Aspose.Slides
description: "Découvrez Aspose.Slides pour Python via .NET : gérez facilement les classeurs de graphiques dans PowerPoint et les formats OpenDocument pour rationaliser les données de votre présentation."
---

## **Définir les données du graphique à partir d’un classeur**

Aspose.Slides fournit des méthodes pour lire et écrire les classeurs de données de graphiques (qui contiennent des données de graphique éditées avec Aspose.Cells). **Remarque :** les données du graphique doivent être organisées de la même manière ou avoir une structure similaire à la source.
...
Parfois, vous avez besoin d’étiquettes de graphique provenant directement des cellules du classeur de données sous‑jacent. Aspose.Slides vous permet de lier les étiquettes de données à des cellules de classeur spécifiques afin que le texte de l’étiquette reflète toujours la valeur de la cellule. L’exemple ci‑dessous montre comment activer les étiquettes « valeur‑à‑partir‑de‑cellule » et associer des étiquettes sélectionnées à des cellules personnalisées dans le classeur du graphique.
...
5. Utilisez une cellule de classeur comme étiquette de données.
6. Enregistrez la présentation.

Le code Python suivant montre comment définir une cellule de classeur comme étiquette de données du graphique :
...
En utilisant la méthode [ChartData.set_external_workbook](https://reference.aspose.com/slides/python-net/aspose.slides.charts/chartdata/set_external_workbook/), vous pouvez affecter un classeur externe à un graphique comme source de données. Cette méthode peut également mettre à jour le chemin d’accès à un classeur externe s’il a été déplacé.

Bien que vous ne puissiez pas modifier les données dans des classeurs stockés sur des emplacements ou ressources distants, vous pouvez toujours les utiliser comme sources de données externes. Si vous fournissez un chemin relatif pour un classeur externe, il est automatiquement converti en chemin complet.
...
- Lorsque `update_chart_data` est à `False`, seul le chemin du classeur est mis à jour ; les données du graphique ne sont pas chargées ou actualisées à partir du classeur cible. Utilisez ce réglage lorsque le classeur cible n’existe pas ou n’est pas disponible.
- Lorsque `update_chart_data` est à `True`, les données du graphique sont chargées et mises à jour à partir du classeur cible.
...
4. Récupérez la source ([ChartDataSourceType](https://reference.aspose.com/slides/python-net/aspose.slides.charts/chartdatasourcetype/)) qui représente la source de données du graphique.
...
Vous pouvez modifier les données dans les classeurs externes de la même façon que dans les classeurs internes. Si un classeur externe ne peut pas être chargé, une exception est levée.
...
**Puis‑je déterminer si un graphique spécifique est lié à un classeur externe ou embarqué ?**
...
Non. La présentation stocke un [lien vers le fichier externe](https://reference.aspose.com/slides/python-net/aspose.slides.charts/chartdata/external_workbook_path/) et l’utilise pour lire les données. Le fichier externe lui‑même n’est pas modifié lors de l’enregistrement de la présentation.
