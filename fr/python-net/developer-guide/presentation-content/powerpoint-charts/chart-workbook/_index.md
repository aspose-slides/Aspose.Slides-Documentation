...
type: docs
weight: 70
url: /fr/python-net/developer-guide/presentation-content/powerpoint-charts/chart-workbook/
keywords:
...
1. Utilisez une cellule du classeur comme étiquette de données.
1. Enregistrez la présentation.

Le code Python suivant montre comment définir une cellule du classeur comme étiquette de données du graphique :
...
- Lorsque `update_chart_data` est réglé sur `False`, seul le chemin du classeur est mis à jour ; les données du graphique ne sont pas chargées ou rafraîchies depuis le classeur cible. Utilisez ce paramètre lorsque le classeur cible n’existe pas ou est indisponible.  
- Lorsque `update_chart_data` est réglé sur `True`, les données du graphique sont chargées et mises à jour depuis le classeur cible.
...
Oui. Si vous indiquez un chemin relatif, il est automatiquement converti en chemin absolu. Ceci est pratique pour la portabilité du projet ; cependant, la présentation stockera le chemin absolu dans le fichier PPTX.
...
Non. La présentation stocke un [lien vers le fichier externe](https://reference.aspose.com/slides/python-net/aspose.slides.charts/chartdata/external_workbook_path/) et l’utilise pour lire les données. Le fichier externe n’est pas modifié lors de l’enregistrement de la présentation.

**Que faire si le fichier externe est protégé par mot de passe ?**

Aspose.Slides n’accepte pas de mot de passe lors de la liaison. Une approche courante consiste à supprimer la protection au préalable ou à préparer une copie décryptée (par exemple, en utilisant [Aspose.Cells](/cells/python-net/)) et à créer le lien vers cette copie.
