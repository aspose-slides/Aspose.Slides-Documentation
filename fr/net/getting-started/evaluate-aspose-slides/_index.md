...
La version d'évaluation d'Aspose.Slides (sans licence spécifiée) offre l'intégralité des fonctionnalités du produit, mais elle insère un filigrane d'évaluation en haut du document à l'ouverture et à l'enregistrement. Vous êtes également limité à une diapositive lors de l'extraction de texte à partir des diapositives de présentation.
...
Voici les « quelques lignes de code » qui transforment le package d'évaluation en version sous licence. Appliquez la licence une fois au démarrage de l'application, avant la création de tout objet `Presentation` — une présentation construite auparavant conserve le filigrane d'évaluation.
...
`SetLicense` accepte également un `Stream`, ce qui est l'option préférable lorsque la licence est fournie en tant que ressource intégrée plutôt qu'un fichier sur disque. Si le chemin est incorrect ou que le fichier a expiré, l'appel lève une exception, de sorte que les échecs apparaissent immédiatement au démarrage au lieu de revenir silencieusement en mode évaluation.
...
Oui. Vous pouvez traiter différents documents en parallèle ; vous ne devez pas partager le même objet de présentation [across threads](/slides/fr/net/multithreading/). Le mode évaluation n'affecte pas cela.
...
Oui. Les [converters](/slides/fr/net/convert-presentation/) fonctionnent ; la sortie inclura un filigrane.
