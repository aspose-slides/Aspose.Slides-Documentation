...
Cette page répertorie toutes les classes, méthodes, propriétés, etc. [ajoutés](/slides/fr/net/public-api-and-backwards-incompatible-changes-in-aspose-slides-for-net-16-2-0/) ou [supprimés](/slides/fr/net/public-api-and-backwards-incompatible-changes-in-aspose-slides-for-net-16-2-0/) introduits avec l'API Aspose.Slides for .NET 16.2.0.
...
CategoryAxisType.Auto - le type d'axe des catégories sera déterminé automatiquement lors de la sérialisation (ce comportement n'est pas encore implémenté)  
...
L'argument enum ExtractionMode indique le mode d'organisation du résultat texte et peut être défini aux valeurs suivantes :
Unarranged - Le texte brut sans tenir compte de la position sur la diapositive  
Arranged - Le texte est placé dans le même ordre que sur la diapositive  
...
La nouvelle classe fournit des méthodes pour convertir le diagramme hérité en un objet SmartArt moderne éditable ou en un GroupShape éditable.  

#### **Un nouveau membre JustifyLow ajouté à l'énumération Aspose.Slides.TextAlignment**
...
De nouvelles propriétés ont été ajoutées à l'interface IOleObjectFrame et à la classe OleObjectFrame qui implémente cette interface. Ces propriétés sont utilisées pour fournir des informations sur un objet intégré à la présentation :
...
La propriété ShowLabelAsDataCallout détermine si le libellé de données spécifié du graphique sera affiché comme annotation de données ou comme libellé de données.
