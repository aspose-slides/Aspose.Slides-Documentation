...
La classe Aspose.Slides.SmartArt.SmartArtNodeCollection (et l'interface associée Aspose.Slides.SmartArt.ISmartArtNodeCollection) héritent de l'interface générique IEnumerable<ISmartArtNode> ainsi que de l'interface ICollection.  
#### **Valeur d'énumération SmartArtLayoutType.Custom ajoutée**
Le type de mise en page SmartArt personnalisé représente un diagramme avec un modèle personnalisé. Les diagrammes personnalisés ne peuvent être chargés qu'à partir d'un fichier de présentation et ne peuvent pas être créés via la méthode ShapeCollection.AddSmartArt(x, y, largeur, hauteur, SmartArtLayoutType.Custom).  
...
**Remarque** : SmartArtShape ne prend pas en charge les propriétés IShape RawFrame, Frame, Rotation, X, Y, Width, Height et lève une System.NotSupportedException lorsqu'on tente d’y accéder.
...
**Remarque** : selon le SmartArtLayoutType, un SmartArtShape peut être partagé entre plusieurs nœuds.
...
{{% /alert %}} 
#### **Méthodes d’enregistrement des diapositives en conservant les numéros de page ajoutées**
...
Ces méthodes permettent aux développeurs d’enregistrer des diapositives spécifiques d’une présentation au format PDF, XPS, TIFF, HTML. Le tableau 'slides' sert à spécifier les numéros de pages, à partir de 1.
