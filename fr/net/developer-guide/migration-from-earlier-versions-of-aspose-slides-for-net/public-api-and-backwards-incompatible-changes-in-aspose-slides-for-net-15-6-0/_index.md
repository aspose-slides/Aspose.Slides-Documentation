...
#### **Les membres IDocumentProperties.Count, .GetPropertyName(int index), .Remove(string name), .Contains(string name) ont été marqués comme obsolètes et leurs remplacements ont été introduits à la place.**
...
La méthode ICommentAuthor.Remove a été ajoutée pour supprimer un auteur de commentaires de la collection.
...
La méthode ICommentAuthorCollection.RemoveAt a été ajoutée pour supprimer un auteur selon l'indice spécifié.
