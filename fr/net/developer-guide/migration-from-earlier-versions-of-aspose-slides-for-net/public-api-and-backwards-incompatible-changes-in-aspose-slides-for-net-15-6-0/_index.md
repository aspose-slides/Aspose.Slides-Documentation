---
title: API publique et modifications incompatibles avec les versions antérieures dans Aspose.Slides pour .NET 15.6.0
...
Cette page répertorie toutes les [ajoutées](/slides/fr/net/public-api-and-backwards-incompatible-changes-in-aspose-slides-for-net-15-6-0/) ou [supprimées](/slides/fr/net/public-api-and-backwards-incompatible-changes-in-aspose-slides-for-net-15-6-0/) classes, méthodes, propriétés, etc., et d’autres changements introduits avec l’API Aspose.Slides for .NET 15.6.0.
...
La propriété IDocumentProperties.Count et les méthodes IDocumentProperties.GetPropertyName(int index), .Remove(string name), .Contains(string name) ont été marquées comme obsolètes.  
La propriété IDocumentProperties.CountOfCustomProperties et les méthodes IDocumentProperties.GetCustomPropertyName(int index), .RemoveCustomProperty(string name), .ContainsCustomProperty(string name) ont été ajoutées à la place.
...
La méthode IDocumentProperties.ClearBuiltInProperties a été ajoutée pour supprimer et rétablir les valeurs par défaut de toutes les propriétés intégrées du document (Company, Subject, Author, etc.).
#### **Les méthodes RemoveAt, Remove et Clear ont été ajoutées à ICommentAuthorCollection**
La méthode ICommentAuthorCollection.RemoveAt a été ajoutée pour supprimer un auteur selon l’index spécifié.  
La méthode ICommentAuthorCollection.Remove a été ajoutée pour supprimer l’auteur spécifié de la collection.  
...
La propriété IDocumentProperties.AppVersion a été ajoutée pour obtenir la propriété intégrée du document qui représente les numéros de version internes utilisés par Microsoft lors du développement.
...
| :- | :- |
|Color |Rendu avec coloration normale |
...
|GrayWhite |Rendu avec coloration gris et blanc |
|BlackGray |Rendu avec coloration noir et gris |
|BlackWhite |Rendu avec coloration noir et blanc |
|Black |Rendu uniquement avec coloration noire |
|White |Rendu avec coloration blanche |
|Hidden |Non rendu |
|NotDefined|signifie que la propriété n’est pas définie|
