...
La propriété Aspose.Slides.IShape.UniqueId fournit un identifiant de forme unique dans le scope d’une présentation. Ces identifiants uniques sont stockés dans les balises personnalisées de la forme.
...
est maintenant obsolète et remplacée par la signature
...
doivent être modifiés en appels tels que

``` csharp
 .SetGroupingItem(1, "Group 1");
``` 

Passez une valeur comme "Group 1" à SetGroupingItem mais pas une valeur de type IChartDataCell. La création d’un IChartDataCell avec une feuille de calcul, une ligne et une colonne définies pour les niveaux de catégorie doit répondre à certaines exigences et a été encapsulée dans la méthode SetGroupingItem(int, object).
