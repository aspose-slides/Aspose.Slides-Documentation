...
Aspose.Slides for .NET fournit une API simple pour gérer les valeurs personnalisées des barres d'erreur. Le code d'exemple s'applique lorsque la propriété **IErrorBarsFormat.ValueType** est égale à **Custom**. Pour spécifier une valeur, utilisez la propriété **ErrorBarCustomValues** d'un point de données spécifique dans la collection **DataPoints** de la série :
