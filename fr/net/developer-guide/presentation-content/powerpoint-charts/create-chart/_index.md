...
- C#
- Aspose.Slides
description: "Apprenez comment créer et personnaliser des graphiques dans les présentations PowerPoint et OpenDocument à l'aide d'Aspose.Slides pour .NET. Cela couvre l'ajout, le formatage et la modification de graphiques dans les présentations avec des exemples de code pratiques en C#."
---

## **Vue d'ensemble**

Cet article fournit un guide complet sur la façon de créer et de personnaliser des graphiques à l'aide d'Aspose.Slides pour .NET. Vous apprendrez à ajouter programmatique un graphique à une diapositive, à le remplir avec des données et à appliquer diverses options de mise en forme pour répondre à vos exigences de conception spécifiques. Tout au long de l'article, des exemples de code détaillés illustrent chaque étape, depuis l'initialisation de la présentation et de l'objet graphique jusqu'à la configuration des séries, des axes et des légendes. En suivant ce guide, vous acquerrez une compréhension solide de l'intégration de la génération dynamique de graphiques dans vos applications .NET, simplifiant le processus de création de présentations basées sur des données.
...
* exposer les motifs et tendances dans les données ;
...
{{% alert color="primary" %}} 
Utilisez l'énumération [ChartType](https://reference.aspose.com/slides/net/aspose.slides.charts/charttype/) sous l'espace de noms [Aspose.Slides.Charts](https://reference.aspose.com/slides/net/aspose.slides.charts/). Les valeurs de cette énumération correspondent à différents types de graphiques.
{{% /alert %}} 
...
Les graphiques en nuage de points (également appelés nuages de points ou graphiques x‑y) sont souvent utilisés pour rechercher des motifs ou démontrer des corrélations entre deux variables.
...
Ce code C# montre comment créer un graphique en nuage de points avec une série de repères différente :
```c#
// Instancier la classe Presentation.
using (Presentation presentation = new Presentation())
{
...
Les graphiques circulaires sont les mieux adaptés pour montrer la relation partie‑à‑tout dans les données, en particulier lorsque les données contiennent des libellés catégoriels avec des valeurs numériques. Cependant, si vos données contiennent de nombreuses parties ou libellés, vous pourriez envisager d'utiliser un graphique à barres à la place.
...
1. Activez les traits de raccordement pour les étiquettes de séries.
...
using (Presentation presentation = new Presentation())
{
    ...
    // Définir la série pour afficher les lignes de liaison du graphique.
...
Par défaut, les points d'un graphique linéaire sont reliés par des lignes continues droites. Si vous souhaitez que les points soient reliés par des tirets, vous pouvez spécifier le type de tiret souhaité comme suit :
```c#
...
Les graphiques en carte arborescente sont les mieux adaptés aux données de ventes lorsqu'il s'agit de montrer la taille relative des catégories de données et d'attirer rapidement l'attention sur les éléments qui sont de grands contributeurs au sein de chaque catégorie.
...
Les graphiques boursiers sont utilisés pour afficher des données financières telles que les cours d'ouverture, de haut, de bas et de clôture, aidant à analyser les tendances du marché et la volatilité. Ils offrent des informations essentielles sur la performance des actions, aidant les investisseurs et les analystes à prendre des décisions éclairées.
...
Les graphiques en entonnoir sont utilisés pour visualiser des processus comportant plusieurs étapes séquentielles, où le volume des données diminue au fur et à mesure qu'il passe d'une étape à la suivante. Ils sont particulièrement utiles pour analyser les taux de conversion, identifier les goulots d'étranglement et suivre l'efficacité des processus de vente ou de marketing.
...
Les graphiques en rayons (Sunburst) sont utilisés pour visualiser des données hiérarchiques, affichant les niveaux sous forme d'anneaux concentriques. Ils aident à illustrer les relations partie‑à‑tout et sont idéaux pour représenter des catégories et sous‑catégories imbriquées de manière claire et compacte.
...
Les graphiques radar sont utilisés pour afficher des données multivariées dans un format bidimensionnel, permettant une comparaison facile de plusieurs variables simultanément. Ils sont particulièrement utiles pour identifier des motifs, des points forts et faibles à travers plusieurs indicateurs de performance ou attributs.
...
Les graphiques à multi‑catégories sont utilisés pour afficher des données impliquant plusieurs regroupements catégoriels, vous permettant de comparer des valeurs sur plusieurs dimensions simultanément. Ils sont particulièrement utiles lorsque vous devez analyser les tendances et les relations au sein de jeux de données complexes et à plusieurs niveaux.
...
Un graphique combiné (ou combo) combine deux types de graphiques ou plus dans un même diagramme. Ce graphique vous permet de mettre en évidence, comparer ou examiner les différences entre deux ou plusieurs ensembles de données, aidant à identifier les relations entre eux.

![The combination chart](combination_chart.png)

Le code C# suivant montre comment créer le graphique combiné affiché ci‑dessus dans une présentation PowerPoint :
```c#
...
Aspose.Slides pour .NET vous permet de mettre à jour les graphiques PowerPoint en modifiant les données, la mise en forme et le style des graphiques. Cette fonctionnalité simplifie le processus de maintien des présentations à jour avec du contenu dynamique et garantit que les graphiques reflètent fidèlement les données actuelles et les normes visuelles.
...
Lorsque vous utilisez des repères par défaut dans les graphiques, chaque série de graphique obtient automatiquement un symbole de repère par défaut différent.
...
Aspose.Slides pour .NET prend en charge un large éventail de types de graphiques, notamment les graphiques à barres, linéaires, circulaires, en aires, en nuage de points, les histogrammes, les radars et bien d’autres. Cette flexibilité vous permet de choisir le type de graphique le plus adapté à vos besoins de visualisation de données.

**Comment ajouter un nouveau graphique à une diapositive ?**

Pour ajouter un graphique, vous créez d'abord une instance de la classe [Presentation](https://reference.aspose.com/slides/net/aspose.slides/presentation), récupérez la diapositive souhaitée en utilisant son indice, puis appelez la méthode pour ajouter un graphique, en spécifiant le type de graphique et les données initiales. Ce processus intègre le graphique directement dans votre présentation.

**Comment mettre à jour les données affichées dans un graphique ?**

Vous pouvez mettre à jour les données d’un graphique en accédant à son classeur de données ([IChartDataWorkbook](https://reference.aspose.com/slides/net/aspose.slides.charts/ichartdataworkbook/)), en effaçant toute série et catégorie par défaut, puis en ajoutant vos données personnalisées. Cela vous permet d’actualiser programmétiquement le graphique pour refléter les dernières données.

**Est‑il possible de personnaliser l’apparence du graphique ?**

Oui, Aspose.Slides pour .NET propose de nombreuses options de personnalisation. Vous pouvez modifier les couleurs, les polices, les étiquettes, les légendes et d’autres éléments de mise en forme pour adapter l’apparence du graphique à vos exigences de conception spécifiques.
