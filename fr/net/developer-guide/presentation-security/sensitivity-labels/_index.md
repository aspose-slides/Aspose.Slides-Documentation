---
title: Gérer les libellés de sensibilité dans les présentations PowerPoint en .NET
...
Les libellés de sensibilité Microsoft Purview aident les organisations à classer et à gouverner les documents. Lors du traitement automatisé d’une présentation, une application peut devoir conserver un libellé existant, appliquer un libellé sélectionné par une stratégie, mettre à jour son état ou migrer les métadonnées de libellé écrites par un flux de travail Microsoft Information Protection (MIP) plus ancien.
...
Les identifiants de libellé de sensibilité et les informations de stratégie sont définis par votre configuration Microsoft Purview. Validez la disponibilité des libellés et les exigences de stratégie dans votre environnement avant d’ajouter ou de migrer des métadonnées. Les valeurs de [ISensitivityLabel.ContentMarkTypes](https://reference.aspose.com/slides/fr/net/aspose.slides/isensitivitylabel/contentmarktypes/) décrivent les marques de contenu associées à un libellé ; elles n’ajoutent pas, à elles seules, de texte ou de formes visibles aux diapositives.
{{% /alert %}}

## **Comprendre les propriétés des libellés de sensibilité**
...
| [ISensitivityLabel.AssignmentMethodType](https://reference.aspose.com/slides/fr/net/aspose.slides/isensitivitylabel/assignmentmethodtype/) | Spécifie si le libellé a été appliqué automatiquement ou suite à une décision utilisateur. |
| [ISensitivityLabel.ContentMarkTypes](https://reference.aspose.com/slides/fr/net/aspose.slides/isensitivitylabel/contentmarktypes/) | Répertorie les types de marques de contenu associés au libellé. |
...
- [SensitivityLabelAssignmentType.Privileged](https://reference.aspose.com/slides/fr/net/aspose.slides/sensitivitylabelassignmenttype/) représente un libellé appliqué suite à une décision utilisateur, y compris les libellés appliqués manuellement, recommandés et obligatoires.

L’énumération [SensitivityLabelContentType](https://reference.aspose.com/slides/fr/net/aspose.slides/sensitivitylabelcontenttype/) identifie la marque associée à un libellé :
...
| [SensitivityLabelContentType.Header](https://reference.aspose.com/slides/fr/net/aspose.slides/sensitivitylabelcontenttype/) | Une marque de contenu d’en-tête est associée au libellé. |
| [SensitivityLabelContentType.Footer](https://reference.aspose.com/slides/fr/net/aspose.slides/sensitivitylabelcontenttype/) | Une marque de contenu de pied de page est associée au libellé. |
| [SensitivityLabelContentType.Watermark](https://reference.aspose.com/slides/fr/net/aspose.slides/sensitivitylabelcontenttype/) | Une marque de contenu de filigrane est associée au libellé. |
| [SensitivityLabelContentType.Encryption](https://reference.aspose.com/slides/fr/net/aspose.slides/sensitivitylabelcontenttype/) | Une protection par chiffrement est associée au libellé. |

Plusieurs types de marques peuvent être associés à un même libellé.

## **Lister les libellés de sensibilité existants**

Lisez la collection de libellés modernes depuis [Presentation.SensitivityLabels](https://reference.aspose.com/slides/fr/net/aspose.slides/presentation/sensitivitylabels/) et énumérez‑la. L’exemple suivant répertorie chaque propriété et chaque marque de contenu stockées pour chaque libellé :
...
## **Ajouter un libellé de sensibilité avec une marque de contenu**

Utilisez [ISensitivityLabelCollection.Add](https://reference.aspose.com/slides/fr/net/aspose.slides/isensitivitylabelcollection/add/) avec l’identifiant du libellé, l’identifiant du site, l’état d’activation et la méthode d’attribution. Après le retour de la méthode, le nouvel [ISensitivityLabel](https://reference.aspose.com/slides/fr/net/aspose.slides/isensitivitylabel/) est créé ; ajoutez les valeurs de marques requises via [ISensitivityLabel.ContentMarkTypes](https://reference.aspose.com/slides/fr/net/aspose.slides/isensitivitylabel/contentmarktypes/).

L’exemple suivant ajoute un libellé sélectionné manuellement, associé aux marques de pied de page et de filigrane, puis enregistre le résultat au format PPTX :
...
Les propriétés de [ISensitivityLabel](https://reference.aspose.com/slides/fr/net/aspose.slides/isensitivitylabel/) sont en lecture/écriture, sauf que la collection retournée par [ISensitivityLabel.ContentMarkTypes](https://reference.aspose.com/slides/fr/net/aspose.slides/isensitivitylabel/contentmarktypes/) est modifiée via ses opérations de liste. Après avoir localisé le libellé requis, vous pouvez mettre à jour son identifiant, son identifiant de site, son état d’activation, sa méthode d’attribution, son état de suppression et ses types de marques de contenu. Enregistrez la présentation pour persister les modifications.
...
## **Lire et migrer les libellés de sensibilité MIP hérité**

Les anciens flux de travail basés sur MIP peuvent stocker les métadonnées de libellés de sensibilité dans les propriétés personnalisées du document au lieu de la collection moderne. Lisez ces métadonnées avec [IDocumentProperties.GetSensitivityLabels](https://reference.aspose.com/slides/fr/net/aspose.slides/idocumentproperties/getsensitivitylabels/). La méthode analyse les propriétés personnalisées héritées et renvoie un tableau d’objets [ISensitivityLabel](https://reference.aspose.com/slides/fr/net/aspose.slides/isensitivitylabel/).

Pour migrer les métadonnées, ajoutez chaque libellé retourné à la collection moderne [ISensitivityLabelCollection](https://reference.aspose.com/slides/fr/net/aspose.slides/isensitivitylabelcollection/) via [ISensitivityLabelCollection.Add](https://reference.aspose.com/slides/fr/net/aspose.slides/isensitivitylabelcollection/add/). Comme l’ajout d’un identifiant de libellé dupliqué lève une exception, l’exemple vérifie la collection de destination avant de copier chaque libellé. Vous pouvez ajouter une validation supplémentaire pour confirmer que chaque libellé hérité existe toujours dans la stratégie Purview actuelle.
...
La migration copie les objets de libellé analysés dans la collection moderne. Elle ne nécessite pas de nettoyer toutes les propriétés personnalisées du document, de sorte que les métadonnées de document non liées restent intactes. Utilisez [IPresentation.Save](https://reference.aspose.com/slides/fr/net/aspose.slides/ipresentation/save/) avec [SaveFormat.Pptx](https://reference.aspose.com/slides/fr/net/aspose.slides.export/saveformat/) pour écrire les métadonnées de libellés modernes dans un fichier PPTX.

## **FAQ**

**L’ajout d’un type de marque de contenu crée‑t‑il un en‑tête, un pied de page ou un filigrane visible sur les diapositives ?**

Non. Les valeurs ajoutées via [ISensitivityLabel.ContentMarkTypes](https://reference.aspose.com/slides/fr/net/aspose.slides/isensitivitylabel/contentmarktypes/) décrivent les marques associées au libellé de sensibilité. Elles ne créent pas de texte ou de formes visibles dans la présentation. Ajoutez séparément le contenu de diapositive correspondant si votre flux de travail doit afficher ces marques.
...
Enregistrez la présentation au format PPTX en appelant [IPresentation.Save](https://reference.aspose.com/slides/fr/net/aspose.slides/ipresentation/save/) avec [SaveFormat.Pptx](https://reference.aspose.com/slides/fr/net/aspose.slides.export/saveformat/), comme illustré dans les exemples ci‑dessus.