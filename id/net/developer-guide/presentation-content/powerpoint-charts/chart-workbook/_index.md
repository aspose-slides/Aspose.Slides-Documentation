...
Artikel ini juga mencakup penggunaan workbook eksternal sebagai sumber data diagram. Contoh‑contoh menunjukkan cara membuat dan menetapkan workbook eksternal, mengambil jalur workbook eksternal yang ditautkan ke diagram, serta menyunting data diagram ketika workbook tersedia.

Untuk sel workbook yang mewakili data yang hilang, lihat [Mengontrol Tampilan Sel Kosong](/slides/id/net/chart-series/) untuk perbedaan antara sel kosong dan nol, serta perbandingan diagram garis dari mode tampilan yang tersedia.
...
Kode C# berikut mendemonstrasikan contoh operasi:
...
Saat Anda mengganti workbook yang disisipkan dengan workbook yang telah dimodifikasi, diagram tetap mempertahankan koleksi seri dan kategori aslinya. Ketidaksesuaian ini dapat menyebabkan [IChart.ValidateChartLayout](https://reference.aspose.com/slides/id/net/aspose.slides.charts/ichart/validatechartlayout/) gagal dengan kesalahan indeks di luar jangkauan. Bersihkan seri dan kategori yang ada sebelum menulis kembali workbook yang diperbarui ke diagram.
...
Membersihkan koleksi memastikan bahwa struktur data diagram konsisten dengan workbook baru, sehingga `ValidateChartLayout` dapat selesai tanpa error.
...
Kode C# berikut menunjukkan cara menetapkan sel workbook sebagai label data diagram:
...
// Menciptakan instance kelas presentasi yang mewakili file presentasi 
...
Kode C# berikut mendemonstrasikan operasi di mana properti [IChartDataWorkbook.Worksheets](https://reference.aspose.com/slides/id/net/aspose.slides.charts/ichartdataworkbook/properties/worksheets) digunakan untuk mengakses koleksi lembar kerja:
...
using (var presentation = new Presentation("sample.pptx"))
{
    ...
    foreach (var shape in slide.Shapes)
    {
        ...
        {
            // Workbook yang disisipkan berformat .xlsb, yang tidak didukung.
            continue;
        }
...
Kode C# berikut mendemonstrasikan proses pembuatan workbook eksternal:
...
Dengan metode **`SetExternalWorkbook`**, Anda dapat menetapkan workbook eksternal ke diagram sebagai sumber datanya. Metode ini juga dapat digunakan untuk memperbarui jalur ke workbook eksternal (jika workbook tersebut telah dipindahkan).

Meskipun Anda tidak dapat menyunting data di workbook yang disimpan di lokasi remote atau sumber daya, Anda tetap dapat menggunakan workbook tersebut sebagai sumber data eksternal. Jika jalur relatif untuk workbook eksternal disediakan, jalur tersebut akan secara otomatis dikonversi menjadi jalur penuh.

Kode C# berikut menunjukkan cara menetapkan workbook eksternal:
...
* Ketika nilai `ChartData` diatur ke `false`, hanya jalur workbook yang diperbarui — data diagram tidak akan dimuat atau diperbarui dari workbook target. Anda dapat menggunakan pengaturan ini ketika workbook target tidak ada atau tidak tersedia.  
* Ketika nilai `ChartData` diatur ke `true`, data diagram akan diperbarui dari workbook target.
...
### **Mendapatkan Jalur Workbook Sumber Data Eksternal dari Diagram**
...
Anda dapat menyunting data di workbook eksternal dengan cara yang sama seperti mengubah isi workbook internal. Ketika workbook eksternal tidak dapat dimuat, sebuah pengecualian akan dilemparkan.

Kode C# berikut merupakan implementasi proses yang dijelaskan:
...
Tidak. Presentasi menyimpan sebuah [tautan ke file eksternal](https://reference.aspose.com/slides/id/net/aspose.slides.charts/chartdata/externalworkbookpath/) dan menggunakannya untuk membaca data. File eksternal itu sendiri tidak dimodifikasi saat presentasi disimpan.

**Apa yang harus saya lakukan jika file eksternal dilindungi kata sandi?**

Aspose.Slides tidak menerima kata sandi saat menautkan. Pendekatan umum adalah menghilangkan perlindungan terlebih dahulu atau menyiapkan salinan yang telah didekripsi (misalnya dengan menggunakan [Aspose.Cells](/cells/net/)) dan menautkan ke salinan tersebut.

**Dapatkah beberapa diagram merujuk ke workbook eksternal yang sama?**

Ya. Setiap diagram menyimpan tautannya masing‑masing. Jika semuanya menunjuk ke file yang sama, memperbarui file tersebut akan tercermin pada setiap diagram pada saat data dimuat kembali.