...
- C#
- Aspose.Slides
description: "Конвертировать презентации PowerPoint в адаптивный HTML в .NET. Сохранить макет, ссылки и изображения с помощью руководства по конвертации Aspose.Slides для быстрых и безупречных результатов."
---

## **Обзор**

Повышайте эффективность рабочего процесса, преобразуя презентации PowerPoint и OpenDocument в HTML с помощью Aspose.Slides для .NET. Это руководство предоставляет подробные инструкции, надёжные примеры кода и проверенные методы, гарантируя надёжный и эффективный процесс конвертации, оптимизированный для просмотра в вебе.

Aspose.Slides предлагает множество параметров — в основном из класса [**HtmlOptions**](https://reference.aspose.com/slides/net/aspose.slides.export/htmloptions) — которые определяют процесс преобразования из формата PowerPoint (или OpenDocument) в HTML:
...
Этот код показывает, как преобразовать презентацию PowerPoint в HTML на C#:
```c#
// Создать экземпляр класса Presentation, представляющего файл презентации (например, PPT, PPTX, ODP и т.д.).
...
При преобразовании презентации PowerPoint или OpenDocument в HTML с заметками докладчика важно полностью сохранить смысл оригинального документа. Этот процесс гарантирует, что не только визуальные элементы слайдов точно воспроизводятся, но и сопутствующие заметки докладчика сохраняются, обогащая контент дополнительным контекстом и инсайтами.
...
Этот код демонстрирует, как преобразовать презентацию PowerPoint в HTML с заметками докладчика на C#:
```c#
using (Presentation presentation = new Presentation("presentation.pptx"))
{
...
Aspose.Slides предоставляет класс [EmbedAllFontsHtmlController](https://reference.aspose.com/slides/net/aspose.slides.export/embedallfontshtmlcontroller), который позволяет встроить все шрифты презентации при её конвертации в HTML.

Чтобы исключить из встраивания определённые шрифты, вы можете передать массив имён шрифтов в параметризованный конструктор класса [EmbedAllFontsHtmlController](https://reference.aspose.com/slides/net/aspose.slides.export/embedallfontshtmlcontroller). Популярные шрифты, такие как Calibri или Arial, не требуют встраивания, так как они уже присутствуют в большинстве систем. Их встраивание лишь увеличит размер получаемого HTML‑документа.
...
Этот код на C# демонстрирует, как преобразовать презентацию PowerPoint в HTML с изображениями высокого качества в 150 DPI (`PicturesCompression.Dpi150`):
```c#
using (Presentation presentation = new Presentation("input.pptx"))
{
    HtmlOptions htmlOptions = new HtmlOptions
...
## **Сохранять CSS и изображения при экспорте в HTML**

Используя новые файлы стилей CSS, вы легко можете изменить внешний вид HTML‑файла, полученного в результате конвертации PowerPoint‑в‑HTML.

Код на C# в этом примере демонстрирует, как с помощью переопределяемых методов создать пользовательский HTML‑документ, включающий ссылку на файл CSS:
```c#
...
Если вы не хотите встраивать шрифты (чтобы не увеличивать размер итогового HTML), можно создать ссылки на все шрифты, реализовав собственную версию `LinkAllFontsHtmlController`.

Этот код на C# показывает, как преобразовать презентацию PowerPoint в HTML, создав ссылки на все шрифты и исключив «Calibri» и «Arial» (поскольку они уже установлены в системе):
```c#
using (Presentation presentation = new Presentation("pres.pptx"))
{
    // Исключить шрифты презентации по умолчанию.
...
Этот код на C# демонстрирует реализацию `LinkAllFontsHtmlController`:
```c#
...
Ограничений на количество слайдов при использовании Aspose.Slides нет. Вы можете конвертировать презентации любого размера. Однако для презентаций с очень большим числом слайдов производительность может зависеть от доступных ресурсов вашего сервера или системы.