...
Начиная с версии [Aspose.Slides 23.2](https://www.nuget.org/packages/Aspose.Slides.NET/23.2.0), реализована поддержка .NET6. Особенность этой поддержки в том, что .NET6 более не поддерживает System.Drawing.Common для Linux ([изменение, несовместимое с предыдущими версиями](https://learn.microsoft.com/en-us/dotnet/core/compatibility/core-libraries/6.0/system-drawing-common-windows-only)), а Slides реализует эту графическую подсистему самостоятельно как компонент C++.
...
.NET6 является предпочтительной версией для Aspose.Slides, используемых в облаке (AWS, Azure или других облачных решениях).

Ранее, когда Aspose.Slides использовался на Linux‑хосте, требовалась установка дополнительных зависимостей (libgdiplus), что часто было неудобно или непрактично (например, при использовании [AWS Lambda](https://aws.amazon.com/lambda)). С Slides для .NET6 эти зависимости больше не нужны, поэтому развертывание становится гораздо проще.

Еще один аспект — проблемы, возникавшие при использовании Aspose.Slides в облачном решении с Windows‑хостом. Например, у [Azure Functions](https://learn.microsoft.com/en-us/azure/azure-functions/functions-overview) есть ограничения для процесса, что приводит к проблемам при экспорте PDF (см. [это](https://github.com/projectkudu/kudu/wiki/Azure-Web-App-sandbox#unsupported-frameworks)). Использование Aspose.Slides для .NET6 решает эту проблему.
...
Иногда в проекте необходимо одновременно использовать зависимости System.Drawing и Slides для .NET6 (например, когда проект .NET6 зависит от других пакетов, которые в свою очередь зависят от System.Drawing). Это может вызывать ошибки, такие как:
...
Начиная с версии 24.8 удалён устаревший публичный API с зависимостями от System.Drawing. Что касается приведённого выше примера кода, изображение слайда можно получить следующим образом.
```cs
...
Новый API описан более подробно в [Modern API](/net/modern-api/).