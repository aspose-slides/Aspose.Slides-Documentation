...
- C#
- Aspose.Slides
description: "Управление слайдами в C# с Aspose.Slides: создание, клонирование, перестановка, скрытие, установка фоновых изображений и размеров, применение переходов и экспорт в PowerPoint и OpenDocument."
---

Эта статья предоставляет набор примеров, демонстрирующих, как работать со слайдами с помощью **Aspose.Slides for .NET**. Вы узнаете, как добавлять, получать доступ, клонировать, переупорядочивать и удалять слайды с использованием класса `Presentation`.
...
static void Add_Slide()
{
    ...
    // Каждый слайд основан на макете, который сам по себе основан на мастер‑слайде.
...
{
    // По умолчанию презентация создаётся с одним пустым слайдом
    using var pres = new Presentation();
...
{
    // По умолчанию в презентации есть один пустой слайд
    using var pres = new Presentation();
    ...
    // Индекс клонированного слайда — 1 (второй слайд в презентации)
...
static void ReOrder_Slide()
{
    ...
    // Переместите клонированный слайд в первую позицию (остальные сдвинутся вниз)
