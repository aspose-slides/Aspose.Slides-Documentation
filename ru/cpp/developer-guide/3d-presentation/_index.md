...
## **Обзор**
Начиная с Aspose.Slides 20.9 возможно создавать и изменять 3D‑модели PowerPoint. Это достигается путем применения набора 3D‑эффектов к 2D‑формам. Создавая вид камеры на форме, вы можете вращать её вокруг оси. Создайте экструдирование или глубину формы, что преобразует её из 2D‑фигуры в 3D‑модель. Настройка светового эффекта на 3D‑форме или изменение материалов делает её более живой. Изменение цветов 3D‑моделей на 3D‑градиент, модификация контура форм, добавление фаски придают 3D‑модели объём. Все 3D‑эффекты могут быть применены как к 3D‑моделям PowerPoint, так и к тексту.
...
## **3D‑вращение**
В PowerPoint вращение формы осуществляется через:

![todo:image_alt_text](img_02_01.png)

Чтобы вращать 3D‑модели PowerPoint, необходимо создать вид камеры на форме. Это делается методом [IThreeDFormat.get_Camera()](https://reference.aspose.com/slides/cpp/class/aspose.slides.three_d_format#ad2f989bd1fd64fd4136e1f17660035d4). Метод вращения вызывается из класса камеры, как если бы вы вращали саму камеру. Фактически, когда вы вращаете камеру относительно формы, вы вращаете форму в 3D‑пространстве.
```cpp
auto shape = slide->get_Shapes()->AddAutoShape(ShapeType::Rectangle, 200.0f, 150.0f, 200.0f, 200.0f);
shape->get_ThreeDFormat()->get_Camera()->SetRotation(20.0f, 30.0f, 40.0f);
...
## **3D‑градиент**
Создать 3D‑градиент на 3D‑модели PowerPoint можно с помощью метода [Shape.get_FillFormat().get_GradientFormat()](https://reference.aspose.com/slides/cpp/class/aspose.slides.fill_format#a1f075336cb7a0e05cd5d7a706b6f4f58):
``` cpp
using namespace Aspose::Slides;
...
## **3D‑текст (WordArt)**
Чтобы применить вращение, экструдирование, свет, градиент к тексту и превратить его в 3D‑текст (WordArt), необходимо обратиться к методу [IAutoShape.get_TextFrame().get_TextFrameFormat().get_ThreeDFormat()](https://reference.aspose.com/slides/cpp/class/aspose.slides.i_text_frame_format#a5e681109403c2e57aa76a500fe508b30):
``` cpp
using namespace Aspose::Slides;
using namespace Aspose::Slides::Export;
...
**Могу ли я получить «эффективные» (финальные) значения 3D‑параметров, учитывающие темы, наследование и пр.?**
...
Да. При [генерации кадров для видео](/slides/ru/cpp/convert-powerpoint-to-video/) 3D‑эффекты рендерятся так же, как и при [экспорте изображений](/slides/ru/cpp/convert-powerpoint-to-png/).