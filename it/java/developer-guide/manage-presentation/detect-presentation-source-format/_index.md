---
title: Determinare il Formato Originale della Presentazione in Java
linktitle: Formato di Origine
type: docs
weight: 35
url: /it/java/detect-presentation-source-format/
keywords:
- formato di origine
...
- Java
- Aspose.Slides
description: "Leggi il formato originale di una presentazione caricata in Java con Aspose.Slides per Java, confronta le API di rilevamento e gestisci file, stream e formati legacy."
---
## **Panoramica**

Dopo aver caricato una presentazione, chiama il metodo [Presentation.getSourceFormat](https://reference.aspose.com/slides/it/java/com.aspose.slides/presentation/#getSourceFormat--) per determinare il suo formato originale. Il metodo è disponibile anche tramite [IPresentation.getSourceFormat](https://reference.aspose.com/slides/it/java/com.aspose.slides/ipresentation/#getSourceFormat--). Usalo quando l'elaborazione successiva dipende dal formato da cui è stata caricata l'istanza corrente.

Il formato di origine è distinto dal [SaveFormat](https://reference.aspose.com/slides/it/java/com.aspose.slides/saveformat/) selezionato per un file di output. Il salvataggio in un altro formato non modifica il formato di origine dell'istanza esistente.

## **Leggere il Formato di Origine di un File**

Questo esempio richiede un file `sample.pptx` esistente. Carica il file e seleziona una policy di elaborazione dell'applicazione usando [Presentation.getSourceFormat](https://reference.aspose.com/slides/it/java/com.aspose.slides/presentation/#getSourceFormat--), anziché il nome file. Modifica il percorso di input per provare altri formati. L'esempio stampa la policy selezionata; sostituisci i messaggi con la logica della tua applicazione.
...
La classe [SourceFormat](https://reference.aspose.com/slides/it/java/com.aspose.slides/sourceformat/) definisce costanti intere che distinguono i seguenti formati di presentazione. Le estensioni di seguito sono estensioni convenzionali, non una ricostruzione del nome file originale.
...
| `Pps` | `.pps` | Presentazione diapositiva PowerPoint 97–2003 |
| `Ppsx` | `.ppsx` | Presentazione diapositiva Office Open XML |
| `Ppsm` | `.ppsm` | Presentazione diapositiva Office Open XML con macro |
...
Questo esempio richiede un file `sample.pps` esistente. Leggere i suoi byte in uno stream di memoria simula un input ricevuto senza nome file, come un valore di database o un array di byte caricato. Il costruttore [Presentation](https://reference.aspose.com/slides/it/java/com.aspose.slides/presentation/) riceve solo lo stream.
...
Se la tua applicazione deve preservare la distinzione, conserva separatamente il nome file originale o i metadati del sottotipo. Un'estensione è un indizio utile per questi sottotipi legacy, ma non dovrebbe essere l'unico criterio per identificare contenuti di presentazione arbitrari.

## **Confrontare il Rilevamento Prima e Dopo il Caricamento**

Usa [PresentationFactory.getPresentationInfo](https://reference.aspose.com/slides/it/java/com.aspose.slides/presentationfactory/#getPresentationInfo-java.lang.String-) e [IPresentationInfo.getLoadFormat](https://reference.aspose.com/slides/it/java/com.aspose.slides/ipresentationinfo/#getLoadFormat--) quando è necessario ispezionare un file prima di caricare il suo modello completo di oggetti di presentazione. Usa [Presentation.getSourceFormat](https://reference.aspose.com/slides/it/java/com.aspose.slides/presentation/#getSourceFormat--) quando l'istanza è già esistente.

Questo esempio richiede `sample.pptx` e stampa i valori interi di `LoadFormat.Pptx` e `SourceFormat.Pptx`, rispettivamente. In produzione, scegli l'API appropriata al tuo stadio di elaborazione; una presentazione già caricata non necessita di una seconda ispezione solo per ottenere il suo formato di origine.
...
Questa mappatura non converte un file né recupera un sottotipo legacy PPS/POT perso durante il caricamento da stream. Per il salvataggio effettivo, seleziona esplicitamente un [SaveFormat](https://reference.aspose.com/slides/it/java/com.aspose.slides/saveformat/), o usa la conversione mostrata in [Save Presentations in Their Original Format](/slides/it/java/save-presentation/#save-presentations-in-their-original-format).

## **Verificare i Formati Salvando e Riaprendo**

Questo esempio autoconclusivo crea una presentazione e scrive tre file nella directory di lavoro, sovrascrivendo file con gli stessi nomi. Riapre ciascun output sia tramite percorso sia tramite uno stream di memoria. Per PPTX e ODP, entrambe le vie segnalano il formato salvato. Per PPS, il caricamento per percorso segnala `Pps`, mentre il caricamento degli stessi byte senza nome file segnala `Ppt`.
...
La tabella seguente riassume l'identificazione del formato di origine per presentazioni con estensioni corrispondenti. I nomi indicano costanti; gli esempi Java stampano i loro valori interi:
...
**Uno stream può sempre distinguere una presentazione legacy, una presentazione diapositiva e un modello?**

No. PPT, PPS e POT condividono lo stesso formato binario. Conserva separatamente il nome file o i metadati del sottotipo quando è necessario fare questa distinzione.

**Quale API devo utilizzare se la presentazione è già caricata?**

Leggi [Presentation.getSourceFormat](https://reference.aspose.com/slides/it/java/com.aspose.slides/presentation/#getSourceFormat--). Usa [PresentationFactory.getPresentationInfo](https://reference.aspose.com/slides/it/java/com.aspose.slides/presentationfactory/#getPresentationInfo-java.lang.String-) per l'ispezione prima del caricamento.