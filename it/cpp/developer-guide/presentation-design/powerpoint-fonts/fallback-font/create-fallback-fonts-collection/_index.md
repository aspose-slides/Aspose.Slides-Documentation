---
title: Configura le raccolte di font di fallback in C++
...
- C++
- Aspose.Slides
description: "Configura una raccolta di font di fallback in Aspose.Slides per C++ per mantenere il testo coerente e nitido nelle presentazioni PowerPoint e OpenDocument."
---
## **Panoramica**
...
Dopo aver creato la raccolta, è possibile assegnarla utilizzando il metodo `set_FontFallBackRulesCollection` del `FontsManager` della presentazione. Il `FontsManager` controlla i font in tutta la presentazione, e ogni istanza di `Presentation` dispone del proprio `FontsManager`.
...
{{% alert color="primary" %}} 
Leggi di più su come [Renderizza la presentazione con il font di fallback](/slides/it/cpp/render-presentation-with-fallback-font/).
{{% /alert %}}
...
No. È necessario aggiungere e utilizzare i font dal proprio lato, sotto la propria responsabilità.

**È possibile utilizzare contemporaneamente la sostituzione/sostituzione per font mancanti e il fallback per glifi mancanti?**
