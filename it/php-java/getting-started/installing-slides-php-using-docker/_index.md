---
title: Installa Aspose.Slides per PHP tramite Java usando Docker
type: docs
weight: 75
url: /it/php-java/installing-slides-php-using-docker/
keywords:
...
- PHP
- Aspose.Slides
description: "Esegui Aspose.Slides in container Docker: configura le immagini, le dipendenze, i font e le licenze per creare servizi scalabili che elaborano PowerPoint e OpenDocument."
---
## **Prerequisiti**
...
### **2. Costruisci l'immagine Docker**
   Esegui il seguente comando nella directory dove si trova il tuo Dockerfile per costruire l'immagine Docker:
   ```bash
   docker build -t aspose-slides-php-java .
   ```

### **3. Avvia il container Docker**
   Una volta costruita l'immagine, avvia il container:
   ```bash
   docker run -p 8080:8080 aspose-slides-php-java
   ```

### **4. Accesso a Aspose.Slides in Docker** 
   Dopo aver avviato il container, lo script genererà un file PDF. Puoi trovare il file di output generato `output.pdf` nella cartella `/tmp` all'interno del container:
   ```bash
   docker exec -it <container-id> ls /tmp
   ```
   Per copiare il file PDF generato sulla tua macchina locale, esegui il seguente comando:
   ```bash
