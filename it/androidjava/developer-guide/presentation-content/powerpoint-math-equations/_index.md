...
In PowerPoint, gli utenti normalmente aggiungono le equazioni tramite **Inserisci > Equazione**:
...
![Un'espressione radice n-esima con x sotto il segno radicale](powerpoint-math-equations_5.png)
...
Gli operatori N-ari sono per grandi operatori con limiti opzionali. Operatori semplici come `+`, `-` e `=` sono solitamente aggiunti come `MathematicalText` e uniti nell'espressione.
...
Usa [MathMatrix](https://reference.aspose.com/slides/it/androidjava/com.aspose.slides/mathmatrix/) per righe e colonne. Le matrici non includono parentesi graffe per impostazione predefinita, quindi racchiudi la matrice quando sono necessarie parentesi tonde, quadre o graffe.
...
Usa `toMathArray` quando sono necessarie equazioni allineate o una pila verticale di espressioni.
...
Usa `enclose` per inserire un'espressione all'interno di delimitatori. È inoltre possibile impostare un carattere di separazione per le espressioni delimitate che contengono diversi elementi.
